...
| `weather_lut_upload` | `simulation/systems/weather_lut_upload_system.gd` | 将 `cell.weather_type/intensity/cloud/precip` 发布到 `WorldData.weather_lut_tex`，维护 prev/current 双缓冲和 `weather_lut_update_usec`。 | GDScript ImageTexture upload；按 weather dirty mask 触发，DCSystem 拓扑上读取 weather 字段，因此跟随 `weather_refresh` 而非 dynamic visual stride。 |
