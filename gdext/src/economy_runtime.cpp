...
bool NativeEconomyRuntime::capture_trade_topology(
    ...
    for (int32_t cell = 0; cell < count; ++cell) {
        ...
        enter_cost[cell] = trade_move_cost_lut[terrain_id];
    ...
    if (_trade_topology.ready && _trade_topology.topology_hash == normalized_hash &&
        (generation == 0 || _trade_topology.topology_generation == generation)) return true;
    const uint64_t resolved_generation = generation != 0 ? generation
        : std::max<uint64_t>(1, _trade_topology.topology_generation + 1);
    ...
    _trade_topology.component_country_generation = 0;
...
bool NativeEconomyRuntime::configure_profile(const Dictionary &profile, std::string &error) {
    ...
    _merchant_procurement_cash_reserve_q16 = std::clamp(
        dict_num<int32_t>(profile, "merchant_procurement_cash_reserve_q16", 16384),
        0, static_cast<int32_t>(Q16_ONE));
    _merchant_market_making_days_q16 = std::clamp(
        dict_num<int32_t>(profile, "merchant_market_making_days_q16", Q16_ONE),
        0, static_cast<int32_t>(Q16_ONE * 8));
    ...
    _trade_signal_pairs_per_slice = std::clamp(dict_num<int32_t>(
        profile, "trade_signal_pairs_per_slice", 16384), 256, 1 << 20);
    _trade_route_searches_per_slice = std::clamp(dict_num<int32_t>(
        profile, "trade_route_searches_per_slice", 2), 1, 64);
    ...
    for (const BuildingGroup &group : _buildings) {
        ...
        for (int32_t i = 0; i < type.input_count; ++i) {
            ...
            const int64_t scaled_numerator = saturating_add(saturating_mul(
                full_effective, utilization_q16, _saturation_count),
            ...
            _production_input_reserve[signal] = saturating_add(
                _production_input_reserve[signal], physical, _saturation_count);
        }
    }
    for (int32_t cell = 0; cell < _cell_count; ++cell) {
        if (_market_signals.cell_offsets.size() != static_cast<size_t>(_cell_count + 1)) break;
        ...
        for (int32_t signal = _market_signals.cell_offsets[cell];
            ...
            const int64_t reserve = _production_input_reserve[signal];
            _production_input_reserved = saturating_add(
                _production_input_reserved, reserve, _saturation_count);
            const int32_t good = _market_signals.good_ids[signal];
            _production_input_reserve_shortfall = saturating_add(
                _production_input_reserve_shortfall, std::max<int64_t>(
                    0, reserve - _market.stock[_market.index(market, good)]),
                _saturation_count);
        }
    }
}
...
bool NativeEconomyRuntime::prepare_cell_wages(int32_t cell, std::string &error) {
    ...
    for (int32_t signal = _labor_signals.cell_offsets[cell];
        ...
        for (int32_t g = begin; g < end; ++g) {
            ...
            if (!building_available(cell, group.type_id, true)) continue;
            const BuildingType &type = _building_types[group.type_id];
            // Affordability damping: derive a per-employee wage ceiling from the
            // building's expected revenue divided across all its employee slots,
            // scaled by _wage_income_cap_ratio_q16. A rational employer cannot
            // sustainably pay wages far above the revenue each employee supports,
            // so the living-cost floor is not allowed to push contract wages past
            // this ceiling. Falls back to no cap (ceiling=0) when revenue is not
            // yet known (e.g. first cycle) so the loss/suspension machinery, not a
            // wage collapse, handles genuinely unaffordable buildings.
            ...
            if (_wage_income_cap_ratio_q16 > 0) {
                ...
                const int64_t basis = group.last_expected_revenue > 0
                    ? group.last_expected_revenue : group.last_revenue;
                if (basis > 0 && group_employee_slots > 0) {
                    const int64_t per_emp = basis / group_employee_slots;
                    affordable_ceiling = mul_div_sat(per_emp,
    ...
    for (BuildingGroup &group : _buildings) {
        ...
        if (group.operating_state == 0) {
            ...
            for (int32_t i = 0; i < type.output_count; ++i) {
                ...
                if (household_available_stock <= STOCK_ROUNDING_TOLERANCE &&
                ...
                int64_t feasible = _market.demand_ema[market_index];
                if (signal >= 0) feasible = saturating_add(
                    feasible, _market_signals.business_demand_ema[signal],
                    _saturation_count);
                int64_t target = mul_div_sat(saturating_add(
                    realized, exports, _saturation_count),
                    _good_target_inventory_days_q16[good], Q16_ONE,
                    _saturation_count);
                target = saturating_add(target, saturating_mul(
                    std::max<int64_t>(0, feasible - realized),
                    std::max(1, _epoch_days), _saturation_count),
                    _saturation_count);
                if (realized == 0 && exports == 0 && signal >= 0) {
                    target = std::max(target, mul_div_sat(
                        _market_signals.offered_supply_ema[signal],
                        _merchant_market_making_days_q16, Q16_ONE,
                        _saturation_count));
                }
                if (signal >= 0 && signal < static_cast<int32_t>(
            ...
            if (group.last_output > 0 && sellable_output > 0) {
                ...
                utilization = saturating_add(utilization, mul_div_sat(
                    std::min(target_utilization, inventory_target_utilization) -
                        utilization,
            ...
            group.planned_utilization_q16 = static_cast<int32_t>(
                std::clamp<int64_t>(utilization, probe_floor_q16, Q16_ONE));
        } else {
            group.planned_utilization_q16 = 0;
        }
        group.purchase_intent_capacity_q16 = 0;
...
bool NativeEconomyRuntime::apply_build_command(const Command &cmd, int32_t owner_slot,
    ...
    for (int32_t i = 0; i < type.construction_count; ++i) {
        ...
        _construction_goods_consumed = saturating_add(_construction_goods_consumed, qty,
                                                       _saturation_count);
        const int32_t signal = market_signal_index(cell, item.good_id);
        if (signal >= 0 && signal < static_cast<int32_t>(
...
bool NativeEconomyRuntime::run_building_employment_cell(int32_t cell, std::string &error) {
    ...
    if (first < last) {
        ...
        for (int32_t g = first; g < last; ++g) {
            ...
            const bool active = group.count > 0 &&
                                building_available(cell, group.type_id, true);
            const BuildingType &type = _building_types[group.type_id];
            // owner 目标：不可用/count<=0 → 0（其在岗人口将全部进池）。
            const int64_t owner_target = active ? planned_owner_demand(group, type) : 0;
        ...
        // 更新全局在岗计数器（_filled_owner_jobs / _filled_employee_jobs）。
        // A1 下这些是 cell 内 owner_employed / employee_employed 之和；此处按 cell
        // 重新聚合避免跨周期漂移（旧逻辑每周期从零累加，这里同样每 cell 重扫）。
        // Route B: merchant slots are no longer skipped — a merchant-post owner
        // (owner_employed>0) now counts toward _filled_owner_jobs like any
        // owner. A floating market-maker (invariant fallback, owner_employed==0)
        // contributes nothing, so aggregating it is harmless.
        _population.for_each_in_cell(cell, [&](int32_t slot) {
...
bool NativeEconomyRuntime::run_building_production_cell(int32_t cell, std::string &error) {
    ...
    build_demand_basis(market, retention_environment,
    ...
    for (size_t owner = 0; owner < payroll_owners.size(); ++owner) {
        ...
        if (cold_exposure_q16 > Q16_ONE - _starvation_satisfaction_threshold_q16) {
            clothing_retention_q16 = Q16_ONE - mul_div_sat(
                Q16_ONE - _starvation_satisfaction_threshold_q16,
                Q16_ONE, cold_exposure_q16, _saturation_count);
        }
        int64_t survival_food_desired = 0;
        int64_t survival_staple_desired = 0;
        for (int32_t n = 0; n < plan.need_count; ++n) {
            const int32_t need_index = plan.need_begin + n;
            const Need &need = _needs[need_index];
            ...
            if (std::find(_survival_food_need_stable_ids.begin(),
                ...
                if (stable_need == _survival_staple_need_stable_id) {
                    survival_staple_desired = saturating_add(
                        survival_staple_desired, food_desired, _saturation_count);
                }
                continue;
            }
            if (stable_need != _survival_clothing_need_stable_id ||
                population <= 0 || clothing_retention_q16 <= 0) continue;
            int64_t produced_score_sum = 0;
            for (int32_t v = 0; v < need.variant_count; ++v) {
                const int32_t variant_id = need.variant_begin + v;
                const VariantChoice &variant = _variants[variant_id];
                if (variant.component_count != 1) continue;
                const NeedComponent &component = _components[variant.component_begin];
                const size_t index = owner * static_cast<size_t>(_market.good_count) +
                                     component.good_id;
                if (retention_produced_goods[index] == 0) continue;
                produced_score_sum = saturating_add(
                    produced_score_sum, retention_variant_scores[variant_id],
                    _saturation_count);
            }
            if (produced_score_sum <= 0) continue;
            const int64_t full_desired = survival_required_units(
                owner_slot, stable_need, _epoch_days,
                retention_environment, _saturation_count);
            const int64_t desired = mul_div_sat(
                full_desired, clothing_retention_q16, Q16_ONE, _saturation_count);
            ...
            for (int32_t v = 0; v < need.variant_count; ++v) {
                ...
                const int64_t next = mul_div_sat(
                    desired, score_prefix, produced_score_sum, _saturation_count);
                const int64_t units = std::max<int64_t>(0, next - allocated);
                allocated = next;
                if (units <= 0) continue;
                retention_targets[index] = saturating_add(
                    retention_targets[index], mul_div_sat(
                        units, component.qty_per_need, GOODS_SCALE, _saturation_count),
                    _saturation_count);
            }
        }
        if (produces_survival_food && population > 0) {
            const int64_t desired = produces_staple_food
                ? survival_staple_desired : survival_food_desired;
            const int64_t numerator = saturating_add(saturating_mul(
                desired, _starvation_satisfaction_threshold_q16,
    ...
    thread_local std::vector<int64_t> retained_by_good;
    retained_by_good.assign(_market.good_count, 0);
    ...
    auto process_phase = [&](bool cycle_flow_phase) -> bool {
        ...
        for (int32_t g = begin; g < end; ++g) {
            ...
            const int64_t owner_demand = saturating_mul(
                group.count, type.owner_slots_per_building, _saturation_count);
            int64_t intent_scale_q16 = owner_demand > 0
            ...
            intent_scale_q16 = std::min<int64_t>(
                intent_scale_q16, group.planned_utilization_q16);
            for (int32_t r = 0; r < type.employee_count; ++r) {
                const JobRole &role = _building_employee_roles[type.employee_begin + r];
                ...
                intent_scale_q16 = std::min(intent_scale_q16, role_scale);
            }
            const int64_t building_days = saturating_mul(
                group.count, std::max(1, _epoch_days), _saturation_count);
            int64_t intent_input_cost = 0;
            for (int32_t i = 0; i < type.input_count; ++i) {
                const ProductionInput &item = _building_inputs[type.input_begin + i];
                const int64_t effective = saturating_mul(
                    building_days, item.quantity, _saturation_count);
                const int32_t selected = select_input_candidate(item, false, effective);
                if (selected < 0) {
                    intent_scale_q16 = 0;
                    continue;
                }
                ...
                intent_input_cost = saturating_add(intent_input_cost, mul_div_sat(
                    physical, _market.price[_market.index(market, candidate.good_id)],
                    GOODS_SCALE, _saturation_count), _saturation_count);
            }
            if (intent_input_cost > 0) intent_scale_q16 = std::min(
                intent_scale_q16, mul_div_sat(
                std::max<int64_t>(0, _population.funds[owner_slot]), Q16_ONE,
                intent_input_cost, _saturation_count));
            intent_scale_q16 = std::clamp<int64_t>(intent_scale_q16, 0, Q16_ONE);
            int64_t scale_q16 = intent_scale_q16;
            int64_t actual_full_input_cost = 0;
            for (int32_t i = 0; i < type.input_count; ++i) {
                const ProductionInput &item = _building_inputs[type.input_begin + i];
                const int64_t effective = saturating_mul(
                    building_days, item.quantity, _saturation_count);
                ...
                const InputCandidate &candidate = _building_input_candidates[selected];
                const int64_t physical = physical_input_quantity(effective, candidate);
                if (physical > 0) scale_q16 = std::min(scale_q16, mul_div_sat(
                    _market.stock[_market.index(market, candidate.good_id)], Q16_ONE,
                    physical, _saturation_count));
                actual_full_input_cost = saturating_add(
                    actual_full_input_cost, mul_div_sat(
                        physical, _market.price[_market.index(market, candidate.good_id)],
                        GOODS_SCALE, _saturation_count), _saturation_count);
            }
            if (actual_full_input_cost > 0) scale_q16 = std::min(
                scale_q16, mul_div_sat(
                    std::max<int64_t>(0, _population.funds[owner_slot]), Q16_ONE,
                    actual_full_input_cost, _saturation_count));
            ...
            if (type.behavior_id == 1 || type.behavior_id == 2) {
                for (int32_t i = 0; i < type.resource_count; ++i) {
                    ...
                    intent_scale_q16 = std::min(intent_scale_q16, resource_scale);
        ...
        sellable_by_good.assign(_market.good_count, 0);
        quota_by_good.assign(_market.good_count, 0);
        budget_by_good.assign(_market.good_count, 0);
        spent_by_good.assign(_market.good_count, 0);
        weight_by_good.assign(_market.good_count, 0);
        touched_goods.clear();
        ...
        for (Offer &offer : offers) {
            ...
            if (offer.retained > 0) {
                ...
                retained_by_good[offer.good] = saturating_add(
                    retained_by_good[offer.good], offer.retained, _saturation_count);
            }
        ...
        for (int32_t good : touched_goods) {
            ...
            const int64_t feasible = saturating_add(
                _market.demand_ema[_market.index(market, good)],
                signal >= 0 ? _market_signals.business_demand_ema[signal] : 0,
                _saturation_count);
            int64_t target = mul_div_sat(saturating_add(
                realized, exports, _saturation_count),
                _good_target_inventory_days_q16[good], Q16_ONE, _saturation_count);
            target = saturating_add(target, saturating_mul(
                std::max<int64_t>(0, feasible - realized),
                std::max(1, _epoch_days), _saturation_count), _saturation_count);
            if (realized == 0 && exports == 0) {
                const int64_t daily_offer = sellable_by_good[good] /
                    std::max(1, _epoch_days);
                target = std::max(target, mul_div_sat(
                    daily_offer, _merchant_market_making_days_q16,
                    Q16_ONE, _saturation_count));
            }
            if (signal >= 0 && signal < static_cast<int32_t>(
                    _production_input_reserve.size())) {
            ...
            const int64_t buy_price = std::max<int64_t>(1, mul_div_sat(
                ...
                _good_merchant_buy_factor_q16[good], Q16_ONE, _saturation_count));
        ...
        const int64_t allocated_budget = std::min(
            merchant_procurement_remaining, total_weight);
        int64_t weight_prefix = 0;
        int64_t budget_distributed = 0;
        for (int32_t good : touched_goods) {
            weight_prefix = saturating_add(
                weight_prefix, weight_by_good[good], _saturation_count);
            const int64_t next = total_weight > 0 ? mul_div_sat(
                allocated_budget, weight_prefix, total_weight, _saturation_count) : 0;
            budget_by_good[good] = std::max<int64_t>(0, next - budget_distributed);
        ...
        for (const Offer &offer : offers) {
            ...
            if (issue_value > 0) {
                ...
                supported = _good_storage_modes[offer.good] == 0
                    ? offer.sellable - sold : 0;
                if (supported > 0) {
    ...
    for (int32_t g = begin; g < end; ++g) {
        BuildingGroup &group = _buildings[g];
        const int64_t realized_cost = saturating_add(
            group.last_input_cost, group.last_base_wages_due, _saturation_count);
    ...
    for (int32_t g = begin; g < end; ++g) {
        ...
        for (int32_t i = 0; i < type.input_count; ++i) {
            ...
            business_observed[candidate.good_id] = saturating_add(
                business_observed[candidate.good_id], planned, _saturation_count);
        }
        int64_t reference_total = 0;
    ...
    for (int32_t good = 0; good < _market.good_count; ++good) {
        supply_observed[good] = std::max<int64_t>(
            0, supply_observed[good] - retained_by_good[good]);
    }
    if (_market_signals.cell_offsets.size() == static_cast<size_t>(_cell_count + 1)) {
        for (int32_t signal = _market_signals.cell_offsets[cell];
             signal < _market_signals.cell_offsets[cell + 1]; ++signal) {
...
Dictionary NativeEconomyRuntime::bootstrap(const Dictionary &population_packet,
    ...
    out["approximation_model"] = "production_income_consumption_v10";
...
bool NativeEconomyRuntime::rebuild_trade_components(std::string &error) {
    ...
    if (_trade_topology.component_country_generation == _epoch_country_generation &&
    ...
    _trade_topology.component_country_generation = _epoch_country_generation;
...
bool NativeEconomyRuntime::begin_trade_plan(std::string &error) {
    ...
    _trade_plan.scan_total = static_cast<int64_t>(_market.market_count) * _market.good_count;
    _trade_plan.country_generation = _epoch_country_generation;
...
bool NativeEconomyRuntime::route_trade_source(int32_t source_index, std::string &error) {
    ...
    auto append_candidate = [&](const TradeSignal &destination, int32_t route_cost) {
        int64_t sat = 0;
        const int64_t quantity = std::min(source.quantity, destination.quantity);
        if (quantity <= 0 || route_cost <= 0) return false;
        const int64_t source_stock = _market.stock[
            _market.index(source.cell, source.good)];
        const int64_t destination_stock = _market.stock[
            _market.index(destination.cell, source.good)];
        const int32_t source_price = estimate_trade_price(
            source.cell, source.good, source_stock - quantity, sat);
        const int32_t destination_price = estimate_trade_price(
            destination.cell, source.good, destination_stock + quantity, sat);
        const int64_t spread = static_cast<int64_t>(destination_price) - source_price;
        const int64_t margin_q16 = spread <= 0 ? 0 : mul_div_sat(
            spread, Q16_ONE, std::max<int64_t>(1, source_price), sat);
        const int64_t profit = spread <= 0 ? 0 : mul_div_sat(
        ...
        if (margin_q16 < _trade_min_margin_q16 || profit <= 0 || capacity <= 0) {
        ...
        candidate.country_generation = _epoch_country_generation;
    ...
    if (_trade_plan.phase == TradePlanStore::SCAN) {
        ...
        for (; _trade_plan.scan_cursor < end; ++_trade_plan.scan_cursor) {
            const int32_t market = static_cast<int32_t>(
                _trade_plan.scan_cursor / _market.good_count);
            ...
            int64_t target = mul_div_sat(demand,
            ...
            if (stock > target && static_cast<int32_t>(_trade_plan.sources.size()) <
                    _trade_max_signals) {
                const int64_t cap = mul_div_sat(
                    stock, _trade_max_stock_share_q16, Q16_ONE, sat);
                const int64_t quantity = std::min(stock - target, cap);
                if (quantity > 0) _trade_plan.sources.push_back(
                    {market, good, country, _market.price[index], quantity});
            } else if (target > stock &&
                       static_cast<int32_t>(_trade_plan.destinations.size()) <
                           _trade_max_signals) {
                _trade_plan.destinations.push_back(
                    {market, good, country, _market.price[index], target - stock});
            }
        }
...
bool NativeEconomyRuntime::dispatch_trade_candidates(std::string &error) {
    ...
    for (const TradeCandidate &candidate : _trade_plan.ready_candidates) {
        ...
        const int32_t source_signal = market_signal_index(
            candidate.source, candidate.good);
        const int64_t production_reserve = source_signal >= 0 &&
            source_signal < static_cast<int32_t>(_production_input_reserve.size())
                ? _production_input_reserve[source_signal] : 0;
        ...
        clipped.quantity = std::min(clipped.quantity,
            std::max<int64_t>(0, _market.stock[market_index] -
                production_reserve - reserved_stock));
        int64_t sat = 0;
        ...
        if (clipped.quantity <= 0) {
            if (_market.stock[market_index] <= production_reserve + reserved_stock)
                ++_trade_rejected_stock;
            else ++_trade_rejected_capacity;
            continue;
        }
        ...
        clipped.destination_price = estimate_trade_price(
            candidate.destination, candidate.good,
            _market.stock[destination_index] + clipped.quantity, sat);
        const int64_t spread = static_cast<int64_t>(clipped.destination_price) -
            clipped.source_price;
        const int64_t margin_q16 = spread <= 0 ? 0 : mul_div_sat(
            spread, Q16_ONE, std::max<int64_t>(1, clipped.source_price), sat);
        clipped.expected_profit = spread <= 0 ? 0 : mul_div_sat(
            clipped.quantity, spread, GOODS_SCALE, sat);
        ...
        if (margin_q16 < _trade_min_margin_q16 || clipped.expected_profit <= 0 ||
            clipped.capacity_work <= 0) {
            ++_trade_rejected_profit;
            continue;
        }
    ...
    auto arrival_for = [&](const TradeCandidate &candidate) {
        ...
        const int64_t periods = (raw_days + std::max(1, _epoch_days) - 1) /
            std::max(1, _epoch_days);
        return _sample_day + periods * std::max(1, _epoch_days);
    };
...
bool NativeEconomyRuntime::process_market_cell(int32_t market, MarketResult &result,
    ...
    build_demand_basis(market, market_environment,
    ...
    for (int32_t local = 0; local < cohort_count; ++local) {
        ...
        int64_t remaining = std::max<int64_t>(0, mul_div_sat(
            cohort_food_required[local], _starvation_satisfaction_threshold_q16,
    ...
    for (int32_t local = 0; local < cohort_count; ++local) {
        ...
        const int64_t effective_death_rate_q32 = saturating_add(
            _signatures[signature_id].death_rate_q32, starvation_rate_q32, sat);
    ...
    for (int32_t good = 0; good < _market.good_count; ++good) {
        ...
        if (signal_index >= 0) {
            ...
            const int64_t observed_daily = saturating_add(
                good_sales[good], nonhousehold, sat) / std::max(1, _epoch_days);
...
bool NativeEconomyRuntime::publish_epoch(std::string &error) {
    ...
    _closing_totals.transit_goods = trade_transit_goods();
    _closing_totals.escrow_cash = trade_escrow_cash();
    _closing_totals.goods_stock = saturating_add(
        _closing_totals.goods_stock, _closing_totals.transit_goods, _saturation_count);
    _closing_totals.cohort_funds = saturating_sub(
        _closing_totals.cohort_funds, _structural_funds_to_treasury, _saturation_count);
    _closing_totals.country_cash = _country_runtime == nullptr ? 0 : _country_runtime->total_cash();
    ...
    if (_country_runtime != nullptr) {
        for (int32_t good = 0; good < _market.good_count; ++good)
            _closing_totals.goods_stock = saturating_add(
                _closing_totals.goods_stock, _country_runtime->total_good(good),
                _saturation_count);
    }
...
Dictionary NativeEconomyRuntime::run_slice(const Dictionary &ctx) {
    ...
    std::string error;
    bool planner_before_epoch = false;
    ...
    if (!_epoch_active) {
        ...
        const bool cycle_due = day_index - _last_committed_day >= _epoch_days;
        if (cycle_due && trade_planner_should_run()) {
            planner_before_epoch = _trade_runtime_mode == 2;
...
int32_t NativeEconomyRuntime::stage_progress_q16() const {
    ...
    switch (_stage) {
        ...
        case Stage::TRADE_DISPATCH: return static_cast<int32_t>(Q16_ONE * 7 / 40);
...
int64_t NativeEconomyRuntime::memory_bytes() const {
    ...
    cap(_epoch_business_demand_ema); cap(_epoch_offered_supply_ema);
...
Dictionary NativeEconomyRuntime::report() const {
    ...
    out["trade_country_generation"] = static_cast<int64_t>(
        _trade_topology.component_country_generation);
...
Dictionary NativeEconomyRuntime::market_cell_snapshot(int32_t cell_idx) const {
    ...
    for (int32_t g = 0; g < _market.good_count; ++g) {
        ...
        int64_t inventory_target = mul_div_sat(
            saturating_add(realized, export_ema, snapshot_saturation),
            _good_target_inventory_days_q16[g], Q16_ONE, snapshot_saturation);
        const int64_t feasible_daily = saturating_add(
            _market.demand_ema[_market.index(market, g)],
            signal >= 0 ? _market_signals.business_demand_ema[signal] : 0,
            snapshot_saturation);
        inventory_target = saturating_add(inventory_target,
            mul_div_sat(std::max<int64_t>(0, feasible_daily - realized),
                _epoch_days, 1, snapshot_saturation), snapshot_saturation);
        if (realized == 0 && export_ema == 0 && signal >= 0) {
            inventory_target = std::max(inventory_target, mul_div_sat(
                _market_signals.offered_supply_ema[signal],
                _merchant_market_making_days_q16, Q16_ONE, snapshot_saturation));
        }
        inventory_target = std::max(inventory_target, input_reserve);
...
bool NativeEconomyRuntime::decode_restore_chunk(const std::vector<uint8_t> &bytes,
    ...
    if (schema != SCHEMA_VERSION && schema != 12 && schema != 11 && schema != 10) {
        ...
        for (uint32_t record = 0; record < records; ++record) {
            ...
            for (int32_t line = 0; line < line_count; ++line) {
                ...
                if (!read_le(bytes, cursor, good) ||
                    ...
                    quantity <= 0 || price < _good_min_price[good] ||
                    price > _good_max_price[good]) {
