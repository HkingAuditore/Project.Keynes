...
then snapshots `DCWorld`, native environment state, PKCM, WorldClock, PKCN v11, PKEF v11,
PKEC v41, PKGP, PKFG, journal, PKTR v6, PKID v3, and player view. Restore regenerates static geography and applies
...
科技/Effect recipe/Trigger/内容绑定 identity、Country Modifier domain 与 Country Effect ingress 幂等证据；PKEC v41 引用匹配的 PKCN identity，并持久化经济状态、显赫家族及其
...
Technology practice and development thresholds persist in PKTR v6 and hand permanent-signal
commands to PKEF v11; technology/research state remains PKCN v11 authority.
See [Native Trigger Runtime](./native-trigger-runtime.md).

Family-effect authoring crosses only at the cold catalog boundary. GDScript compiles
`FamilyEffectDefinition` plus Trait-to-Effect CSR; Economy owns families, branches, source bindings and
frozen metrics; Effect owns packed programs, instances, stack groups, plans and ACK; Modifier/Country/
Economy/Gameplay remain the only writers at their safe commits. Exact-good output uses shared-per-good
plus sparse `(cell, good)` overrides, so this extension does not add a per-cell catalog matrix.
