...
def main():
    patch(
        ROOT / "docs/cpp-dots-runtime/authority-migration.md",
        [
            (
                "更新时间：2026-09-11（Country D12 已放行：`implemented` 与生产 request 均为 `0x806`；C2 field-init 已修；Effect F7 Host SHADOW stage 已接入）",
                "更新时间：2026-09-11（E8 Modifier 已放行：`implemented` 与生产 request 均为 `0x846`；Effect 仍为 F7 SHADOW）",
            ),
            (
                ...
                "CLIMATE|COUNTRY|MODIFIER|COMMIT = 0x846`；Climate 滞后一日回灌，Country 经 Host read-view 回写，"
                "Modifier 经 snapshot 回灌 `ModifierRuntime`）。\n"
                "`runtime_climate_authority_enabled` 为真时生产请求 `authoritative_domain_mask=0x846`。关掉该开",
            ),
            (
                "Modifier E2-E7、Ideology G2-G7、Trigger H2-H6、Effect F2-F7 的 SHADOW 现状不变（E8/F8/G8/",
                "Ideology G2-G7、Trigger H2-H6、Effect F2-F7 的 SHADOW 现状不变（F8/G8/",
            ),
            (
                "| `implemented_domain_mask()` | **编译期 constexpr**（`native_simulation_host.h`） | 该域**有真实 POD handler**，不代表它是权威 | `CLIMATE\\|COUNTRY\\|COMMIT = 0x806` |\n"
                "| `authoritative_domain_mask` | 启动配置键，由 GDScript 传入（`world_runtime_host.gd`） | 本次会话**实际要 worker 承担权威**的域 | ACTIVE 时 `0x806` |",
                "| `implemented_domain_mask()` | **编译期 constexpr**（`native_simulation_host.h`） | 该域**有真实 POD handler**，不代表它是权威 | `CLIMATE\\|COUNTRY\\|MODIFIER\\|COMMIT = 0x846` |\n"
                "| `authoritative_domain_mask` | 启动配置键，由 GDScript 传入（`world_runtime_host.gd`） | 本次会话**实际要 worker 承担权威**的域 | ACTIVE 时 `0x846` |",
            ),
            ("`0x806 ≠ 0xFFF`", "`0x846 ≠ 0xFFF`"),
            (
    ...
    patch(
        ROOT / "docs/cpp-dots-runtime/native-simulation-host.md",
        [
            (
                "当前 `implemented_domain_mask()` 为 `CLIMATE | COUNTRY | COMMIT = 0x806`。Modifier、Economy 等仍不在 ACTIVE authority mask。因此：",
                "当前 `implemented_domain_mask()` 为 `CLIMATE | COUNTRY | MODIFIER | COMMIT = 0x846`。"
                "Effect、Economy 等仍不在 ACTIVE authority mask。因此：",
            ),
            (
            ...
            (
                ...
                "\n"
                "Modifier stage 位于 EFFECT -> MODIFIER -> ... -> COMMIT，SHADOW 与 ACTIVE 共用 "
                "`execute_modifier_worker_stage`。生产 ACTIVE（mask `0x846`）在 Country 之后、Climate park "
                "日跳过对齐 Country：可选 Effect POD 上游 → Modifier plan/ACK/snapshot/commit；成功日写入 "
                "`completed_domain_mask` 的 MODIFIER 位。失败隔离，不撤销已提交 Climate/Country。\n"
                "\n"
                "grant 后：Host 为命令唯一写者；主线程抑制 `run_modifier_daily` / `modifier_daily_system`；"
                "`world_runtime_host` 非阻塞消费 snapshot 并 `ModifierRuntime.apply_pod_snapshot`；"
                "`main_wait_on_sim_us` 保持 0。Effect 仍为 F7 SHADOW，不抑制 `EffectRuntime`。",
            ),
            (
    ...
    patch(
        ROOT / "docs/cpp-dots-runtime/runtime-authority-matrix.md",
        [
            (
            ...
            (
                ...
                "Modifier E2-E8 已完成：生产 ACTIVE 唯一写者、snapshot 回灌 `ModifierRuntime`、抑制 `modifier_daily`。"
                "当前 Host mask 为 `CLIMATE|COUNTRY|MODIFIER|COMMIT = 0x846`。Effect 仍为 F7 SHADOW。",
            ),
        ],
    )
