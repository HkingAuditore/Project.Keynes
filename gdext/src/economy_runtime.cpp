...
constexpr int64_t price_adjustment_reference(
    ...
    return period_change_q16 < 0
        ? std::max<int64_t>(PRICE_NUMERIC_GUARD_MIN, current_price)
...
bool NativeEconomyRuntime::prepare_building_economic_plan(
    ...
    for (int32_t active = active_begin; active < active_end; ++active) {
        ...
        if (type.kind != 2 && group.operating_state == 1 && group_index <
...
bool NativeEconomyRuntime::run_building_employment_cell(
    ...
    auto planned_owner_demand = [&](const BuildingGroup &group, const BuildingType &type) {
        ...
        if (group.operating_state == 1) {
            // Suspension preserves the installed building, not its labor claim.
            // Releasing every owner lets scarce population move upstream; an
            // approved recovery probe hires again through the normal path.
            return int64_t{0};
        }
...
bool NativeEconomyRuntime::run_endogenous_building_investment(
    ...
    if (initialize) {
        ...
        _investment_merchant_cash_by_cell.assign(_cell_count, 0);
        _investment_outstanding_credit_by_cell.assign(_cell_count, 0);
        for (const int32_t merchant_slot : _merchant_slots) {
            const int32_t merchant_cell =
                _population.page_cell[merchant_slot / PAGE_SIZE];
            if (merchant_cell < 0 || merchant_cell >= _cell_count) continue;
            _investment_merchant_cash_by_cell[merchant_cell] = saturating_add(
                _investment_merchant_cash_by_cell[merchant_cell],
    ...
    for (int32_t ordinal = ordinal_begin; ordinal < ordinal_end; ++ordinal) {
        ...
        const bool capture_investment_diagnostics = capital_review &&
        ...
        for (int32_t type_id = 0; type_id < static_cast<int32_t>(
            ...
            const BuildingType &type = _building_types[type_id];
            // Every technology-unlocked type participates. Upgrade tiers and
            // service types are evaluated by the same market/capital rules;
            // they are no longer silently removed from the candidate set.
            if (!building_available(cell, type_id, true)) continue;
            ...
            for (int32_t i = 0; i < type.output_count; ++i) {
                ...
                const int64_t pressure_strength = _investment_min_shortage_q16 > 0
                    ? mul_div_sat(item.pressure_q16, Q16_ONE,
                        _investment_min_shortage_q16, _saturation_count)
                    : (item.pressure_q16 > 0 ? 4 * Q16_ONE : 0);
                const int64_t utilization_strength =
                    _investment_min_utilization_q16 > 0
                    ? mul_div_sat(item.utilization_q16, Q16_ONE,
                        _investment_min_utilization_q16, _saturation_count)
                    : (item.utilization_q16 > 0 ? 4 * Q16_ONE : 0);
                item.driver_strength_q16 = std::max(
                    pressure_strength, utilization_strength);
            ...
            if (driver.good_id < 0 ||
                (shortage_q16 < _investment_min_shortage_q16 &&
                 utilization_q16 < _investment_min_utilization_q16)) {
                reject(INVESTMENT_REJECTION_MARKET_SIGNAL);
                continue;
            }
            // New buildings have no producer history and therefore do not face
            // a sell-through gate. Existing output is judged only on the same
            // marginal good that supplied the market-pressure signal.
            if (driver.sellable > 0) {
                if (driver.sell_through_q16 < 4 * Q16_ONE / 5) {
                    reject(INVESTMENT_REJECTION_SELL_THROUGH);
                    continue;
                }
                if (driver.discard_q16 > Q16_ONE / 10) {
                    reject(INVESTMENT_REJECTION_DISCARD);
                    continue;
                }
            }
            if (!evaluate_building_conditions(type_id, cell)) {
            ...
            for (int32_t i = 0; i < type.output_count; ++i) {
                ...
                } else if (issue_value <= 0 && prospective_quantity > 0) {
                    // A first entrant has no sales history. Limit its revenue to
                    // the persistent shortage/inventory gap it can actually fill.
                    absorption_q16 = std::clamp<int64_t>(mul_div_sat(
                        output_signal.deficit, Q16_ONE, prospective_quantity,
                        _saturation_count), 0, Q16_ONE);
                }
                const int64_t absorbed_quantity = mul_div_sat(
                    prospective_quantity, absorption_q16, Q16_ONE,
                    _saturation_count);
            ...
            if (type_best.type >= 0) insert_portfolio(type_best);
        }
        if (portfolio_size <= 0) {
        ...
        auto previously_allocated_owner_population = [&](int32_t sponsor) {
            ...
            for (int32_t j = 0; j < portfolio_size; ++j) {
                if (portfolio[j].sponsor != sponsor) continue;
        ...
        for (int32_t i = 0; i < portfolio_size; ++i) {
            ...
            if (profession_transition && !move_cohort_population(
                    ...
                    owner_population, error, &source_drained)) return false;
            if (source_drained) {
        ...
        if (portfolio_population_changed &&
            !rebuild_merchant_ranges(error)) return false;
        if (portfolio_population_changed &&
            cell < static_cast<int32_t>(
                _investment_merchant_cash_by_cell.size())) {
            int64_t refreshed_merchant_cash = 0;
            for (int32_t k = _merchant_offsets[cell];
                 k < _merchant_offsets[cell + 1]; ++k) {
                refreshed_merchant_cash = saturating_add(
                    refreshed_merchant_cash, std::max<int64_t>(
                        0, _population.funds[_merchant_slots[k]]),
                    _saturation_count);
            }
            _investment_merchant_cash_by_cell[cell] =
                refreshed_merchant_cash;
        }
            ...
            if (_building_commit_phase == 0) {
                ...
                for (int32_t g = 0; g < static_cast<int32_t>(_buildings.size()); ++g) {
                    ...
                    if (!liquidation_eligible) {
                        // Supply, natural-resource or financing blockage is
                        // retryable. Require consecutive executable-but-bad
                        // reviews before removing installed capacity.
                        group.recovery_failed_reviews = 0;
                        continue;
                    }
                    ...
                    unit_period_output = mul_div_sat(
                        unit_period_output,
                        std::max<int64_t>(1, liquidation_capacity_q16),
                        Q16_ONE, _saturation_count);
                    const int64_t supported_count = unit_period_output > 0
                        ? std::min<int64_t>(group.count,
                            saturating_add(std::max<int64_t>(
                                0, group.last_sold),
                                unit_period_output - 1,
                                _saturation_count) /
                                unit_period_output)
                        : 0;
                    ...
                    const int64_t max_retire = std::max<int64_t>(1,
                        saturating_add(saturating_mul(
                            ...
                            _saturation_count), Q16_ONE - 1,
                            _saturation_count) / Q16_ONE);
                    const int64_t retire_count = std::min(
                        confirmed_excess, max_retire);
                    ...
                    _recovery_liquidated_buildings = saturating_add(
                        _recovery_liquidated_buildings, retire_count,
                        _saturation_count);
