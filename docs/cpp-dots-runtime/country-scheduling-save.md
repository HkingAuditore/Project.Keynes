...
当前 writer 写出 PKCN v4 与 PKEC v29。PKCN v4 在 v3 研究状态之上增加五类默认税率、
按职业/物资/建筑的稀疏覆盖和税务政策版本；PKEC v29 保留 generation-safe 的逐 cell
上批补贴申请、财政累计和确定性 hash，并持久化家族特性、地块威望分支及特性命令。
PKEC reader 仅接受 v29，v28 及更早版本明确拒绝；catalog mismatch 不放宽。恢复仍必须先 PKCN 后 PKEC。
