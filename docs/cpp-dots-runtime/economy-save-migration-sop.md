...
营业税实缴、营业补贴、维护应付、实际 workforce-capacity-days（Q16）、冻结市场报价回执和
冻结经营成本报价。它们都是 period-total 诊断并进入 `state_hash`；v41-v46 恢复时按零初始化。
...
财政 reserve/fiscal escrow 后冻结，因此国库现金被财政托管占满时不会继续产生无资金科研收入预期。

PKEC v46 在交易订单每条物资行末尾追加 `transaction_transfer:i64`，保存最终吸收点的
交易税转移。v45 及更早订单没有该列，读取时按零交易税迁移；其余订单与 cell record
布局保持不变。

## PKEC v45（历史 writer；v44/v43/v42/v41 向前兼容读）
...
reader 接受 v47、v46、v45、v44、v43、v42 与 v41；v43 恢复时 startup demand 固定为 `OFF`。

## PKEC v42（历史 writer；当前仍可读）

PKEC v42 保留 v41 的 FamilyStore、降水 lane 与既有 section，
...
当前写出 schema 为 PKEC v47，并与 PKCN v12、PKEF v11 交叉绑定。PKEC v46/v45/v44/v43/v42/v41 仅向前兼容读；v40 及更早版本全部拒绝；
...
当前 PKEC v47 与 PKCN v12 要求 save 的稳定 ID 表（含 technology IDs）与当前 catalog 完全一致，
