#!/usr/bin/env python3
"""Patch wave-4 electrical and pre-print knowledge reveal conditions."""
...
WAVE4_START = "tech.electrification"
WAVE4_END = "tech.steam_sawmilling"


def load_wave4_evidence() -> dict[str, list[str]]:
    text = AUTHORING.read_text(encoding="utf-8")
    start = text.find('"%s":' % WAVE4_START)
    end = text.find('"%s":' % WAVE4_END)
    if start < 0 or end < 0:
        raise SystemExit("wave4 markers missing in authoring script")
    end = text.find("\n", end)
    block = text[start:end]
    evidence: dict[str, list[str]] = {}
    for match in re.finditer(r'"([^"]+)": \[([^\]]*)\]', block):
        evidence[match.group(1)] = re.findall(r'"([^"]+)"', match.group(2))
    if WAVE4_START not in evidence or WAVE4_END not in evidence:
        raise SystemExit("failed to parse wave4 evidence")
    return evidence
