...
func tick(ctx) -> Dictionary:
	...
	return {
		...
		"factor_lookup_ms": float(res.get("factor_lookup_ms", 0.0)),
		"slot_refresh_ms": float(res.get("slot_refresh_ms", 0.0)),
		"knob_update_ms": float(res.get("knob_update_ms", 0.0)),
		"native_call_ms": float(res.get("native_call_ms", 0.0)),
		"fallback_ms": float(res.get("fallback_ms", 0.0)),
