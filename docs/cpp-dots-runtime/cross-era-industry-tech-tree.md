...
- 野生动物保留为早期生态资源：`wild_game -> game_meat/raw_hide/fur`，由 `stone_age_hunting_camp` 产出，农业时代后重要性下降。当前石器营地为 1 个猎人自营岗位，标称日产 `310/36` GOODS_SCALE 野味/生皮，并抽取 `163` 野生动物；食品产量按主食采集营地 `948` 与蛋白/主食需求比（约 `153/468`）对齐，野生动物抽取则对齐采集营地资源强度 `500/948`（`310 × 500 / 948`）。副产品生皮仍受本地衣着链约束。资源扣减使用独立资源单位，不能直接解释为商品重量转换率。
- 同档蛋白建筑一并按该比例校准：`freshwater_fishing_camp` 日产 `310` 鱼并抽取 `52`；`marine_fish_collector`（2 岗）日产 `620`、抽取 `104`（与淡水同强度 `52/岗`）；`small_game_trapline` 相对狩猎营保留约 `17%` 产量升级与更低抽取强度（`364/20/7`，抽取 `96`）。
- 雇佣关系随时代复杂化：自营/家庭劳动 -> 学徒和行会 -> 地主/农奴/契约劳工 -> 工业工人/技师/工程师/经理。

### 食物劳动生产率基线

经济标定把 `survival_household` 的主食、蛋白质和蔬果需求合计为 `824` 个完整生存食物单位。
下表表示一名实际投入生产的农业劳动者在满资源、满投入和正常利用率下可覆盖的完整生存篮子数，
不是 NativeEconomyRuntime 的硬上限；国家/建筑 Modifier、地域因子和后续生产法可以继续超过基线。
早期阶段保持低效率，机械动力、化肥、精准农业和自动化阶段出现大幅跃升，以避免古老农田与先进
生产方式拉不开差距。

| 时代 | 石器 | 农耕 | 王国 | 帝国 | 探索 | 启蒙 | 蒸汽 | 电气 | 原子 | 信息 | 智能 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 完整生存篮子/劳动者 | 1.15 | 1.35 | 1.60 | 2.00 | 2.50 | 3.20 | 6.00 | 18.00 | 40.00 | 90.00 | 180.00 |

四条主粮线共享这条时代基线，但地形和劳动组织会造成同一时代的合理差异：雨养小麦、旱稻、
玉米园圃和高地块茎田属于农耕早期；租佃、水田、庄园、机械化农场和精准农场分别在后续时代
提高每名劳动者的覆盖人口。缺少某一主粮专门方法时，可以使用 `subsistence_farm`、综合农场、
加工食品或贸易补足，不把“没有该作物专门建筑”误当成产能封顶。
