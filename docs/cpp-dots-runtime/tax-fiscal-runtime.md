...
policy ID 不进入存档或确定性 hash。PKEC v46 保存交易订单的交易税列，并在恢复后从 PKCN
重新编译缓存；v45 订单按零交易税迁移。
...
- 营业税正税按建筑本批实际取得的生产者回款计税，多输出建筑汇总实际回款；负税只按未税投入、
  实际支付工资和维护费补贴，不含业主生活成本、奖金、债务和税费。正税源头扣除并可从业主
...
- PKEC v46 保存逐 cell 上批补贴权重、generation-safe 国家 handle、财政累计值、确定性 hash
  和交易订单的交易税转移列。
