...
累计时代审计从采集营地、商栈、伐木场、采石场及一次性桥接库存（原木 1000、采集物 250、燧石 500）出发，同时检查建造材料、日常投入、科技和自然资源条件。每个 rank 都反复扩张到固定点；不可达建筑会报告缺失入口和强连通闭环。测试 bootstrap 只在开局投放一次桥接库存，之后所有扩建仍支付目录中的非零成本；每个有人口市场保留商栈和可运行食物根。常规初始采集规模继续要求十年资源储量；为避免有真实矿藏却不足十年满负荷的地图被误判为空经济，建设闭包所需的最小伐木场和采石场允许按一年本地资源储量各保留一座。它们仍使用并扣减真实本地资源。随机地图上的连通候选区域只有同时具备上述资源安全来源时才会生成初始人口、建筑和桥接库存；缺少入口的孤立区域保持无人状态，并写入 `construction_source_skipped_components` 诊断。整张地图不存在任何可闭合候选区域时，bootstrap 才以 `bootstrap_construction_source_missing` 明确失败，并报告三种根资源的正储量地块数、峰值和根建筑数量。
