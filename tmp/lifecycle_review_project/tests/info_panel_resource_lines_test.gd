...
func _initialize() -> void:
	...
	map.res_timber_reserve_arr.resize(1)
	map.res_iron_ore_reserve_arr.resize(1)
	map.res_timber_extra_change_arr.resize(1)
	map.res_timber_reserve_arr[0] = 0.42
	map.res_iron_ore_reserve_arr[0] = 0.20
	map.res_timber_extra_change_arr[0] = -0.05
	...
	if label == null:
		...
		# timber: land_only renewable, expect 储量数值可见
		if not txt.contains("0.420"):
			failures.append("missing timber reserve 0.420 (got: %s)" % txt)
		if not txt.contains("额外变化 -0.050"):
			failures.append("missing timber extra_change -0.050 (got: %s)" % txt)
	...
	# ── 水域 cell：land_only 资源应显示"水域不可用"，而非储量数值
	...
	if not wtxt.contains("水域不可用"):
		# 仅当存在 land_only 资源时才要求；timber 即 land_only
