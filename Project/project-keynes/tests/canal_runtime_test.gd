...
func _run() -> void:
	...
	if int(saved.get("schema", 0)) != 34 or not bool(restored.get("ok", false)) \
	...
	_expect("PKEC v34 restores an in-flight canal project exactly",
		int(saved.get("schema", 0)) == 34 and bool(restored.get("ok", false))
