...
#
# GM 面板的经济录制控制面。经济快照抓取、CSV v23 编码和写盘全部由
...
var probe_cells := PackedInt32Array()
var _main = null
...
func start() -> void:
	...
	if not world.has_method("start_economy_csv_recording"):
		push_error("[economy-record] native CSV v23 API unavailable; rebuild dots_ext")
		return
	...
	if cell_count <= 0:
		...
		return
	var cell_indices := probe_cells.duplicate()
