...
| Population, market, buildings, taxable events and fiscal escrow | PKEC / `NativeEconomyRuntime` | PKSV `pkec` / PKEC v23 | Restore only after PKCN and trade topology |
...
| Economy/building modifiers | Economy `ModifierStore` + `BuildingIdentityStore` | embedded in PKEC v23 | Factors feed deterministic output/construction/trade helpers, never ledgers directly |
...
| `economy_daily` | `NativeEconomyRuntime` 持有滚动五相结算、stage/cursor、PopulationStore、MarketStore、BUILDING_GRAPH、国内 Trade stores、税率冻结、应税事件、补贴权重与 fiscal escrow；国家税表与国库由 `NativeCountryRuntime` 提供。 | 独立 native vectors；due-cell sample 冻结环境、`cell → country`、科技、税率与 Modifier factor；worker 只读 dense 税率并写独占 cell 财政 lane，仅资源净 delta 写回 extra-change slots。 | 有界 continuation 隔离半成品；财政提交归还未用额度、收取正税并要求 escrow 清零；发布 summaries、审计与 PKEC v23 stream。 | 无大规模 GDScript fallback；国家桥无效时 disabled。 | 本地市场、税收财政、商人信用和国内贸易默认 **ACTIVE / 每 cell 固定 5 日**。 | 进口/出口关税仅有政策、Modifier、存档和 UI 占位；国内贸易关税事件恒为零，跨国贸易/外交仍未接入。 |
