...
func _run() -> void:
	...
	if int(saved.get("schema", 0)) != 35 or not bool(restored.get("ok", false)) \
	...
	_expect("PKEC v35 restores an in-flight canal project exactly",
		int(saved.get("schema", 0)) == 35 and bool(restored.get("ok", false))
