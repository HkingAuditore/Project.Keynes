...
- **① 统一距水场（post_base，纯内部中间场）**：`run_native_world_generate_post_base_pass` 在 lake/river 最终判定后、水缘生态 pass 之前，做海/湖/大河**多源 BFS**：`water_dist[i]`=到最近水体格距（`water_dist_max` 截断），`water_src_kind[i]`=最近水源类型（0=海洋性 / 1=湖 / 2=大河）。海洋 source = `pk_is_water_terrain` 的 ocean-connected 水格；湖 = `LAKE(18)`；大河 = `RIV && RFLOW≥water_big_river_flow_min`。`std::vector` 局部量，仅供下面 ②③ 消费，不出境。
- **② 气候回灌（仅湖/河项；海洋项不重做）**：用距水场只补 base 阶段不存在的湖/河近邻湿润带——湖滨增湿地板 `M=max(M, lake_moist_floor·exp(-wd/lake_moist_scale))`（kind=1）、河谷 riparian 增湿 `M+=river_riparian_gain·exp(-wd/river_riparian_scale)`（kind=2，封顶 1）。海洋调温/沿海湿度地板/`land_continentality` 由 base pass 维持，**这里不重做避免双重计算**。全部 **floor/additive 单调 + 指数衰减 + 半径截断**，**绝不全局抬湿、绝不二次 biome redecide**（援引 post_base 既有警告：二次重判抹湿是"陆地普遍过湿、沙漠消失"的元凶；故只改 `M[]`，不重判）。湖泊**温度**调节因 `TEMP[]` 只读 + bind 后 republish 会重算而**本期延后**（需专门 temp-adjust 通道）。
...
**海岸线视觉平滑（coast-smooth，2026-06-26，纯 shader、不动烘焙/分类）**：起因——河流边缘由连续 SDF（`flow_tex`）驱动、`smoothstep` 出边 → 平滑有机；而水陆边界是 **per-cell 六边形枚举硬边**（`is_water` 取自 `enum_lut`，烘焙期 Bayer dither 已刻意跳过水陆边界以护海岸硬边）→ 海岸呈六边形阶梯/锯齿，远丑于河流。修复在 shader 端用**线性滤波 `height_tex` 的亚像素连续高程**在 `sea_level` 附近做"等高线 AA"：① `compute_shore_halo`（`shore_common`）的 halo 边缘锐度从块状邻居计数 `shore_w` 改由平滑高度带 `sea_band` 主导（`mix(0.34,0.62,max(sea_band,shore_w*0.5))`）；② 陆侧（`land_pipeline::apply_land_color_overlays`）近岸陆地像素若平滑高程低于水线→按 `1-smoothstep(sea-band, sea+band*0.3, elev)` 渐隐为 `color_coast_water`；③ 水侧（`water_pipeline::apply_water_overlay_surface`）近岸水像素若平滑高程高于水线→按 `smoothstep` 渐隐为沙滩色，与陆侧沿同一 height 等高线收敛 → 海岸线跟着亚像素等高线走、消除六边形硬边。②③ 均由 `shore_w`（近岸 gate）限制只在海岸生效，开阔深水/内陆洼地不受影响。可调 uniform：`coast_smooth_strength`（默认 0.65，0=关回退旧硬边）、`coast_smooth_band`（默认 0.055 归一化高程，AA 带半宽）——运行期实时可调、无需 rebuild/重烘（纯 shader uniform）。依赖 `height_tex` 已带海岸坡度（由 #1/#2a/#4 carve 提供的亚像素高度过渡），故与海岸 SDF carve 协同。Mobile LOW/MID 路径无 halo，coast-smooth 亦随之旁路。
