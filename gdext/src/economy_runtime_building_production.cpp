...
bool NativeEconomyRuntime::building_constructible(int32_t cell, int32_t type_id,
                                                   bool frozen) const {
    // 技术进步提供更多生产方式，而不是脚本化淘汰旧方式。投资市场会按
    // 资本回报、单位成本和当地约束选择层级；这里仅判断知识与依赖闭包。
    return building_available(cell, type_id, frozen);
}
...
bool NativeEconomyRuntime::run_building_production_cell(
    ...
    if (trace_detail) {
        population_store().for_each_in_cell(cell, [&](int32_t slot) {
            ...
            trace_cell_funds.push_back(population_store().funds[slot]);
    ...
    if (retention_owner_by_slot.size() < population_store().active.size()) {
        retention_owner_by_slot.resize(population_store().active.size(), -1);
        retention_slot_stamp.resize(population_store().active.size(), 0);
    }
    ...
    for (size_t owner = 0; owner < payroll_owners.size(); ++owner) {
        ...
        const uint32_t signature_id = population_store().signature_id[owner_slot];
        ...
        const int64_t population = std::max<int64_t>(0, population_store().population[owner_slot]);
    ...
    for (int32_t owner_signature : payroll_owners) {
        ...
        for (int32_t g = begin; g < end; ++g) {
            ...
            for (int32_t i = 0; i < type.output_count; ++i) {
                ...
                const int64_t shortage = std::clamp<int64_t>(
                    market_store().last_shortage_q16[idx], 0, Q16_ONE);
                ...
                const int64_t demand = saturating_add(
                    ...
                    _saturation_count);
                if (demand > 0) {
                    const int64_t stock = market_store().stock[idx];
        ...
        const int64_t owner_cash = std::max<int64_t>(0, population_store().funds[owner_slot]);
    ...
    auto process_phase = [&](bool cycle_flow_phase) -> bool {
        ...
        for (int32_t g = begin; g < end; ++g) {
            ...
            if (_merchant_credit_runtime_mode == 2 && credit_cap > 0) {
                for (int32_t k = _merchant_offsets[cell];
                    ...
                    merchant_credit_cash = saturating_add(
                        ...
                        std::max<int64_t>(0, population_store().funds[_merchant_slots[k]]),
                        _saturation_count);
                }
            }
            ...
            if (observe_price_ceiling && type.input_count > 0) {
                const int64_t affordable_scale = clamp_scale_to_group_budget(
                    std::min(intent_scale_q16, ceiling_resource_scale), false);
                const int64_t intent_cost = quote_group_inputs(group, type, affordable_scale, false,
                    &ceiling_input_candidates, &ceiling_input_quantities);
                if (intent_cost <= settlement_budget) {
                    for (int32_t i = 0; i < type.input_count; ++i) {
                        const int32_t selected = ceiling_input_candidates[i];
                        ...
                        const int32_t available = select_input_candidate(item, true, effective);
                        int64_t available_scale = 0;
                        if (available >= 0) {
                            const auto &candidate = _building_input_candidates[available];
                            const int64_t physical = effective_production_input_quantity(cell,
                                candidate.good_id, physical_input_quantity(effective, candidate),
                                _saturation_count);
                            available_scale = physical > 0 ? std::min<int64_t>(Q16_ONE,
                                mul_div_sat(market_store().stock[market_store().index(market, candidate.good_id)],
                                    Q16_ONE, physical, _saturation_count)) : Q16_ONE;
                        }
                        const int64_t unfilled = mul_div_sat(requested,
                            std::max<int64_t>(0, requested_scale - available_scale),
                            std::max<int64_t>(1, requested_scale), _saturation_count);
                        const int32_t signal = market_signal_index(cell, _building_input_candidates[selected].good_id);
                        if (signal >= 0 && signal < static_cast<int32_t>(_epoch_ceiling_business_requested.size())) {
                            _epoch_ceiling_business_requested[signal] = saturating_add(
                                _epoch_ceiling_business_requested[signal], requested, _saturation_count);
                            _epoch_ceiling_business_unfilled[signal] = saturating_add(
                                _epoch_ceiling_business_unfilled[signal], unfilled, _saturation_count);
                        }
                    }
                }
            }
            ...
            // Reuse the final preflight bills verbatim, so splitting a recipe
            // into several input edges cannot change the rounded fee or tax.
            for (size_t policy = 0; policy < input_policy_count; ++policy) {
                const InputPolicyBill &bill = input_policy_bills[policy];
                const TransactionQuote quote = quote_transaction(cell, bill.good,
        ...
        for (int32_t good : touched_goods) {
            ...
            if (_good_storage_modes[good] == 1) {
                ...
                quota_by_good[good] = merchant_procurement_quota(
                    ...
                    market_store().stock[market_store().index(market, good)], realized,
                    exports, _saturation_count);
            }
            buy_factor_by_good[good] = effective_merchant_buy_factor_q16(
                market, good, target,
                market_store().stock[market_store().index(market, good)], _saturation_count);
            ...
            const int64_t base_weight = goods_cost(quota_by_good[good], outlay_price, _saturation_count);
            ...
            const bool survival_good = _survival_food_good_mask[good] != 0 ||
                _survival_clothing_good_mask[good] != 0;
            const int64_t protected_reserve = merchant_protected_reserve(signal);
            const bool input_gap = signal >= 0 &&
                protected_reserve >
            ...
            const int64_t market_index = market_store().index(market, good);
            priority_q16 = saturating_add(priority_q16, mul_div_sat(
                std::clamp<int64_t>(market_store().last_shortage_q16[market_index],
                                    0, Q16_ONE),
            ...
            if (signal >= 0) {
                ...
                const int64_t stock = std::max<int64_t>(
                    0, market_store().stock[market_index]);
                if (reserve > stock) {
                    priority_q16 = saturating_add(priority_q16, mul_div_sat(
        ...
        auto procurement_tier = [&](int32_t good) {
            ...
            if (signal >= 0 &&
                merchant_protected_reserve(signal) >
                    market_store().stock[market_store().index(market, good)]) return 1;
            return 2;
        };
        ...
        for (const Offer &offer : offers) {
            if (procurement_bill_good != offer.good) {
                procurement_bill = {};
            ...
            if (issue_value > 0) {
                ...
                const int64_t retail_value = mul_div_sat(
                    sold, market_store().price[market_store().index(market, offer.good)],
            ...
            if (business_tax > 0)
                trace_record_cashflow(cell,
                    population_store().handle_for_slot(offer.owner_slot),
    ...
    for (int32_t owner_signature : payroll_owners) {
        ...
        const int64_t available = owner_slot >= 0
            ? std::min(total_due, std::max<int64_t>(
                0, population_store().funds[owner_slot])) : 0;
        int64_t prefix = 0;
        int64_t allocated = 0;
    ...
    thread_local std::vector<int64_t> maintenance_observed;
    maintenance_observed.assign(cell_signal_count, 0);
    thread_local std::vector<std::pair<int32_t, int64_t>> ceiling_maintenance_wallets;
    ...
    for (int32_t g = begin; g < end; ++g) {
        ...
        for (int32_t i = 0; i < type.maintenance_count; ++i) {
            ...
            const int64_t desired = saturating_mul(
                building_days, item.quantity, _saturation_count);
            ...
            if (signal >= 0 && signal < static_cast<int32_t>(
                ...
                _epoch_desired_business_demand[signal] = saturating_add(
                    _epoch_desired_business_demand[signal], desired,
                    _saturation_count);
            }
            const int64_t price = maintenance_settlement_price(
                cell, item.good_id, _saturation_count);
            const int64_t stock_index = market_store().index(market, item.good_id);
            const int64_t stock = std::max<int64_t>(0, market_store().stock[stock_index]);
            const int64_t affordable = price > 0
                ? mul_div_sat(std::max<int64_t>(0, population_store().funds[owner_slot]),
                              GOODS_SCALE, price, _saturation_count)
                : 0;
            const int64_t bought = std::min(desired, std::min(stock, affordable));
            if (observe_price_ceiling && signal >= 0 &&
                signal < static_cast<int32_t>(_epoch_ceiling_business_requested.size())) {
                auto wallet = std::find_if(ceiling_maintenance_wallets.begin(),
                    ceiling_maintenance_wallets.end(), [owner_slot](const auto &entry) {
                        return entry.first == owner_slot;
                    });
                if (wallet == ceiling_maintenance_wallets.end()) {
                    ceiling_maintenance_wallets.push_back({owner_slot,
                        std::max<int64_t>(0, population_store().funds[owner_slot])});
                    wallet = ceiling_maintenance_wallets.end() - 1;
                }
                wallet->second = std::min(wallet->second,
                    std::max<int64_t>(0, population_store().funds[owner_slot]));
                const int64_t requested = price > 0 ? std::min(desired,
                    mul_div_sat(wallet->second, GOODS_SCALE, price, _saturation_count)) : 0;
                wallet->second -= goods_cost(requested, price, _saturation_count);
                _epoch_ceiling_business_requested[signal] = saturating_add(
                    _epoch_ceiling_business_requested[signal], requested, _saturation_count);
                _epoch_ceiling_business_unfilled[signal] = saturating_add(
                    _epoch_ceiling_business_unfilled[signal], std::max<int64_t>(0, requested - bought), _saturation_count);
            }
            const int64_t unmet = std::max<int64_t>(0, desired - bought);
            _maintenance_unmet = saturating_add(
                _maintenance_unmet, unmet, _saturation_count);
            if (unmet > 0 && price > 0) {
                _maintenance_unpaid_value = saturating_add(
                    _maintenance_unpaid_value,
                    mul_div_sat(unmet, price, GOODS_SCALE, _saturation_count),
                    _saturation_count);
            }
            if (bought <= 0 || price <= 0) continue;
            ...
            if (signal >= 0 && signal < static_cast<int32_t>(
                    _epoch_nonhousehold_withdrawals.size())) {
                _epoch_nonhousehold_withdrawals[signal] = saturating_add(
                    _epoch_nonhousehold_withdrawals[signal], bought,
                    _saturation_count);
            }
            if (signal >= 0 && signal < static_cast<int32_t>(
                    _epoch_funded_business_demand.size())) {
                _epoch_funded_business_demand[signal] = saturating_add(
                    _epoch_funded_business_demand[signal], bought,
                    _saturation_count);
            }
            if (signal >= cell_signal_begin && signal < cell_signal_end) {
                const size_t local = static_cast<size_t>(signal - cell_signal_begin);
                maintenance_observed[local] = saturating_add(
                    maintenance_observed[local], bought, _saturation_count);
            }
            trace_record_cashflow(cell, population_store().handle_for_slot(owner_slot),
                                  CASHFLOW_PRODUCTION_INPUT, 0, cost);
            if (credit_local_merchants(cell, cost,
                                       CASHFLOW_MERCHANT_BUSINESS,
                                       &_saturation_count) != cost) {
                error = "building_maintenance_has_no_merchant_owner";
                return false;
            }
        }
        group.last_maintenance_cost = paid;
        group.last_operating_cost = saturating_add(
    ...
    for (size_t i = 0; i < cell_signal_count; ++i) {
    ...
    if (trace_detail) {
        ...
        for (size_t i = 0; i < trace_cell_slots.size(); ++i) {
            ...
            add(FIELD_COHORT_FUNDS, SUBJECT_COHORT, handle, -1,
                trace_cell_funds[i], population_store().funds[slot]);
            add(FIELD_COHORT_EPOCH_INCOME, SUBJECT_COHORT, handle, -1,
                trace_cell_income[i], population_store().epoch_income[slot]);
...
void NativeEconomyRuntime::merge_building_production_result(ProductionResult &result) {
    ...
    merge(_maintenance_goods_consumed, result.maintenance_goods_consumed);
    merge(_maintenance_unmet, result.maintenance_unmet);
    merge(_maintenance_unpaid_value, result.maintenance_unpaid_value);
