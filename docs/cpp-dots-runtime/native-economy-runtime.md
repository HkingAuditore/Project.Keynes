...
- PKEC v25 是当前 writer：在 v24 基础上追加 `building_plan_days` 语义 knob（schema>=25 读取并做等值校验；旧 schema 存档按当前 profile 值直接放行）。保存 BuildingIdentityStore、Economy Modifier domain、逐 cell 六列冻结环境、
