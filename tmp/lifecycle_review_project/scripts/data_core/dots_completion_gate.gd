...
const SEGMENTS: Array = [
	# ─── dots-flag-prune-pr1（2026-05-22）：所有 segment 已退役 ────────────────
	# 原 SEGMENTS 包含 stage_b_combined / season_refresh × 2 / physical_circulation /
	# pass_b_simd / ocean_water_simd / ocean_land_simd / fronts_soa / resident_knobs
	# 共 9 段。对应的 ClimateProfile flag 字段已在本轮全部删除——所有 hot pass
	# 已恒走 ext != null + has_method 探测分支，C++ 不可用时透明 fallback 到 GDScript，
	# 不再需要 caller 端 flag 验收闭环。本表清空保留外壳，避免 caller import 链断裂。
]
