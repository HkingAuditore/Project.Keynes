...
以上第 17-25 节是本轮实施所依据的设计与验收基线。实际落地范围、已验证结果和仍需
长周期确认的项目见下方实施记录；后续参数调整仍应按 Patch 1 到 Patch 6 分层进行，
避免用下游价格、工资或贸易参数掩盖上游物理生产和投资公式错误。

## 26. 2026-07-20 实施记录

本轮已实施 Patch 1 到 Patch 4 中可以在现有架构内闭环验证的部分，原生
`NativeEconomyRuntime` 仍是唯一经济权威，默认五日滚动结算、PKEC v15 和状态哈希
口径均未改变。

### 26.1 已完成的机制修复

1. 就业汇总改为逐地块、逐 epoch 原子替换贡献。非到期地块在建造或人口结构变更后
   重算就业时，不再扣除本 epoch 尚未计入的旧值，因此失业人口不会出现负数。
2. 自动投资按 `(cell,type)` 聚合所有 owner-lot，使用总安装数量计算已有日产能；普通
   商品只有需求超过已有产能 110%，生存商品超过 105% 才允许扩建。
3. 投资可行性改为“投入和工资按利用率缩放 + 完整业主生计成本”，目标利润按经营
   成本加成计算。另要求既有产能售出率至少 80%、弃置率不超过 10%，并检查回本期、
   出资人资本、建材、投入链和自然资源。
4. 新增 0-13 投资拒绝码；已有产能足够、停产产能、业主空缺、生计不足、售出率、
   弃置、利润、回本期等原因可以直接在建筑快照和 CSV 中定位。
5. 亏损停产建筑保留一个恢复业主，但不保留员工需求，也不产生采购和产出；继续使用
   连续两个盈利反事实周期的原有恢复状态机。
6. 贸易诊断区分当前未解决超期信号与累计唯一超期事件，并按商品记录最后尝试日、
   最后拒绝码和当前是否超期。拒绝码 1-8 分别表示无价差、利润率、路线、来源库存、
   运力、目的地现金、订单上限和已派单。
7. CSV schema 升至 v11。资源行保留自然净变化和人工有符号变化，同时新增自然正向/
   负向净变化、人工生成/开采的正值列。这里的自然正负列是净变化方向拆分，不等同于
   同一天生态出生与死亡的独立 gross 观测。

### 26.2 已落地的内容参数

- `chipped_stone_tools`：默认价 `75000`，最低价 `7500`，最高价 `210000`；石器工坊
  物理产量保持 `220/日`，未用提高产量掩盖价格和投资公式问题。
- `processed_food`：采用 D2，`inventory_target_ratio_q16=43691`，
  `cost_anchor_weight_q16=32768`；公共火塘投入和产出配方保持原值。
- codegen 将石器工坊和公共火塘标记为石器时代物理平衡锚，价格/生活成本校准不能再
  静默改写其吞吐量。两项商品和两项建筑的 curated 源与生成资源已逐字核对一致。

### 26.3 已通过的验证

- `gdext/rebuild.bat`：`template_debug` 与 `template_release` 均构建成功。
- `building_runtime_test.gd`：通过。覆盖非到期建造就业汇总、十日投资窗口、已有产能、
  业主生计、商人死亡修复、续片守恒、save round-trip、scalar/worker hash 等。
- `economy_trade_runtime_test.gd`：通过。新增字段下派单、到货、托管、跨国边界、
  PROBE/OFF、save/restore 和 scalar/worker hash 均保持正确。
- `goods_storage_schema_test.gd`、`economy_data_recorder_test.gd` 和
  `economy_test_bootstrap_test.gd`：通过。CSV v11 的 106/23/55/21/39 列 schema、异步
  三次提交、资源稳定 ID 映射和市场压力快照均已验证。
- `verify_economy_runtime.ps1`：通过；`git diff --check` 无空白错误。
- 全程人口、货币和商品 audit 为 0；续片与非续片权威状态哈希一致。

### 26.4 尚未完成的验收

- 仓库全量 codegen `-Check` 在本轮目标文件之前即被既有
  `data/goods/game_meat.tres` 漂移阻塞；本轮没有把该批无关生成变化混入补丁。
- 尚未完成相同种子的两年、十年重新录制，因此不能宣称采购率、财富 Gini、火塘利润、
  石器长期价格触顶率和资源长期走廊已达到第 23 节硬门槛。
- 自然资源 CSV 已能区分净增长、净消退、人工生成和人工开采；若需要同一天自然 gross
  生成与 gross 死亡的独立值，仍需在 `run_natural_resource_pass` 增加仅录制期启用的
  选中地块累加器，不能从 closing-opening 净值反推。

### 26.5 两年生产路径复验

使用 `tmp_economy_two_year_50x_probe.gd`、seed `20260718`、50 倍速、730 日和生产用
MapGenerator/SUS/native CSV 路径完成复验。固定探针地块是 cell 1227，不是原始 cell
1520，因此结果用于机制级长期回归，不能替代原地块同地图、同初态 A/B。最终 CSV 前缀为：

```text
tmp/economy_50x_2y_seed20260718_cell1227_implementation_trade_fair_scan_v11
```

运行完成 `730/730` 次捕获和写入，`ledger_failures=0`、`fatal=false`，模拟段约
37.2 秒。Godot headless 退出时仍报告 DummyRenderer RID/Resource 泄漏警告，这是探针
清理问题；没有导致经济 fatal 或 CSV 丢行。

| 指标 | 第一年 | 第二年 | 门槛/结论 |
|---|---:|---:|---|
| 实物产出 | 54,711,949,025 | 50,383,080,212 | 第二年/第一年 `92.09%`，低于 `95%`，失败 |
| 普通产出弃置率 | `20.91%` | `21.66%` | 高于 `10%`，失败 |
| 商人采购 spent/budget | `56.86%` | `48.37%` | 第二年明显低于 `60%`，失败 |
| 商业需求资金覆盖 | `88.93%` | `88.13%` | 低于 `90%`，失败 |
| 自动投资启动数 | 294 | 86 | 自动投资已经发生，但第二年显著放缓 |
| 最大亏损停产组 | 22 | 21 | 少量存在，未出现全局停产雪崩 |

cell 1227 的建筑数量保持为石器工坊 3、公共火塘 2、狩猎营地 12、采集地 24、木材
采集 3，没有再次出现石器工坊 `3 -> 12 -> 全停产`。第 730 天石器工坊仍为满利用率，
产出和售出均为 3300；石器价格仍达到 `210000` 上限，但贸易在第 607 天记录实际派单
并出现进口，不再是永久无尝试状态。加工食品第 730 天价格约 87513、库存 297603，
短缺 episode 已解除。

职业现金 Gini 从第 365 天 `0.707` 降至第 730 天 `0.216`，期末财富集中门槛通过，
但经常性现金流仍不健康：第 730 天猎人收入 72,831、支出 3,750,803；工匠总资金仅
190,001；商人总资金 40,455,461。所有 cohort 满意度仍显示 1.0，和严重负现金流不一致，
说明满意度当前容易饱和，不能单独作为健康证据。全地块收入/支出比为 `1.033`，该总量
掩盖了猎人和工匠的职业性风险。

资源方面，cell 1227 两年变化为：木材 `100000 -> 106510.95`（约 `+6.51%`）、野生
动物 `12065.48 -> 13834.04`（约 `+14.66%`）、燧石 `68530.30 -> 68509.18`
（约 `-0.031%`）。本场景未见资源耗竭，燧石投影寿命远高于 3650 日；但这里只证明
该地块、该产业强度下安全，不能替代十年资源压力测试。

### 26.5.1 第 2-7 项实施与最终验证

本轮保持 `NativeEconomyRuntime` 为唯一可变经济权威，默认五日滚动结算、PKEC v15 和状态哈希均未改变；CSV 诊断升级为 v12。

已实现：真实采购机会/分配/未花出额；自产留存消费的实物估值和现金/生计覆盖率；当前超期贸易拒绝原因分桶；无主动短缺恢复时的高丢弃快速利用率反馈；以及停产、注资、恢复、重新招聘的完整回归。真实短缺和生存下限仍有优先权。全部聚焦测试和三类账本均通过。

最终固定 seed `20260718`、cell `1227`、五日结算，标签为 `economy_validation_180_v12`、`economy_validation_730_v12`、`economy_validation_3650_v12`。三次均 `ledger_failures=0`、`fatal=false`。曾尝试让丢弃无条件覆盖 shortage，但 120 周期人口保持率降到 93.98%，低于 95% 门槛，已按回滚条件撤销，未把该失败变体当最终结果。

| 指标 | 180 天 | 730 天 | 3650 天 | 结论 |
|---|---:|---:|---:|---|
| 末 180 天全局产出丢弃率 | 18.33% | 20.20% | 20.34% | 仍高于 10%，未解决 |
| 采购 spent/allocated | 100% | 100% | 100% | 采购执行健康 |
| 当前超期贸易信号 | 5,650 | 3,054 | 2,455 | 下降但未清零 |
| 自动投资累计启动 | 199 | 313 | 454 | 已生效，十年仍缓慢增加 |
| 野生动物存量 | 12,372 | 13,855 | 25,149 | 无耗竭，明显恢复 |
| 木材存量 | 101,543 | 106,513 | 136,867 | 无耗竭 |
| 燧石存量 | 68,524 | 68,509 | 68,431 | 十年约 -0.15% |
| population/money/goods error | 0/0/0 | 0/0/0 | 0/0/0 | 精确守恒 |

第 3650 天的超期原因：未尝试 1,241、来源库存不足 1,202、无价差 3、利润率 9，路线/运力/目的地现金/订单上限均为 0。因此应修复信号覆盖和本地来源供给，不能提高贸易运力。

十年末人口加权个人现金 Gini 为 `0.683`，财富分配门槛未通过。现金/生计覆盖率：采集者 `0.90/0.91`，猎人 `3.73/2.34`（实物价值 4,944,000），工匠 `0.86/0.86`，商人 `0.66/0.66`，矿工 `0.96/0.96`。自产实物诊断证明猎人的短期生计并不差，但采集者、工匠、商人和矿工期末现金流仍需继续处理。

未通过项：长期丢弃仍约 20.3%，现金 Gini 为 0.683，满意度仍可能饱和，双倍建筑资源压力尚未运行。下一轮必须按建筑/商品拆分丢弃贡献并按拒绝桶修复，不能先改工资、配方或贸易运力。

### 26.6 贸易公平扫描修复与剩余问题

两年探针首次发现 summary 的当前超期数会被 `start_epoch()` 清零，而市场行仍显示
超期；该口径已改为在提交边界扫描全部 live signal clock 重算。贸易候选池原先按固定
cell/good 顺序截断，已派单或已拒绝目的地还会长期占据每组前 8 名，使后续地块永久
得不到尝试。本轮进一步实施：

1. 活跃信号扫描起点按模拟日确定性旋转，保持固定上限但覆盖不同 cell/good 区间。
2. 目的地按“从未尝试 -> 已尝试未派单 -> 已派单”排序，再按年龄、价格和数量排序。
3. 完整扫描后没有任何同国同商品来源的目的地直接记录拒绝码 4，不再保持原因 0。
4. 当前最大年龄和当前超期数从全部信号重算；累计值仍只对每个 shortage episode 加一。

修复后第 730 天 summary 如实报告最大信号年龄 729 日、当前超期 3653、累计 episode
32300。这个数字不是修复回归，而是此前被错误隐藏的真实全球贸易缺口。cell 1227 的
石器和加工食品已得到派单/进口；采集植物的旧超期 episode 在期末已经解除，但第 365
天仍曾超期 33 日，说明 15 日响应硬门槛整体仍失败。

下一阶段不应继续扩大路线搜索或无条件增加运力。应按以下顺序处理：

1. 先把采购 spent/budget 从第二年的 `48.37%` 提高到至少 `60%`，定位商人现金、库存
   目标重分配和 producer offer 中未成交的具体原因。
2. 对超期信号按拒绝码分桶；码 4 占主导时修复真实来源产能/库存，码 2 才调整利润阈值，
   码 3 才检查拓扑和路线预算，码 6 才处理目的地现金。
3. 将普通商品弃置从 `21.66%` 降到 `10%` 以下，优先减少持续过量生产或提高同地采购，
   不得通过销毁、补贴或抬价伪造改善。
4. 针对猎人和工匠做职业预算追踪。只有在雇主五日经营成本、业主生计和下一周期投入
   均可覆盖时，才允许 5%-10% 的职业定向收入调整。
5. 完成同初态的 cell 1520 两年 A/B 和十年压力测试后，才能决定是否继续改工资、配方、
   商人预算或贸易参数。
