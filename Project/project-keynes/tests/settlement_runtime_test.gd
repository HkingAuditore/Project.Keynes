...
func _run_runtime_checks(compiled: Dictionary) -> void:
	...
	_expect("PKEC v27 可导出", not chunks.is_empty())
	...
	if restored != null and not chunks.is_empty():
		...
		_expect("PKEC v27 可往返恢复", bool(ended.get("ok", false)))
		_expect("PKEC v27 状态哈希一致",
