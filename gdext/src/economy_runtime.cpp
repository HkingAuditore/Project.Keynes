...
int64_t NativeEconomyRuntime::living_merchant_population(int32_t cell) const {
    int64_t total = 0;
    int64_t sat = 0;
    _population.for_each_in_cell(cell, [&](int32_t slot) {
        if (is_merchant_slot(slot))
            total = saturating_add(total, _population.population[slot], sat);
    });
    return total;
}
...
void NativeEconomyRuntime::sum_family_expedition_holdings(
    ...
    for (int32_t expedition = 0; expedition < static_cast<int32_t>(
        ...
        population = saturating_add(population,
            family_expedition_payload_people(expedition), saturation);
...
int64_t NativeEconomyRuntime::family_expedition_payload_people(
    ...
    const uint32_t end = std::min<uint32_t>(
        ...
        begin + _family_expeditions.payload_count[expedition]);
    for (uint32_t payload = begin; payload < end; ++payload)
        people += std::max<int64_t>(0,
            _family_expedition_payloads[payload].people);
    return people;
}
...
bool NativeEconomyRuntime::rebuild_merchant_ranges(std::string &error) {
    ...
    for (int32_t cell = 0; cell < _cell_count; ++cell) {
        ...
        collect_cell_merchants();
        if (static_cast<int32_t>(_merchant_slots.size()) == _merchant_offsets[cell]) {
            ...
            if (population > 0) {
                ...
                _merchant_repairs = saturating_add(
                    _merchant_repairs, repairs, _saturation_count);
                collect_cell_merchants();
                if (static_cast<int32_t>(_merchant_slots.size()) ==
                    _merchant_offsets[cell]) {
                    error = "merchant_invariant_broken:cell=" + std::to_string(cell);
                    return false;
                }
            }
        }
...
bool NativeEconomyRuntime::repair_cell_merchant_and_rebuild(
        int32_t cell, std::string &error) {
    int64_t repairs = 0;
    if (!ensure_merchant_invariant(cell, repairs, error)) return false;
    _merchant_repairs = saturating_add(
        _merchant_repairs, repairs, _saturation_count);
    return rebuild_merchant_ranges(error);
}
...
bool NativeEconomyRuntime::advance_country_research_procurement(
    ...
    if (continuation.phase == 1) {
        ...
        if (route == CountryWorkerAssetRoute::GATE_SOFT_UNAVAILABLE) {
            // M4 research peer is not open yet under Country unique-writer.
            // Soft-reject this candidate; do not fatal the economy epoch.
            return reject_prepare("country_economy_operation_gate_closed");
        }
        if (route != CountryWorkerAssetRoute::NOT_APPLICABLE) {
            // Unexpected enqueue/terminal under research: keep hard fault —
            // research must not look completed when the peer never ran.
            continuation.phase = 7;
            continuation.last_error = error.empty()
                ? "country_research_worker_route_unexpected" : error;
            continuation.active = false;
            return false;
        }
...
NativeEconomyRuntime::block_or_enqueue_country_worker_asset(
    ...
    // Soft-unavailable (not TERMINAL_ERROR): callers must skip the op without
    // failing the whole economy runtime — Country unique-writer + Economy sync
    // is an expected production pairing before M2–M5 land.
    const uint32_t op_bit = 1u << static_cast<uint32_t>(operation);
    if ((_d7_operation_gate_mask & op_bit) == 0u) {
        error = "country_economy_operation_gate_closed";
        return CountryWorkerAssetRoute::GATE_SOFT_UNAVAILABLE;
    }
...
bool NativeEconomyRuntime::fiscal_peer_journal_matches(
    ...
    if (record.request_id != request.request_id ||
        ...
        error = "country_economy_asset_peer_journal_identity_mismatch";
        return false;
    }
    return true;
}
...
bool NativeEconomyRuntime::service_country_economy_asset_peer(
    ...
    for (uint32_t inspected = 0; inspected < max_requests; ++inspected) {
        ...
        const uint32_t op_bit =
            1u << static_cast<uint32_t>(request.operation);
        const bool gate_open = (_d7_operation_gate_mask & op_bit) != 0u;
        const bool prepared =
        ...
        if (!fiscal) {
            ...
            if (!gate_open || !prepared ||
                ...
                record_fiscal_peer_terminal(
                    ...
                    "country_economy_asset_request_invalid");
                std::string submit_error;
                if (!submit_terminal(
                        request, RuntimeEconomyAssetResultCode::REJECTED,
                        RuntimeEconomyAssetState::REJECTED, 0, 0,
                    ...
                    if (!_simulation_host->requeue_country_economy_asset_request(
                            request.request_id, requeue_error)) {
                        error = requeue_error.empty() ? submit_error
                                                      : requeue_error;
                        return false;
                    }
                    error = submit_error.empty()
            ...
            if (!apply_peer_asset_side_effects(request, side_error)) {
                record_fiscal_peer_terminal(
                    request, RuntimeEconomyAssetResultCode::REJECTED,
                    RuntimeEconomyAssetState::REJECTED, 0, 0,
                    side_error.empty() ? "country_economy_asset_side_effect_failed"
                                       : side_error.c_str());
                std::string submit_error;
                if (!submit_terminal(
                        request, RuntimeEconomyAssetResultCode::REJECTED,
                        RuntimeEconomyAssetState::REJECTED, 0, 0,
                        side_error.empty()
                            ? "country_economy_asset_side_effect_failed"
                            : side_error.c_str(),
                        submit_error)) {
                    std::string requeue_error;
                    if (!_simulation_host->requeue_country_economy_asset_request(
            ...
            record_fiscal_peer_terminal(
                ...
                RuntimeEconomyAssetState::COMPLETED, committed_qty,
                committed_cash, "");
            std::string submit_error;
            if (!submit_terminal(
                    request, RuntimeEconomyAssetResultCode::COMPLETED,
                    RuntimeEconomyAssetState::COMPLETED, committed_qty,
                    committed_cash, "", submit_error)) {
                std::string requeue_error;
                if (!_simulation_host->requeue_country_economy_asset_request(
                        request.request_id, requeue_error)) {
                    error = requeue_error.empty() ? submit_error : requeue_error;
...
bool NativeEconomyRuntime::apply_peer_asset_side_effects(
    ...
    case RuntimeEconomyAssetOperation::CASH_FROM_COHORT: {
        const int32_t slot = request.target_slot;
        if (slot < 0 ||
            slot >= static_cast<int32_t>(_population.funds.size()) ||
            amount <= 0) {
    ...
    case RuntimeEconomyAssetOperation::GOOD_FROM_MARKET: {
        ...
        if (market < 0 || market >= _market.market_count || good < 0 ||
            good >= _market.good_count || amount <= 0) {
            error = "country_economy_market_goods_lane_invalid";
            return false;
        }
        const size_t index = static_cast<size_t>(_market.index(market, good));
        if (index >= _market.stock.size()) {
            error = "country_economy_market_goods_lane_invalid";
            return false;
        }
        if (request.operation == RuntimeEconomyAssetOperation::GOOD_TO_MARKET) {
            if (_market.stock[index] >
                    std::numeric_limits<int64_t>::max() - amount) {
                error = "country_economy_market_goods_overflow";
                return false;
            }
            _market.stock[index] += amount;
        } else {
            if (_market.stock[index] < amount) {
    ...
    case RuntimeEconomyAssetOperation::TREASURY_SPEND:
        // Country already transferred treasury value; no Economy-local store.
        return true;
    ...
    case RuntimeEconomyAssetOperation::FISCAL_COLLECT:
        error = "country_economy_asset_side_effect_fiscal_unexpected";
        return false;
    }
    error = "country_economy_asset_side_effect_unknown";
    return false;
}

void NativeEconomyRuntime::publish_shadow_graph_stage_reference(
        RuntimeEconomyGraphStage stage) {
    // Same SHADOW-only guard as BUILDING_PLAN: skip when worker owns ECONOMY.
    if (_simulation_host == nullptr ||
...
bool NativeEconomyRuntime::bind_soa_view(EconomySoAView &view,
                                         std::string &error) {
    error.clear();
    view = EconomySoAView{};
    ...
    view.good_count = static_cast<uint32_t>(std::max(0, _market.good_count));
    ...
    view.building_active_cells = _building_active_cells.empty()
        ? nullptr : _building_active_cells.data();
    view.building_active_cells_count = _building_active_cells.size();
    view.epoch_plan_cells =
        _epoch_plan_cells.empty() ? nullptr : _epoch_plan_cells.data();
    view.epoch_plan_cells_count = _epoch_plan_cells.size();
    view.survival_food_good_mask = _survival_food_good_mask.empty()
        ? nullptr : _survival_food_good_mask.data();
    view.survival_clothing_good_mask = _survival_clothing_good_mask.empty()
        ? nullptr : _survival_clothing_good_mask.data();
    view.survival_mask_count = std::min(_survival_food_good_mask.size(),
                                        _survival_clothing_good_mask.size());
    view.population_by_cell = _cell_population_total.empty()
        ? nullptr : _cell_population_total.data();
...
std::unique_ptr<EconomyGraphStageOps>
NativeEconomyRuntime::make_graph_stage_ops(bool mutate) {
    return std::make_unique<NativeEconomyGraphStageOps>(this, mutate);
}

NativeEconomyRuntime::Command NativeEconomyRuntime::command_from_pod(
        const RuntimeEconomyPodCommand &pod) const {
    Command cmd;
    cmd.opcode = pod.opcode;
    cmd.target_handle = static_cast<uint64_t>(
        pod.target_cohort != 0 ? pod.target_cohort : pod.target_building);
    cmd.i32_0 = pod.target_cell;
    cmd.i32_1 = static_cast<int32_t>(pod.target_country);
    cmd.i64_0 = pod.payload0;
...
bool NativeEconomyRuntime::apply_pod_command(
        const RuntimeEconomyPodCommand &pod, std::string &error) {
    error.clear();
    if (pod.opcode == COMMAND_BUILD_CANAL && pod.payload0 == 0) {
        error = "economy_command_build_canal_token_missing";
        return false;
    }
    const Command cmd = command_from_pod(pod);
    return apply_command(cmd, error);
}

bool NativeEconomyRuntime::enqueue_or_apply_pod_command(
        const RuntimeEconomyPodCommand &pod, std::string &error) {
    error.clear();
    if (_epoch_active) {
        _epoch_commands.push_back(command_from_pod(pod));
        return true;
    }
    return apply_pod_command(pod, error);
}

void NativeEconomyRuntime::fill_economy_audit_snapshot(
        int64_t &pop_err, int64_t &money_err, int64_t &goods_err,
        uint32_t &dirty_family_mask, int64_t &changed_cells,
        int64_t &changed_cohorts) const {
    pop_err = 0;
    money_err = 0;
    goods_err = 0;
    dirty_family_mask = 0;
    changed_cells = static_cast<int64_t>(_population_changed_cells.size());
    changed_cohorts = 0;
    if (_fatal) {
        pop_err = 1;
        money_err = 1;
    ...
    if (_epoch_active) return;
    const int64_t population_expected =
        _opening_totals.population + _births - _deaths +
    ...
    pop_err = _closing_totals.population - population_expected;
    money_err = money_close -
        (money_open + _explicit_money_mint - _explicit_money_burn);
    goods_err = _closing_totals.goods_stock - goods_expected;
    dirty_family_mask = _families.active_count > 0 ? 1u : 0u;
    changed_cohorts = _population.active_count;
}

void NativeEconomyRuntime::fill_economy_business_summary(
        int64_t &summary_population, int64_t &summary_funds,
        int32_t &summary_markets, int32_t &summary_buildings,
        int32_t &summary_cohorts, int32_t &summary_families) const {
    summary_population = _closing_totals.population != 0
        ? _closing_totals.population
        : _opening_totals.population;
    summary_funds = (_closing_totals.cohort_funds != 0 ||
                     _closing_totals.country_cash != 0)
        ? (_closing_totals.cohort_funds + _closing_totals.country_cash +
           _closing_totals.escrow_cash)
        : (_opening_totals.cohort_funds + _opening_totals.country_cash +
           _opening_totals.escrow_cash);
    summary_markets = _market.market_count;
    summary_buildings = static_cast<int32_t>(_buildings.size());
    summary_cohorts = static_cast<int32_t>(std::min<int64_t>(
        std::max<int64_t>(0, _population.active_count),
        static_cast<int64_t>(std::numeric_limits<int32_t>::max())));
    summary_families = static_cast<int32_t>(std::min<int64_t>(
        std::max<int64_t>(0, _families.active_count),
        static_cast<int64_t>(std::numeric_limits<int32_t>::max())));
}
...
bool NativeEconomyRuntime::kernel_run_household_market_boundary(
    ...
    if (!_bootstrapped || _fatal) {
        ...
        result.state_hash = static_cast<uint64_t>(
            std::max<int64_t>(0, state_hash()));
        result.ok = true;
        return true;
    }
    // One household post-building finalize chunk when that phase is active;
    // otherwise ok boundary (full settle remains on compact slice).
    if (_household_market_phase == 1 && !_epoch_building_cells.empty()) {
        const int32_t begin = _household_post_cursor;
        const int32_t end = household_post_slice_end(begin);
        ...
        for (int32_t relative = 0; relative < count; ++relative) {
            ...
            finalize_household_building_cell(
                _epoch_building_cells[static_cast<size_t>(begin + relative)],
                sat, restarted, failed);
            _saturation_count =
                saturating_add(_saturation_count, sat, _saturation_count);
            _recovery_restarted = saturating_add(_recovery_restarted,
                                                 restarted, _saturation_count);
            _recovery_failed = saturating_add(_recovery_failed, failed,
                                              _saturation_count);
        }
        _household_post_cursor = end;
        if (_household_post_cursor >=
            static_cast<int32_t>(_epoch_building_cells.size())) {
...
bool NativeEconomyRuntime::coordinate_country_fiscal_transaction(
    ...
    error.clear();
    if (_country_runtime == nullptr || !_country_runtime->economy_available()) {
        error = "country_fiscal_peer_unavailable";
        return false;
    }
    ...
    if (!static_cast<bool>(begun.get("ok", false))) {
        error = String(begun.get("code", "country_fiscal_country_rejected")).utf8().get_data();
        return false;
    }
    ...
    if (transaction_id == 0 || session_epoch == 0 || country_generation == 0 ||
        ...
        error = "country_fiscal_peer_identity_invalid";
        return false;
    }
    const size_t country_index = static_cast<size_t>(country);
    const int64_t escrow = country_index < _fiscal_escrow_by_country.size()
        ? _fiscal_escrow_by_country[country_index] : 0;
    if (operation != NativeCountryRuntime::ECONOMY_ASSET_FISCAL_RESERVE &&
        escrow < prepared_quantity) {
        _country_runtime->acknowledge_economy_asset_peer_prepared(
            transaction_id, session_epoch, country_generation, peer_generation,
            false, String("country_fiscal_peer_escrow_insufficient"));
        error = "country_fiscal_peer_escrow_insufficient";
        return false;
    }
    if (operation == NativeCountryRuntime::ECONOMY_ASSET_FISCAL_RESERVE &&
        escrow > std::numeric_limits<int64_t>::max() - prepared_quantity) {
        _country_runtime->acknowledge_economy_asset_peer_prepared(
            transaction_id, session_epoch, country_generation, peer_generation,
            false, String("country_fiscal_peer_escrow_overflow"));
        error = "country_fiscal_peer_escrow_overflow";
        return false;
    }
    const godot::Dictionary prepared =
        _country_runtime->acknowledge_economy_asset_peer_prepared(
            ...
            true, String());
    if (!static_cast<bool>(prepared.get("ok", false))) {
        error = "country_fiscal_peer_prepare_failed";
        return false;
    }
    ...
    if (!static_cast<bool>(commit.get("ok", false))) {
        error = String(commit.get("code", "country_fiscal_country_commit_failed")).utf8().get_data();
        return false;
    }
    ...
    if (actual <= 0 || actual > prepared_quantity) {
        error = "country_fiscal_peer_commit_quantity_invalid";
        return false;
    }
    if (operation == NativeCountryRuntime::ECONOMY_ASSET_FISCAL_RESERVE)
    ...
    if (!static_cast<bool>(applied.get("ok", false)) ||
        String(applied.get("status_name", "")) != "completed") {
        error = "country_fiscal_peer_apply_ack_failed";
        return false;
    }
    committed = actual;
    return true;
...
bool NativeEconomyRuntime::coordinate_country_cohort_cash(
    ...
    if (transaction_id == 0 || session_epoch == 0 || country_generation == 0 ||
        prepared_quantity <= 0 || peer_generation == 0) {
        error = "country_cohort_cash_identity_invalid";
        return false;
    }

    const int64_t funds_before = std::max<int64_t>(0, _population.funds[cohort_slot]);
    const int64_t income_before = std::max<int64_t>(
        0, _population.epoch_income[cohort_slot]);
    if (operation == NativeCountryRuntime::ECONOMY_ASSET_CASH_TO_COHORT) {
        if (funds_before > std::numeric_limits<int64_t>::max() - prepared_quantity ||
            income_before > std::numeric_limits<int64_t>::max() - prepared_quantity) {
            _country_runtime->acknowledge_economy_asset_peer_prepared(
                transaction_id, session_epoch, country_generation, peer_generation,
                false, String("country_cohort_cash_credit_overflow"));
            error = "country_cohort_cash_credit_overflow";
            return false;
        }
    } else if (funds_before < prepared_quantity) {
        _country_runtime->acknowledge_economy_asset_peer_prepared(
            transaction_id, session_epoch, country_generation, peer_generation,
            false, String("country_cohort_cash_insufficient"));
        error = "country_cohort_cash_insufficient";
        return false;
    }
    ...
    const godot::Dictionary prepared =
        _country_runtime->acknowledge_economy_asset_peer_prepared(
            ...
            true, String());
    if (!static_cast<bool>(prepared.get("ok", false))) {
        error = "country_cohort_cash_peer_prepare_failed";
        return false;
    }
    const godot::Dictionary commit =
        _country_runtime->commit_economy_asset_transaction(transaction_id);
    if (!static_cast<bool>(commit.get("ok", false))) {
        error = String(commit.get("code", "country_cohort_cash_country_commit_failed"))
            .utf8().get_data();
        return false;
    }
    const int64_t actual = static_cast<int64_t>(
        commit.get("committed_quantity", prepared_quantity));
    if (actual <= 0 || actual > prepared_quantity) {
        error = "country_cohort_cash_commit_quantity_invalid";
        return false;
    }

    touch_accounting_slot(cohort_slot);
    if (operation == NativeCountryRuntime::ECONOMY_ASSET_CASH_TO_COHORT) {
        _population.funds[cohort_slot] = saturating_add(
            _population.funds[cohort_slot], actual, _saturation_count);
        _population.epoch_income[cohort_slot] = saturating_add(
            _population.epoch_income[cohort_slot], actual, _saturation_count);
    } else {
        _population.funds[cohort_slot] -= actual;
        _population.epoch_expense[cohort_slot] = saturating_add(
            _population.epoch_expense[cohort_slot], actual, _saturation_count);
    }
    const godot::Dictionary applied =
        _country_runtime->acknowledge_economy_asset_peer_applied(
            transaction_id, session_epoch, country_generation, peer_generation,
    ...
    if (!static_cast<bool>(applied.get("ok", false)) ||
        String(applied.get("status_name", "")) != "completed") {
        error = "country_cohort_cash_peer_apply_ack_failed";
        return false;
    }
    committed = actual;
    return true;
...
bool NativeEconomyRuntime::coordinate_country_treasury_spend(
    ...
    const CountryWorkerAssetRoute route = block_or_enqueue_country_worker_asset(
            ...
            error);
    if (route == CountryWorkerAssetRoute::GATE_SOFT_UNAVAILABLE) {
        // Treasury spend must not silently create construction/canal without
        // paying. Reject the coordinator so callers soft-fail the command.
        committed_cash = 0;
        return false;
    }
    if (route != CountryWorkerAssetRoute::NOT_APPLICABLE)
...
bool NativeEconomyRuntime::run_government_research_procurement(std::string &error) {
    ...
    // Country unique-writer: M4 research peer is closed. Skip the whole stage
    // rather than sealing candidates that can only soft-reject.
    if (_country_runtime->sync_store_writes_forbidden()) {
        _country_research_procurement_done = true;
        _country_research_procurement_continuation =
            CountryResearchProcurementContinuation{};
        return true;
    }
    ...
    if (!_country_research_procurement_initialized) {
        ...
        for (const int32_t market : _epoch_market_ids) {
            ...
            _country_research_procurement_candidates.push_back({
                country, market, good, _market.price[index]});
        }
        std::stable_sort(_country_research_procurement_candidates.begin(),
            _country_research_procurement_candidates.end(),
            [](const CountryResearchProcurementCandidate &lhs,
               const CountryResearchProcurementCandidate &rhs) {
...
bool NativeEconomyRuntime::capture_environment(int64_t day_index, const float *temperature,
    ...
    _environment_precipitation_q16.resize(count);
    _environment_snow_q16.resize(count);
    _environment_weather_q16.resize(count);
    if (_cell_previous_precipitation_q16.size() != static_cast<size_t>(count))
        _cell_previous_precipitation_q16.assign(static_cast<size_t>(count), 0);
    if (_cell_rain_event_q16.size() != static_cast<size_t>(count))
        _cell_rain_event_q16.assign(static_cast<size_t>(count), 0);
    ...
    for (int32_t i = 0; i < count; ++i) {
        ...
        constexpr int32_t RAIN_EVENT_THRESHOLD_Q16 = Q16_ONE / 4;
        const int32_t threshold =
            i < static_cast<int32_t>(_epoch_cell_rain_event_threshold_q16.size())
                ? std::max(1, _epoch_cell_rain_event_threshold_q16[
                    static_cast<size_t>(i)])
                : RAIN_EVENT_THRESHOLD_Q16;
        _cell_rain_event_q16[static_cast<size_t>(i)] =
            (current >= threshold && current > previous)
                ? static_cast<int32_t>(Q16_ONE) : 0;
        _cell_previous_precipitation_q16[static_cast<size_t>(i)] = current;
        ...
        mix(static_cast<uint32_t>(_environment_precipitation_q16[i]));
...
bool NativeEconomyRuntime::valid_family_effect_entity_handle(
        uint64_t handle) const {
    int32_t index = -1;
    return _families.valid_handle(handle, index) ||
        _family_influences.valid_handle(handle, index);
}
...
bool NativeEconomyRuntime::capture_country_epoch(std::string &error) {
    ...
    refresh_city_output_modifier_factors();
...
void NativeEconomyRuntime::refresh_epoch_bullion_quota() {
    ...
    if (_cell_count <= 0 || _market.good_count <= 0 ||
    ...
    for (const int32_t cell : _epoch_settlement_cells) {
        ...
        cell_population[static_cast<size_t>(cell)] = population;
        const int32_t country = cell < static_cast<int32_t>(
                _epoch_cell_country.size())
            ? _epoch_cell_country[static_cast<size_t>(cell)] : -1;
        if (country >= 0 && country < _epoch_country_count)
            country_population[static_cast<size_t>(country)] = saturating_add(
                country_population[static_cast<size_t>(country)], population,
                _saturation_count);
        cell_weight[static_cast<size_t>(cell)] = funds;
    }
    // Country treasury cash is apportioned by population after fiscal reserve;
    // this keeps the world cap global while allowing a cash-poor but populated
    ...
    if (_country_runtime != nullptr) {
        for (const int32_t cell : _epoch_settlement_cells) {
            ...
            const int32_t country = cell < static_cast<int32_t>(
                ...
                ? _epoch_cell_country[static_cast<size_t>(cell)] : -1;
            if (country < 0 || country >= _epoch_country_count) continue;
            const int64_t national_cash = std::max<int64_t>(0,
                _country_runtime->cash_for_slot(country));
            const int64_t population = cell_population[static_cast<size_t>(cell)];
            const int64_t total_population = country_population[
                static_cast<size_t>(country)];
            if (national_cash <= 0 || population <= 0 || total_population <= 0)
                continue;
            cell_weight[static_cast<size_t>(cell)] = saturating_add(
                cell_weight[static_cast<size_t>(cell)],
                mul_div_sat(national_cash, population, total_population,
    ...
    // Only spendable money can justify new currency issuance. Fiscal/trade/
    // expedition escrow is locked and must not be counted as the monetary
    // base; using _opening_totals.country_cash here would also double-count
    // fiscal cash after the reservation pass has moved it into escrow.
    const int64_t spendable_country_cash = _country_runtime != nullptr
        ? std::max<int64_t>(0, _country_runtime->total_cash())
        : std::max<int64_t>(0, _opening_totals.country_cash);
    ...
    for (const int32_t cell : _epoch_settlement_cells) {
        ...
        // cell with only one monetary producer never strands the other half of
        // an artificial per-good split, while competing goods still contend for
        // the same issuance budget during settlement.
        // The product can exceed int64 even though the apportioned quotient is
        // bounded by epoch_cap (weight <= total_weight).  Do the quotient and
        // remainder in 128-bit arithmetic so saturation cannot turn the
        // fractional remainder into billions of fake whole units.
        const uint64_t ua = static_cast<uint64_t>(epoch_cap);
        const uint64_t ub = static_cast<uint64_t>(
            cell_weight[static_cast<size_t>(cell)]);
        const uint64_t ud = static_cast<uint64_t>(total_weight);
        uint64_t quotient = 0;
        uint64_t remainder = 0;
        if (ua <= std::numeric_limits<uint64_t>::max() / ub) {
            const uint64_t product = ua * ub;
            quotient = product / ud;
            remainder = product % ud;
        } else {
#if defined(_MSC_VER) && defined(_M_X64)
            uint64_t product_hi = 0;
            const uint64_t product_lo = _umul128(ua, ub, &product_hi);
            if (product_hi >= ud) {
                // This cannot occur while ub <= ud and epoch_cap fits in
                // int64, but keep the path saturating if a future caller
                ...
                quotient = _udiv128(product_hi, product_lo, ud, &remainder);
            }
#else
            const unsigned __int128 product =
                static_cast<unsigned __int128>(ua) * ub;
            quotient = static_cast<uint64_t>(product / ud);
            remainder = static_cast<uint64_t>(product % ud);
#endif
        }
        rows.push_back({static_cast<uint64_t>(static_cast<uint32_t>(cell)),
                        cell_weight[static_cast<size_t>(cell)],
                        static_cast<int64_t>(std::min<uint64_t>(
                            quotient, static_cast<uint64_t>(epoch_cap))),
                        static_cast<int64_t>(remainder)});
    }
    if (rows.empty()) return;

    // Largest-remainder apportionment keeps the sum exactly at epoch_cap (when
    // there are enough weighted cells) instead of losing the fractional tail
    // independently in every cell. Ties use the stable cell id, preserving
    // replay equality and making tiny but positive cells discoverable.
    int64_t base_total = 0;
    for (const QuotaRow &row : rows)
        base_total = saturating_add(base_total, row.base, _saturation_count);
    int64_t remainder_units = std::max<int64_t>(
        0, epoch_cap - std::min(epoch_cap, base_total));
    // Largest-remainder apportionment has fewer leftover units than rows.  A
    // violated bound indicates a numeric regression; clamp it so malformed
    // inputs can never turn this bounded step into an unbounded hot loop.
    const int64_t max_remainder_units = static_cast<int64_t>(rows.size()) - 1;
    if (remainder_units > std::max<int64_t>(0, max_remainder_units)) {
        ++_saturation_count;
        remainder_units = std::max<int64_t>(0, max_remainder_units);
    }
    _epoch_bullion_quota_remainder_units = remainder_units;
    std::vector<size_t> remainder_order(rows.size());
    for (size_t i = 0; i < rows.size(); ++i) remainder_order[i] = i;
    std::stable_sort(remainder_order.begin(), remainder_order.end(),
        [&](size_t lhs, size_t rhs) {
            ...
            return rows[lhs].key < rows[rhs].key;
        });
    for (int64_t i = 0; i < remainder_units; ++i)
        rows[remainder_order[static_cast<size_t>(i) % rows.size()]].base =
            saturating_add(rows[remainder_order[static_cast<size_t>(i) % rows.size()]].base,
                           1, _saturation_count);

    std::sort(rows.begin(), rows.end(), [](const QuotaRow &a, const QuotaRow &b) {
        return a.key < b.key;
    });
    for (const QuotaRow &row : rows) {
        _epoch_bullion_quota_keys.push_back(row.key);
        _epoch_bullion_quota_initial.push_back(std::max<int64_t>(0, row.base));
        _epoch_bullion_quota_remaining.push_back(std::max<int64_t>(0, row.base));
        _epoch_bullion_quota_initial_total = saturating_add(
            _epoch_bullion_quota_initial_total, std::max<int64_t>(0, row.base),
            _saturation_count);
        _epoch_bullion_quota_remaining_total = saturating_add(
...
int64_t NativeEconomyRuntime::bullion_cell_quota_remaining(
    ...
    return index < _epoch_bullion_quota_remaining.size()
        ? std::max<int64_t>(0, _epoch_bullion_quota_remaining[index]) : 0;
}
...
int64_t NativeEconomyRuntime::bullion_cell_quota_initial(
    ...
    const size_t index = static_cast<size_t>(it -
        _epoch_bullion_quota_keys.begin());
    return index < _epoch_bullion_quota_initial.size()
        ? std::max<int64_t>(0, _epoch_bullion_quota_initial[index]) : 0;
}
...
void NativeEconomyRuntime::refresh_city_modifier_factors() {
    ...
    const auto shared_factor = [&](int32_t stat_id) {
        return modifier_factor_q16(_modifier_runtime->effective_value(
            ModifierRuntime::ECONOMY, stat_id, 0,
...
void NativeEconomyRuntime::refresh_city_output_modifier_factors() {
    ...
    const bool shape_valid =
        ...
        _city_output_cell_offsets.size() == cells + 1 &&
        _city_output_good_indices.size() == _city_output_factors_q16.size();
    if (_modifier_runtime == nullptr) {
        if (_epoch_city_output_factor_valid && shape_valid) return;
        _city_output_shared_goods_q16.assign(good_count, Q16_ONE);
        _city_output_cell_offsets.assign(cells + 1, 0);
        _city_output_good_indices.clear();
        _city_output_factors_q16.clear();
        _city_output_scope_cells_scratch.clear();
        _city_output_scope_stat_ids_scratch.clear();
        _epoch_city_output_factor_stat_version = 0;
        _epoch_city_output_factor_valid = true;
        return;
    }

    const uint64_t version = _modifier_runtime->stat_bucket_version(
        ModifierRuntime::ECONOMY, _city_good_output_stat_ids.data(),
        _city_good_output_stat_ids.size());
    if (_epoch_city_output_factor_valid && shape_valid &&
        _epoch_city_output_factor_stat_version == version)
        return;

    _city_output_shared_goods_q16.resize(good_count);
    for (size_t good = 0; good < good_count; ++good) {
        _city_output_shared_goods_q16[good] = modifier_factor_q16(
            _modifier_runtime->effective_value(
                ModifierRuntime::ECONOMY,
                _city_good_output_stat_ids[good], 0,
                std::numeric_limits<uint64_t>::max(), 1.0));
    }

    _modifier_runtime->collect_scope_stat_entries(
        ModifierRuntime::ECONOMY, ModifierRuntime::GROUP,
        _city_good_output_stat_ids.data(), _city_good_output_stat_ids.size(),
        _city_output_scope_cells_scratch,
        _city_output_scope_stat_ids_scratch);
    std::unordered_map<int32_t, int32_t> good_by_stat;
    good_by_stat.reserve(good_count);
    for (size_t good = 0; good < good_count; ++good)
        good_by_stat.emplace(_city_good_output_stat_ids[good],
                             static_cast<int32_t>(good));
    struct OutputOverride {
        int32_t cell = -1;
        int32_t good = -1;
        int32_t factor_q16 = Q16_ONE;
        bool operator<(const OutputOverride &other) const {
            return cell != other.cell ? cell < other.cell : good < other.good;
        }
    };
    std::vector<OutputOverride> overrides;
    overrides.reserve(_city_output_scope_cells_scratch.size());
    for (size_t entry = 0;
         entry < _city_output_scope_cells_scratch.size() &&
         entry < _city_output_scope_stat_ids_scratch.size(); ++entry) {
        const uint64_t scope = _city_output_scope_cells_scratch[entry];
        const auto found = good_by_stat.find(
            _city_output_scope_stat_ids_scratch[entry]);
        if (scope >= cells || found == good_by_stat.end()) continue;
        const int32_t good = found->second;
        const int32_t factor_q16 = modifier_factor_q16(
            _modifier_runtime->effective_value(
                ModifierRuntime::ECONOMY,
                _city_good_output_stat_ids[static_cast<size_t>(good)], 0,
                scope, 1.0));
        if (factor_q16 == _city_output_shared_goods_q16[
                static_cast<size_t>(good)])
            continue;
        overrides.push_back({static_cast<int32_t>(scope), good, factor_q16});
    }
    std::sort(overrides.begin(), overrides.end());
    _city_output_cell_offsets.assign(cells + 1, 0);
    for (const OutputOverride &entry : overrides)
        ++_city_output_cell_offsets[static_cast<size_t>(entry.cell) + 1];
    for (size_t cell = 1; cell < _city_output_cell_offsets.size(); ++cell)
        _city_output_cell_offsets[cell] += _city_output_cell_offsets[cell - 1];
    _city_output_good_indices.resize(overrides.size());
    _city_output_factors_q16.resize(overrides.size());
    for (size_t index = 0; index < overrides.size(); ++index) {
        _city_output_good_indices[index] = overrides[index].good;
        _city_output_factors_q16[index] = overrides[index].factor_q16;
    }
    _epoch_city_output_factor_stat_version = version;
    _epoch_city_output_factor_valid = true;
}

int32_t NativeEconomyRuntime::city_good_output_factor_q16(
        int32_t cell, int32_t good_id) const {
    if (good_id < 0 || good_id >= static_cast<int32_t>(
            _city_output_shared_goods_q16.size()))
        return Q16_ONE;
    const int32_t shared = _city_output_shared_goods_q16[
        static_cast<size_t>(good_id)];
    if (cell < 0 || cell + 1 >= static_cast<int32_t>(
            _city_output_cell_offsets.size()))
        return shared;
    const int32_t begin = _city_output_cell_offsets[
        static_cast<size_t>(cell)];
    const int32_t end = _city_output_cell_offsets[
        static_cast<size_t>(cell + 1)];
    const auto first = _city_output_good_indices.begin() + begin;
    const auto last = _city_output_good_indices.begin() + end;
    const auto found = std::lower_bound(first, last, good_id);
    if (found == last || *found != good_id) return shared;
    return _city_output_factors_q16[static_cast<size_t>(
        std::distance(_city_output_good_indices.begin(), found))];
}
...
class NativeEconomyBuildingPlanExecutor final : public EconomyBuildingPlanExecutor {
public:
    NativeEconomyRuntime *runtime = nullptr;
    NativeEconomyRuntime::BuildingPlanResult *legacy_result = nullptr;
...
bool NativeEconomyBuildingPlanExecutor::execute(
        int32_t active_begin, int32_t active_end,
        const int32_t *cells_override, int32_t cells_count,
        EconomyBuildingPlanResult &result, std::string &error) {
    if (runtime == nullptr || legacy_result == nullptr) {
        error = "economy_building_plan_executor_unbound";
        result.ok = false;
        return false;
    }
    std::vector<int32_t> override_storage;
    const std::vector<int32_t> *override_ptr = nullptr;
    if (cells_override != nullptr && cells_count > 0) {
        override_storage.assign(cells_override, cells_override + cells_count);
        override_ptr = &override_storage;
    }
    if (!runtime->prepare_building_economic_plan_body(
            active_begin, active_end, override_ptr, *legacy_result, error)) {
        result.ok = false;
        if (!error.empty()) {
            const size_t n = error.size() < sizeof(result.error) - 1u
                ? error.size() : sizeof(result.error) - 1u;
            std::memcpy(result.error, error.data(), n);
            result.error[n] = '\0';
        }
        return false;
    }
    result.ok = legacy_result->ok;
    result.saturation_count = legacy_result->saturation_count;
    result.merchant_credit_budget = legacy_result->merchant_credit_budget;
    result.merchant_credit_committed = legacy_result->merchant_credit_committed;
    result.recovery_candidates = legacy_result->recovery_candidates;
    result.recovery_approved = legacy_result->recovery_approved;
    result.loss_suspended_building_groups =
        legacy_result->loss_suspended_building_groups;
    result.unprofitable_building_groups =
        legacy_result->unprofitable_building_groups;
    result.utilization_sum_q16 = legacy_result->utilization_sum_q16;
    result.worker_ms = legacy_result->worker_ms;
    return legacy_result->ok;
}
...
bool NativeEconomyRuntime::prepare_building_economic_plan(
    ...
    result.reset();
    NativeEconomyBuildingPlanExecutor executor;
    executor.runtime = this;
    executor.legacy_result = &result;
    EconomyBuildingPlanResult kernel_result;
    const int32_t *override_data =
        cells_override != nullptr ? cells_override->data() : nullptr;
    const int32_t override_count = cells_override != nullptr
        ? static_cast<int32_t>(cells_override->size()) : 0;
    return economy_kernel_prepare_building_plan(
        executor, active_begin, active_end, override_data, override_count,
        kernel_result, error);
}

bool NativeEconomyRuntime::prepare_building_economic_plan_body(
        int32_t active_begin, int32_t active_end,
        const std::vector<int32_t> *cells_override,
        BuildingPlanResult &result, std::string &error) {
    result.reset();
    ...
    auto cell_monetary_units_for = [&](int32_t cell) -> int64_t {
        ...
        // or never-observed group therefore has no factual fill yet, but it
        // still needs a bounded nameplate prior so its first owner/employee
        ...
        for (int32_t r = 0; r < type.employee_count; ++r) {
            ...
            const int64_t role_capacity = !observed_capacity ? Q16_ONE
                : role_slots > 0
                ? std::clamp<int64_t>(mul_div_sat(
        ...
        // Keep a full-capacity daily quote separate from the current staffed
        // quote. The latter is used for margin gating; the former is used at
        // the end of the plan to apply the effective execution scale exactly
        ...
        if (observed_capacity) {
            ...
            // the receipt to a full-capacity per-building-day quote here;
            // apply the current effective scale only once below.
            full_capacity_revenue = group.last_observed_capacity_days_q16 > 0
                ? mul_div_sat(observed_receipts, Q16_ONE,
                    std::max<int64_t>(1,
        ...
        if (group.operating_state != 1) {
            ...
            // gating. It may run only far enough to cover the frozen
            // subsistence/clothing floor computed for this owner, while its
            // monetary forecast remains the factual receipt above (possibly
            // zero). Resource, input, climate, and working-capital checks
            // below still constrain what can actually execute.
        ...
        group.last_quoted_market_receipt = group.last_expected_revenue;
        group.last_quoted_operating_cost = mul_div_sat(
            saturating_mul(group_days, operating, _saturation_count),
            effective_execution_q16, Q16_ONE, _saturation_count);
        group.last_maintenance_due = mul_div_sat(
            saturating_mul(group_days, maintenance_cost, _saturation_count),
            effective_execution_q16, Q16_ONE, _saturation_count);
...
void NativeEconomyRuntime::refresh_building_modifier_factors() {
    ...
    for (size_t group_index = 0; group_index < _buildings.size();
        ...
        const int32_t family_owned_factor_q16 =
            family_group_owned_output_factor_q16(static_cast<int32_t>(group_index));
        const size_t cell_sector_index = static_cast<size_t>(
            std::max(0, group.cell)) * 5U + static_cast<size_t>(sector);
        const int32_t cell_sector_factor_q16 =
            group.cell >= 0 && cell_sector_index <
                _epoch_cell_sector_output_factor_q16.size()
            ? _epoch_cell_sector_output_factor_q16[cell_sector_index] : Q16_ONE;
        if (cache.country_factor_q16 == country_factor_q16 &&
            ...
            cache.family_owned_factor_q16 == family_owned_factor_q16 &&
            cache.cell_sector_factor_q16 == cell_sector_factor_q16 &&
        ...
        if (!cell_country_valid) {
            ...
            group.output_factor_q16 = modifier_factor_q16(
                static_cast<double>(family_owned_factor_q16) / Q16_ONE *
                static_cast<double>(cell_sector_factor_q16) / Q16_ONE);
        } else {
            ...
            const double exact_building_factor = exact_building_stat_id >= 0
                ? _modifier_runtime->effective_value(
                    ModifierRuntime::ECONOMY, exact_building_stat_id, 0,
                    static_cast<uint64_t>(group.cell), 1.0)
                : 1.0;
            group.output_factor_q16 = modifier_factor_q16(
                ...
                (static_cast<double>(family_owned_factor_q16) / Q16_ONE) *
                (static_cast<double>(cell_sector_factor_q16) / Q16_ONE) *
                building_factor * exact_building_factor);
        }
...
int64_t NativeEconomyRuntime::effective_building_output_quantity_for_target(
    ...
    if (_modifier_runtime != nullptr) {
        ...
        const int32_t exact_building_stat_id = type_id >= 0 &&
                type_id < static_cast<int32_t>(
                    _city_building_output_stat_ids.size())
            ? _city_building_output_stat_ids[static_cast<size_t>(type_id)] : -1;
        const double exact_building_factor = exact_building_stat_id >= 0
            ? _modifier_runtime->effective_value(
                ModifierRuntime::ECONOMY, exact_building_stat_id, 0,
...
int64_t NativeEconomyRuntime::projected_owner_income_per_day(
    ...
    if (observed_capacity) {
        int64_t workforce_q16 = Q16_ONE;
        const int64_t owner_slots = saturating_mul(
            group.count, type.owner_slots_per_building, sat);
        workforce_q16 = owner_slots > 0
            ? std::clamp<int64_t>(mul_div_sat(
                owner_jobs, Q16_ONE, owner_slots, sat), 0, Q16_ONE)
            : Q16_ONE;
        for (int32_t r = 0; r < type.employee_count; ++r) {
            const JobRole &role = _building_employee_roles[type.employee_begin + r];
            const int64_t role_slots = saturating_mul(
                group.count, role.slots_per_building, sat);
            const int32_t role_index = group.employee_fill_begin + r;
            ...
            const int64_t role_capacity = role_slots > 0
                ? std::clamp<int64_t>(mul_div_sat(
                    filled, Q16_ONE, role_slots, sat), 0, Q16_ONE)
                : Q16_ONE;
            workforce_q16 = std::min(workforce_q16, role_capacity);
        }
        const int64_t current_capacity_days_q16 = saturating_mul(
            workforce_q16,
            saturating_mul(std::max<int64_t>(1, group.count), days, sat), sat);
        // This is a capacity-days multiplier, not a utilization ratio. It may
        // legitimately exceed Q16 when the current period is longer or has
        // more staffed capacity than the observed period.
        fact_scale_q16 = current_capacity_days_q16 > 0
            ? mul_div_sat(current_capacity_days_q16, Q16_ONE,
                std::max<int64_t>(1, group.last_observed_capacity_days_q16), sat)
            : 0;
    }
    auto scale_fact = [&](int64_t value) -> int64_t {
        return observed_capacity
            ? mul_div_sat(std::max<int64_t>(0, value), fact_scale_q16,
                          Q16_ONE, sat)
    ...
    int64_t wage_cost = 0;
    if (observed_capacity) {
        // These are the previous committed period's actual cash outflows.
        // They remain authoritative even when the next plan is forced to a
        // lower utilization by a newly scarce input or resource.
        input_cost = scale_fact(group.last_input_cost);
        maintenance_cost = scale_fact(group.last_maintenance_cost);
        wage_cost = scale_fact(group.last_wages_paid);
    } else {
        const int64_t utilization = std::clamp<int64_t>(
            group.planned_utilization_q16, 0, Q16_ONE);
        if (utilization <= 0) return 0;
        input_cost = saturating_mul(
            saturating_mul(group.sample_unit_input_cost, group.count,
                           sat),
            days, sat);
        input_cost = mul_div_sat(input_cost, utilization, Q16_ONE, sat);
        maintenance_cost = saturating_mul(
        ...
        for (int32_t r = 0; r < type.employee_count; ++r) {
            ...
            int64_t role_cost = saturating_mul(
                saturating_mul(filled, wage, sat), days, sat);
            role_cost = mul_div_sat(role_cost, utilization, Q16_ONE, sat);
    ...
    // Once a group has a recorded workforce-capacity day, the prior period's
    // receipts are the factual anchor. A zero receipt is meaningful and must
    // not fall back to a stale/nameplate quote; only never-observed groups use
    // the frozen greenfield quote.
    const int64_t quoted_revenue = observed_capacity
        ? observed_receipts
        : (group.last_quoted_market_receipt > 0
            ? group.last_quoted_market_receipt : group.last_expected_revenue);
    ...
    if ((_epoch_active_tax_mask & owner_tax_mask) == 0) {
        const int64_t owner_pool = std::max<int64_t>(0,
            saturating_sub(quoted_revenue,
                saturating_add(saturating_add(
                    saturating_add(input_cost, wage_cost, sat),
                    maintenance_cost, sat), owner_living_cost, sat), sat));
...
bool NativeEconomyRuntime::should_run(int64_t day_index) const {
    ...
    // When Country is worker-authoritative, sync should_run is not a live day
    // owner — waiting on it starves economy forever (runtime graph skips the
    // country slice). Peer asset traffic still flows via the simulation host.
    if (!_epoch_active && _country_runtime != nullptr &&
        _country_runtime->should_run(day_index)) {
        const bool country_on_worker =
            _country_runtime->sync_store_writes_forbidden() ||
            (_simulation_host != nullptr &&
             (_simulation_host->domain_is_worker_authoritative(
                  RuntimeDomainId::COUNTRY) ||
              _simulation_host->country_authority_owner_is_worker()));
        if (!country_on_worker) return false;
    }
...
bool NativeEconomyRuntime::apply_family_population_reward(
    ...
    if (cmd.i32_0 == -1) {
        ...
        if (family >= 0 && family < static_cast<int32_t>(
                _family_colonization_population_reward.size()))
            _family_colonization_population_reward[static_cast<size_t>(family)] =
                static_cast<int32_t>(std::clamp<int64_t>(amount, 0,
                    std::numeric_limits<int32_t>::max()));
        return true;
    }
...
bool NativeEconomyRuntime::apply_family_absorb_anonymous(
        const Command &cmd, std::string &error) {
    (void)error;
    int32_t branch = -1;
    if (!_family_influences.valid_handle(cmd.target_handle, branch)) {
    ...
    if (cmd.i32_0 == 1) {
        if (family >= 0 && family < static_cast<int32_t>(
            ...
            _family_absorb_bonus_q16[static_cast<size_t>(family)] =
                static_cast<int32_t>(std::clamp<int64_t>(cmd.i64_0, 0,
                    4 * Q16_ONE));
        return true;
    }
    int64_t remaining = std::max<int64_t>(0, cmd.i64_0);
    ...
    _population.for_each_in_cell(cell, [&](int32_t slot) {
        ...
        const int64_t available = std::max<int64_t>(0,
            _population.population[slot] - claimed);
        const int64_t take = std::min(remaining, available);
        if (take <= 0) return;
        add_family_household_people(family_handle, slot, take);
        remaining -= take;
    });
    return true;
}

bool NativeEconomyRuntime::apply_family_purchase_discount(
        const Command &cmd, std::string &error) {
    (void)error;
    int32_t branch = -1;
    if (!_family_influences.valid_handle(cmd.target_handle, branch)) {
    ...
    ensure_family_policy_factors();
    if (family >= 0 && family < static_cast<int32_t>(
            _family_purchase_factor_q16.size()))
        _family_purchase_factor_q16[static_cast<size_t>(family)] =
            static_cast<int32_t>(std::clamp<int64_t>(cmd.i64_0, 0, Q16_ONE));
    return true;
}
...
void NativeEconomyRuntime::apply_family_colonization_population_reward(
    ...
    int32_t family = -1;
    if (!_families.valid_handle(family_handle, family)) return;
    Command reward;
    reward.opcode = COMMAND_FAMILY_POPULATION_REWARD;
    reward.i32_0 = 0;
    reward.i64_0 = amount;
    reward.sequence = 0;
    for (int32_t branch = 0; branch < static_cast<int32_t>(
        ...
        if (_family_influences.active[branch] == 0 ||
            ...
            _family_influences.cell[branch] != destination) continue;
        reward.target_handle = _family_influences.handle_for_index(branch);
        std::string ignored;
        apply_family_population_reward(reward, ignored);
        return;
    }
    int32_t target_slot = -1;
    int64_t best_people = -1;
    _population.for_each_in_cell(destination, [&](int32_t slot) {
        int64_t attributed = 0;
        const uint64_t cohort = _population.handle_for_slot(slot);
        for (const FamilyMembershipEdge &edge : _family_memberships) {
            if (edge.cohort_handle == cohort)
                attributed += std::max<int64_t>(0, edge.people);
        }
        const int64_t anonymous = std::max<int64_t>(0,
            _population.population[slot] - attributed);
        if (anonymous > best_people || (anonymous == best_people &&
    ...
    if (target_slot < 0) {
        ...
        target_slot = _population.allocate_slot(destination,
            static_cast<uint32_t>(signature));
    ...
    _external_population_delta = saturating_add(
        _external_population_delta, actual, _saturation_count);
    _structural_touched_cells.push_back(destination);
    auto membership = std::find_if(_family_memberships.begin(),
        _family_memberships.end(), [&](const FamilyMembershipEdge &edge) {
            return edge.family_handle == family_handle &&
                edge.cohort_handle == cohort_handle;
    ...
    _family_indices_dirty = true;
    std::vector<EventLeg> legs;
    if (trace_detail_for_cell(destination)) {
        legs.push_back({FIELD_COHORT_POPULATION, SUBJECT_COHORT,
            static_cast<int64_t>(cohort_handle), -1, before,
            _population.population[target_slot]});
    }
    trace_append(EVENT_POPULATION_SOURCE,
        static_cast<int32_t>(Stage::LEDGER_APPLY), destination,
        SUBJECT_FAMILY_BRANCH, static_cast<int64_t>(family_handle),
        0, COMMAND_FAMILY_POPULATION_REWARD, actual, before,
        _population.population[target_slot], 0,
        legs.empty() ? nullptr : &legs);
}
...
bool NativeEconomyRuntime::apply_command(const Command &cmd, std::string &error) {
    ...
    switch (cmd.opcode) {
        ...
        case COMMAND_TRANSFER_TO_COHORT: {
            ...
            int64_t amount = 0;
            std::string cohort_cash_error;
            if (!coordinate_country_cohort_cash(
                    slot, country_handle,
                    NativeCountryRuntime::ECONOMY_ASSET_CASH_TO_COHORT,
                    std::max<int64_t>(0, cmd.i64_0), amount, cohort_cash_error)) {
                error = cohort_cash_error.empty()
                    ? "country_cohort_cash_transfer_failed" : cohort_cash_error;
                return false;
            }
        ...
        case COMMAND_TRANSFER_FROM_COHORT: {
            ...
            if (!coordinate_country_cohort_cash(
                    slot, country_handle,
                    NativeCountryRuntime::ECONOMY_ASSET_CASH_FROM_COHORT,
                    offered, amount, cohort_cash_error)) {
                error = cohort_cash_error.empty()
                    ? "country_cohort_cash_transfer_failed" : cohort_cash_error;
                return false;
            }
        ...
        case COMMAND_FAMILY_ABSORB_ANONYMOUS:
            return apply_family_absorb_anonymous(cmd, error);
        case COMMAND_FAMILY_PURCHASE_DISCOUNT:
            return apply_family_purchase_discount(cmd, error);
        case COMMAND_FAMILY_SET_SPLIT_POLICY:
            return apply_family_set_split_policy(cmd, error);
...
bool NativeEconomyRuntime::commit_structural(const StructuralCommand &cmd,
    ...
    if (cmd.opcode == STRUCTURAL_REMOVE_EMPTY) {
        ...
        std::string cohort_cash_error;
        if (!coordinate_country_cohort_cash(
                source, country_handle,
                NativeCountryRuntime::ECONOMY_ASSET_CASH_FROM_COHORT,
                estate_funds, moved, cohort_cash_error)) {
            error = cohort_cash_error.empty()
                ? "country_treasury_estate_transfer_failed" : cohort_cash_error;
            return false;
        }
        if (moved != estate_funds) {
            // Country unique-writer + closed M2 cash peer: keep the empty
            // cohort until a later day can transfer the estate. Do not burn
            // money and do not release the slot.
            if (moved == 0 &&
                cohort_cash_error == "country_economy_operation_gate_closed")
                return true;
...
bool NativeEconomyRuntime::move_cohort_population(int32_t source, int32_t dest_cell,
    ...
    if (_population.population[source] == 0) {
        ...
        if (!coordinate_country_cohort_cash(
                source, country_handle,
                NativeCountryRuntime::ECONOMY_ASSET_CASH_FROM_COHORT,
                residue_funds, moved, cohort_cash_error)) {
            error = cohort_cash_error.empty()
                ? "country_treasury_residue_transfer_failed" : cohort_cash_error;
            return false;
        }
        if (moved != residue_funds) {
            // Same gate soft-unavailable path as STRUCTURAL_REMOVE_EMPTY:
            // keep residual funds on the drained slot until Country peer opens.
            if (moved == 0 &&
                cohort_cash_error == "country_economy_operation_gate_closed") {
                if (source_drained_out != nullptr) *source_drained_out = false;
                return true;
...
bool NativeEconomyRuntime::worker_run_compact_slice(int64_t day_index,
                                                     std::string &error,
                                                     bool *done,
                                                     bool *pending_input) {
    error.clear();
    if (done != nullptr) *done = false;
    if (pending_input != nullptr) *pending_input = false;
    Dictionary ctx;
    ctx["day_index"] = day_index;
    const Dictionary result = run_slice_compact(ctx);
    const bool fatal = static_cast<bool>(result.get("fatal", false));
    const bool slice_done = static_cast<bool>(result.get("done", false));
    const bool pending = static_cast<bool>(result.get("pending_input", false));
    if (done != nullptr) *done = slice_done;
    if (pending_input != nullptr) *pending_input = pending;
    if (pending) {
        // Soft input barrier: main thread has not yet captured same-day
        // environment/building context. Not a ledger fatal — Host parks day.
        error = String(result.get("yield_reason", "pending_input"))
                    .utf8()
                    .get_data();
        if (error.empty()) error = "pending_input";
        return true;
    }
    if (fatal) {
        error = String(result.get("fatal_reason", "economy_worker_fatal"))
                    .utf8()
                    .get_data();
        if (error.empty()) error = "economy_worker_fatal";
        return false;
    }
    return true;
}
...
Dictionary NativeEconomyRuntime::run_slice_internal(const Dictionary &ctx, bool compact) {
    ...
    if (!_epoch_active &&
        ...
        if (!start_epoch(day_index, error)) {
            // Soft input barrier: missing same-day env/building capture is a
            // main-thread scheduling gap under worker authority, not a ledger
            // fatal. Park the slice until capture_economy_day_inputs runs.
    ...
    while (_epoch_active && !_fatal && !slice_budget_exhausted()) {
        if (_stage == Stage::BUILDING_PLAN) {
            ...
            if (evaluate_phase) {
                ...
                publish_shadow_graph_stage_reference(
    ...
    _price_numeric_floor_hits = saturating_add(_price_numeric_floor_hits,
        ...
        if (_stage == Stage::GOVERNMENT_RESEARCH_PROCUREMENT) {
            ...
            // procurement state.
            if (!_country_research_procurement_done) {
                if (finish_chunk_and_should_yield()) break;
                continue;
            }
        ...
        for (size_t i = 0; i < _family_expeditions.active.size(); ++i) {
            if (_family_expeditions.active[i] != 0 &&
                ...
                total = saturating_add(total,
                    family_expedition_payload_people(static_cast<int32_t>(i)),
                    sat);
        }
        return total;
    };
...
bool NativeEconomyRuntime::rebuild_family_behavior_cache() {
    ...
    for (int32_t branch = 0; branch < static_cast<int32_t>(
        ...
        int32_t family_index = -1;
        if (!_families.valid_handle(_family_influences.family_handle[branch],
                family_index) || family_index < 0 ||
    ...
    for (const FamilyTraitRoll &roll : _family_traits) {
        ...
        if (!_families.valid_handle(roll.family_handle, family_index) ||
            roll.trait_id < 0 || roll.trait_id >= static_cast<int32_t>(
                _family_trait_behavior_offsets.size()) - 1)
            continue;
        for (int32_t edge = _family_trait_behavior_offsets[roll.trait_id];
            ...
            const int32_t factor = static_cast<int32_t>(
                std::clamp<int64_t>(scaled, 0, 4 * Q16_ONE));
            const int32_t score_term = edge < static_cast<int32_t>(
                    _family_trait_behavior_score_terms.size())
                ? _family_trait_behavior_score_terms[edge] : 0;
            FamilyBehaviorFactorRow base;
            base.score_term = score_term;
            base.axis = _family_trait_behavior_axes[edge];
            base.selector_kind = _family_trait_behavior_selector_kinds[edge];
            base.selector_id = _family_trait_behavior_selector_ids[edge];
            base.factor_q16 = factor;
            const bool has_conditions =
                edge + 1 < static_cast<int32_t>(
                    _family_trait_behavior_condition_offsets.size()) &&
                _family_trait_behavior_condition_offsets[edge] <
                    _family_trait_behavior_condition_offsets[
    ...
    for (const FamilyEffectBinding &binding : _family_effect_bindings) {
        if (binding.definition_key != "family.effect.family_learning") continue;
        int32_t branch = -1;
        int32_t family_index = -1;
        if (!_family_influences.valid_handle(binding.branch_handle, branch) ||
            !_families.valid_handle(_family_influences.family_handle[branch],
                family_index))
            continue;
    ...
    std::sort(expanded.begin(), expanded.end(), [](const TaggedRow &a,
            const TaggedRow &b) {
        return std::tie(a.family_index, a.row.cell, a.row.score_term, a.row.axis,
                        a.row.selector_kind, a.row.selector_id) <
               std::tie(b.family_index, b.row.cell, b.row.score_term, b.row.axis,
                        b.row.selector_kind, b.row.selector_id);
    });
    std::vector<TaggedRow> merged;
    merged.reserve(expanded.size());
    for (const TaggedRow &entry : expanded) {
        if (!merged.empty() &&
            merged.back().family_index == entry.family_index &&
            merged.back().row.cell == entry.row.cell &&
            merged.back().row.score_term == entry.row.score_term &&
            merged.back().row.axis == entry.row.axis &&
            merged.back().row.selector_kind == entry.row.selector_kind &&
            merged.back().row.selector_id == entry.row.selector_id) {
            const int64_t combined = static_cast<int64_t>(
                merged.back().row.factor_q16) * entry.row.factor_q16 / Q16_ONE;
            merged.back().row.factor_q16 = static_cast<int32_t>(
                std::clamp<int64_t>(combined, 0, 4 * Q16_ONE));
        } else {
            merged.push_back(entry);
        }
    }
    _family_behavior_factor_offsets.assign(_families.active.size() + 1, 0);
    for (const TaggedRow &entry : merged) {
        if (entry.family_index >= 0 && entry.family_index <
            static_cast<int32_t>(_families.active.size()))
            ++_family_behavior_factor_offsets[static_cast<size_t>(entry.family_index) + 1];
    }
    for (size_t i = 0; i + 1 < _family_behavior_factor_offsets.size(); ++i)
        _family_behavior_factor_offsets[i + 1] += _family_behavior_factor_offsets[i];
...
void NativeEconomyRuntime::ensure_family_policy_factors() {
    const size_t n = _families.active.size();
    if (_family_purchase_factor_q16.size() < n)
        _family_purchase_factor_q16.resize(n, Q16_ONE);
    if (_family_investment_factor_q16.size() < n)
        _family_investment_factor_q16.resize(n, Q16_ONE);
    if (_family_birth_factor_q16.size() < n)
        _family_birth_factor_q16.resize(n, Q16_ONE);
    if (_family_absorb_bonus_q16.size() < n)
        _family_absorb_bonus_q16.resize(n, 0);
    if (_family_colonization_population_reward.size() < n)
        _family_colonization_population_reward.resize(n, 0);
}

void NativeEconomyRuntime::reset_family_policy_factors(int32_t family_index) {
    ensure_family_policy_factors();
    if (family_index < 0 || family_index >= static_cast<int32_t>(
            _family_purchase_factor_q16.size()))
        return;
    _family_purchase_factor_q16[static_cast<size_t>(family_index)] = Q16_ONE;
    if (family_index < static_cast<int32_t>(_family_investment_factor_q16.size()))
        _family_investment_factor_q16[static_cast<size_t>(family_index)] = Q16_ONE;
    if (family_index < static_cast<int32_t>(_family_birth_factor_q16.size()))
        _family_birth_factor_q16[static_cast<size_t>(family_index)] = Q16_ONE;
    if (family_index < static_cast<int32_t>(_family_absorb_bonus_q16.size()))
        _family_absorb_bonus_q16[static_cast<size_t>(family_index)] = 0;
    if (family_index < static_cast<int32_t>(
            _family_colonization_population_reward.size()))
        _family_colonization_population_reward[static_cast<size_t>(family_index)] = 0;
}

int32_t NativeEconomyRuntime::family_colonization_population_reward_amount(
        uint64_t family_handle) const {
    int32_t family = -1;
    if (!_families.valid_handle(family_handle, family) || family < 0 ||
        family >= static_cast<int32_t>(_family_colonization_population_reward.size()))
        return 0;
    return std::max(0, _family_colonization_population_reward[
        static_cast<size_t>(family)]);
}

bool NativeEconomyRuntime::family_trait_technology_unlocked(
        int32_t trait_id, int32_t cell) const {
    if (trait_id < 0 || trait_id >= static_cast<int32_t>(
            _family_trait_technology_prerequisite_offsets.size()) - 1)
        return true;
    const int32_t begin = _family_trait_technology_prerequisite_offsets[
        static_cast<size_t>(trait_id)];
    const int32_t end = _family_trait_technology_prerequisite_offsets[
        static_cast<size_t>(trait_id) + 1];
    if (begin >= end)
        return true;
    const uint8_t match_any =
        trait_id < static_cast<int32_t>(_family_trait_technology_match_any.size())
            ? _family_trait_technology_match_any[static_cast<size_t>(trait_id)]
            : 0;
    if (match_any != 0)
        return cell_has_requirements(cell, begin, end,
            _family_trait_technology_prerequisites, true);
    return cell_has_all_requirements(cell, begin, end,
        _family_trait_technology_prerequisites, true);
}

bool NativeEconomyRuntime::family_effect_technology_unlocked(
        int32_t effect_id, int32_t cell) const {
    if (effect_id < 0 || effect_id >= static_cast<int32_t>(
            _family_effect_prerequisite_offsets.size()) - 1)
        return true;
    const int32_t begin = _family_effect_prerequisite_offsets[
        static_cast<size_t>(effect_id)];
    const int32_t end = _family_effect_prerequisite_offsets[
        static_cast<size_t>(effect_id) + 1];
    if (begin >= end)
        return true;
    const uint8_t match_any =
        effect_id < static_cast<int32_t>(_family_effect_technology_match_any.size())
            ? _family_effect_technology_match_any[static_cast<size_t>(effect_id)]
            : 0;
    if (match_any != 0)
        return cell_has_requirements(cell, begin, end,
            _family_effect_prerequisites, true);
    return cell_has_all_requirements(cell, begin, end,
        _family_effect_prerequisites, true);
}

bool NativeEconomyRuntime::family_trait_origin_gates_allow(
        int32_t trait_id, int32_t origin_cell) const {
    if (trait_id < 0) return true;
    const size_t count = _family_trait_ids.size();
    if (static_cast<size_t>(trait_id) >= count) return true;
    auto needs_place = [&]() -> bool {
        const bool landform = trait_id + 1 < static_cast<int32_t>(
                _family_trait_origin_landform_offsets.size()) &&
            _family_trait_origin_landform_offsets[trait_id] <
                _family_trait_origin_landform_offsets[trait_id + 1];
        const bool water = trait_id < static_cast<int32_t>(
                _family_trait_origin_adjacent_water.size()) &&
            _family_trait_origin_adjacent_water[trait_id] != 0;
        const bool population = trait_id < static_cast<int32_t>(
                _family_trait_origin_population_max.size()) &&
            _family_trait_origin_population_max[trait_id] > 0;
        const bool temperature = trait_id < static_cast<int32_t>(
                _family_trait_origin_temperature_max_q16.size()) &&
            _family_trait_origin_temperature_max_q16[trait_id] >= 0;
        const bool resource = trait_id + 1 < static_cast<int32_t>(
                _family_trait_required_resource_offsets.size()) &&
            _family_trait_required_resource_offsets[trait_id] <
                _family_trait_required_resource_offsets[trait_id + 1];
        const bool tax = trait_id < static_cast<int32_t>(
                _family_trait_require_tax_or_subsidy.size()) &&
    ...
    if (trait_id + 1 < static_cast<int32_t>(
            _family_trait_origin_landform_offsets.size())) {
        const int32_t begin = _family_trait_origin_landform_offsets[trait_id];
        const int32_t end = _family_trait_origin_landform_offsets[trait_id + 1];
        if (begin < end) {
            const int32_t landform = origin_cell < static_cast<int32_t>(
                    _building_landform.size())
                ? _building_landform[static_cast<size_t>(origin_cell)] : -1;
            bool matched = false;
            for (int32_t i = begin; i < end; ++i) {
                if (static_cast<int32_t>(_family_trait_origin_landforms[
                        static_cast<size_t>(i)]) == landform) {
                    matched = true;
                    break;
                }
            }
    ...
    if (trait_id < static_cast<int32_t>(
            _family_trait_origin_adjacent_water.size()) &&
        _family_trait_origin_adjacent_water[trait_id] != 0) {
        constexpr int32_t kLandformDelta = 9;
        const int32_t landform = origin_cell < static_cast<int32_t>(
                _building_landform.size())
            ? _building_landform[static_cast<size_t>(origin_cell)] : -1;
    ...
    if (trait_id < static_cast<int32_t>(
            _family_trait_origin_population_max.size()) &&
        _family_trait_origin_population_max[trait_id] > 0) {
        const int64_t population = origin_cell < static_cast<int32_t>(
                _committed_cells.size())
            ? _committed_cells[static_cast<size_t>(origin_cell)].population : 0;
        if (population > _family_trait_origin_population_max[trait_id])
            return false;
    }
    if (trait_id < static_cast<int32_t>(
            _family_trait_origin_temperature_max_q16.size()) &&
        _family_trait_origin_temperature_max_q16[trait_id] >= 0) {
        const int32_t temperature = origin_cell < static_cast<int32_t>(
                _environment_temperature_q16.size())
            ? _environment_temperature_q16[static_cast<size_t>(origin_cell)]
            : Q16_ONE;
        if (temperature > _family_trait_origin_temperature_max_q16[trait_id])
            return false;
    }
    if (trait_id + 1 < static_cast<int32_t>(
            _family_trait_required_resource_offsets.size())) {
        const int32_t begin = _family_trait_required_resource_offsets[trait_id];
        const int32_t end = _family_trait_required_resource_offsets[trait_id + 1];
        if (begin < end) {
            bool found = false;
            for (int32_t i = begin; i < end; ++i) {
                const int32_t resource_id = _family_trait_required_resource_ids[
                    static_cast<size_t>(i)];
                if (carrying_resource_stock(resource_id, origin_cell) > 0) {
                    found = true;
                    break;
                }
                ...
                if (resource_id >= 0 && idx < _resource_snapshot.size() &&
                    _resource_snapshot[idx] > 0) {
                    found = true;
                    break;
                }
            }
    ...
    if (trait_id < static_cast<int32_t>(
            _family_trait_require_tax_or_subsidy.size()) &&
        _family_trait_require_tax_or_subsidy[trait_id] != 0) {
        const bool cell_tax = origin_cell < static_cast<int32_t>(
                _epoch_cell_active_tax_mask.size()) &&
        ...
        if (!cell_tax && !country_tax) return false;
    }
    return true;
}

bool NativeEconomyRuntime::family_trait_context_allows(
        int32_t trait_id, int32_t family_index) const {
    if (family_index < 0 || family_index >= static_cast<int32_t>(
            _families.active.size()) || _families.active[family_index] == 0)
        return false;
...
void NativeEconomyRuntime::fill_family_behavior_metrics(
        int32_t family_index, int32_t branch, int32_t cell, int64_t *metrics,
        int32_t metric_count) const {
    if (metrics == nullptr || metric_count <= 0) return;
    std::fill(metrics, metrics + metric_count, 0);
    auto count_q16 = [](int64_t value) -> int64_t {
        if (value <= 0) return 0;
        if (value > std::numeric_limits<int64_t>::max() / Q16_ONE)
    ...
    if (family_index >= 0 && family_index < static_cast<int32_t>(
            _families.active.size()) && _families.active[family_index] != 0) {
        family_handle = _families.handle_for_index(family_index);
        if (metric_count > FAMILY_METRIC_MAGNITUDE_Q16)
            metrics[FAMILY_METRIC_MAGNITUDE_Q16] = Q16_ONE;
        if (metric_count > FAMILY_METRIC_FAMILY_POPULATION)
            metrics[FAMILY_METRIC_FAMILY_POPULATION] =
                count_q16(family_population(family_handle));
        if (metric_count > FAMILY_METRIC_FAMILY_CASH_CLAIM)
            metrics[FAMILY_METRIC_FAMILY_CASH_CLAIM] =
                count_q16(family_cash_claim(family_handle));
    }
    if (branch >= 0 && branch < static_cast<int32_t>(
            _family_influences.active.size()) &&
        _family_influences.active[branch] != 0) {
        if (metric_count > FAMILY_METRIC_BRANCH_PRESTIGE_Q16)
            metrics[FAMILY_METRIC_BRANCH_PRESTIGE_Q16] =
                _family_influences.score_q16[branch];
        if (metric_count > FAMILY_METRIC_BRANCH_POPULATION)
            metrics[FAMILY_METRIC_BRANCH_POPULATION] =
                count_q16(_family_influences.population[branch]);
        if (metric_count > FAMILY_METRIC_BRANCH_IS_LOCAL_PRESTIGE_MAX) {
            int32_t best_score = _family_influences.score_q16[branch];
            uint64_t best_handle = family_handle;
            const int32_t local_cell = _family_influences.cell[branch];
            for (int32_t i = 0; i < static_cast<int32_t>(
                    _family_influences.active.size()); ++i) {
                if (_family_influences.active[i] == 0 ||
                    _family_influences.cell[i] != local_cell) continue;
                const int32_t score = _family_influences.score_q16[i];
                const uint64_t handle = _family_influences.family_handle[i];
            ...
            metrics[FAMILY_METRIC_BRANCH_IS_LOCAL_PRESTIGE_MAX] =
                (family_handle != 0 && family_handle == best_handle)
                    ? Q16_ONE : 0;
        }
    }
    if (cell < 0 || cell >= _cell_count) return;
    if (metric_count > FAMILY_METRIC_CELL_TEMPERATURE_Q16)
        metrics[FAMILY_METRIC_CELL_TEMPERATURE_Q16] =
            cell < static_cast<int32_t>(_environment_temperature_q16.size())
                ? _environment_temperature_q16[cell] : 0;
    if (metric_count > FAMILY_METRIC_CELL_PRECIPITATION_Q16)
        metrics[FAMILY_METRIC_CELL_PRECIPITATION_Q16] =
            cell < static_cast<int32_t>(_environment_precipitation_q16.size())
                ? _environment_precipitation_q16[cell] : 0;
    if (metric_count > FAMILY_METRIC_CELL_RESOURCE_SHORTAGE_Q16)
        metrics[FAMILY_METRIC_CELL_RESOURCE_SHORTAGE_Q16] =
            cell < static_cast<int32_t>(_cell_effect_shortage_q16.size())
                ? std::clamp<int64_t>(_cell_effect_shortage_q16[cell], 0, Q16_ONE)
                : 0;
    if (metric_count > FAMILY_METRIC_CELL_TRADE_EVENTS)
        metrics[FAMILY_METRIC_CELL_TRADE_EVENTS] =
            cell < static_cast<int32_t>(_cell_trade_gen.size())
                ? count_q16(static_cast<int64_t>(_cell_trade_gen[cell])) : 0;
    if (metric_count > FAMILY_METRIC_CELL_POPULATION)
        metrics[FAMILY_METRIC_CELL_POPULATION] =
            cell < static_cast<int32_t>(_committed_cells.size())
                ? count_q16(_committed_cells[cell].population) : 0;
    if (metric_count > FAMILY_METRIC_CELL_LANDFORM) {
        const int32_t landform = cell < static_cast<int32_t>(
                _building_landform.size()) ? _building_landform[cell] : 0;
        metrics[FAMILY_METRIC_CELL_LANDFORM] = count_q16(landform);
    }
    if (metric_count > FAMILY_METRIC_CELL_ESSENTIALS_SHORTAGE_Q16)
        metrics[FAMILY_METRIC_CELL_ESSENTIALS_SHORTAGE_Q16] =
            cell < static_cast<int32_t>(_cell_essentials_shortage_q16.size())
                ? std::clamp<int64_t>(_cell_essentials_shortage_q16[cell], 0,
                    Q16_ONE) : 0;
    if (metric_count > FAMILY_METRIC_CELL_RAIN_EVENT)
        metrics[FAMILY_METRIC_CELL_RAIN_EVENT] =
            cell < static_cast<int32_t>(_cell_rain_event_q16.size())
                ? _cell_rain_event_q16[cell] : 0;
    if (metric_count > FAMILY_METRIC_CELL_RESOURCE_ABUNDANCE_Q16)
        metrics[FAMILY_METRIC_CELL_RESOURCE_ABUNDANCE_Q16] =
            cell < static_cast<int32_t>(_cell_resource_abundance_q16.size())
                ? std::clamp<int64_t>(_cell_resource_abundance_q16[cell], 0,
                    Q16_ONE) : 0;
    const FamilyIndustryStats *stats = family_industry_stats_for(family_index, cell);
    if (metric_count > FAMILY_METRIC_HAS_OWNED_MANUFACTURING)
        metrics[FAMILY_METRIC_HAS_OWNED_MANUFACTURING] =
            stats != nullptr && stats->has_owned_manufacturing != 0 ? Q16_ONE : 0;
    if (metric_count > FAMILY_METRIC_DISTINCT_SECTOR_COUNT)
        metrics[FAMILY_METRIC_DISTINCT_SECTOR_COUNT] =
            stats != nullptr ? count_q16(stats->distinct_sector_count) : 0;
    if (metric_count > FAMILY_METRIC_DOMINANT_SECTOR_ID)
        metrics[FAMILY_METRIC_DOMINANT_SECTOR_ID] =
            stats == nullptr || stats->dominant_sector_id < 0
                ? -Q16_ONE : count_q16(stats->dominant_sector_id);
    if (metric_count > FAMILY_METRIC_DOMINANT_SECTOR_SHARE_Q16)
        metrics[FAMILY_METRIC_DOMINANT_SECTOR_SHARE_Q16] =
            stats != nullptr ? stats->dominant_sector_share_q16 : 0;
    if (metric_count > FAMILY_METRIC_COMPLETE_CHAIN_COUNT)
        metrics[FAMILY_METRIC_COMPLETE_CHAIN_COUNT] =
            stats != nullptr ? count_q16(stats->complete_chain_count) : 0;
    if (metric_count > FAMILY_METRIC_MAX_LOCAL_CHAIN_SHARE_Q16)
        metrics[FAMILY_METRIC_MAX_LOCAL_CHAIN_SHARE_Q16] =
            stats != nullptr ? stats->max_local_chain_share_q16 : 0;
    if (metric_count > FAMILY_METRIC_MAX_CHAIN_UPGRADE_FAMILY_ID)
        metrics[FAMILY_METRIC_MAX_CHAIN_UPGRADE_FAMILY_ID] =
            stats == nullptr || stats->max_chain_upgrade_family_id < 0
                ? 0 : count_q16(stats->max_chain_upgrade_family_id);
    int64_t cell_population = 0;
    int64_t cell_employed = 0;
    int64_t cell_funds = 0;
    std::vector<uint8_t> employed_professions;
    if (metric_count > FAMILY_METRIC_CELL_UNEMPLOYMENT_Q16 ||
        metric_count > FAMILY_METRIC_CELL_VACANT_PROFESSION_COUNT ||
        metric_count > FAMILY_METRIC_FAMILY_CASH_PER_CAPITA_VS_CELL_Q16) {
        employed_professions.assign(_profession_ids.size(), 0);
        _population.for_each_in_cell(cell, [&](int32_t slot) {
            const int64_t people = std::max<int64_t>(0, _population.population[slot]);
            cell_population += people;
            const int64_t employed = std::max<int64_t>(0,
                _population.owner_employed[slot]) +
                std::max<int64_t>(0, _population.employee_employed[slot]);
            cell_employed += employed;
            cell_funds += std::max<int64_t>(0, _population.funds[slot]);
            const int32_t signature = static_cast<int32_t>(
                _population.signature_id[slot]);
            if (employed > 0 && signature >= 0 &&
                signature < static_cast<int32_t>(_signatures.size())) {
                const int32_t profession = _signatures[
                    static_cast<size_t>(signature)].profession_id;
                if (profession >= 0 &&
                    static_cast<size_t>(profession) < employed_professions.size())
                    employed_professions[static_cast<size_t>(profession)] = 1;
            }
        });
    }
    if (metric_count > FAMILY_METRIC_CELL_UNEMPLOYMENT_Q16) {
        const int64_t unemployed = std::max<int64_t>(0, cell_population - cell_employed);
        metrics[FAMILY_METRIC_CELL_UNEMPLOYMENT_Q16] = cell_population > 0
            ? std::clamp<int64_t>(unemployed * Q16_ONE / cell_population, 0, Q16_ONE)
            : 0;
    }
    if (metric_count > FAMILY_METRIC_CELL_RESOURCE_CLASS_COUNT) {
        int32_t resource_classes = 0;
        const size_t resources = _resource_ids.size();
        if (resources > 0 && _resource_snapshot.size() >=
            ...
            for (size_t resource = 0; resource < resources; ++resource) {
                const size_t idx = resource * static_cast<size_t>(_cell_count) +
                    static_cast<size_t>(cell);
                if (_resource_snapshot[idx] > 0) ++resource_classes;
            }
        }
        metrics[FAMILY_METRIC_CELL_RESOURCE_CLASS_COUNT] = count_q16(resource_classes);
    }
    int64_t sector_count[5] = {0, 0, 0, 0, 0};
    int64_t total_buildings = 0;
    int32_t manufacturing_groups = 0;
    int32_t knowledge_groups = 0;
    int32_t extractive_groups = 0;
    bool can_produce_corn = false;
    ...
    if (building_csr) {
        for (int32_t group_index = _building_cell_offsets[cell];
            ...
            if (group.count <= 0 || group.type_id < 0 ||
                group.type_id >= static_cast<int32_t>(_building_types.size()))
                continue;
            const int32_t sector = group.type_id < static_cast<int32_t>(
                    _building_economic_sectors.size())
                ? _building_economic_sectors[static_cast<size_t>(group.type_id)] : 2;
            if (sector >= 0 && sector < 5) {
                sector_count[sector] += group.count;
                total_buildings += group.count;
            }
            if (sector == 2) ++manufacturing_groups;
            if (sector == 4) ++knowledge_groups;
            if (sector == 1) ++extractive_groups;
            const BuildingType &type = _building_types[
                static_cast<size_t>(group.type_id)];
            if (_family_corn_good_id >= 0) {
                for (int32_t output = type.output_begin;
                     output < type.output_begin + type.output_count; ++output) {
                    if (_building_outputs[static_cast<size_t>(output)].good_id ==
                            _family_corn_good_id) {
                        can_produce_corn = true;
                        break;
                    }
                }
            }
            for (int32_t edge = type.labor_signal_begin;
                 edge < type.labor_signal_begin + type.labor_signal_count; ++edge) {
                if (edge < 0 || edge >= static_cast<int32_t>(
                        _building_type_labor_signal_professions.size()))
                    continue;
                const int32_t profession = _building_type_labor_signal_professions[
                    static_cast<size_t>(edge)];
                if (profession >= 0 &&
                    static_cast<size_t>(profession) < demanded_professions.size())
                    demanded_professions[static_cast<size_t>(profession)] = 1;
            }
        }
    }
    for (size_t profession = 0; profession < demanded_professions.size(); ++profession) {
        if (demanded_professions[profession] != 0 &&
            (profession >= employed_professions.size() ||
             employed_professions[profession] == 0))
            ++vacant_professions;
    }
    if (metric_count > FAMILY_METRIC_CELL_MANUFACTURING_BUILDING_COUNT)
        metrics[FAMILY_METRIC_CELL_MANUFACTURING_BUILDING_COUNT] =
            count_q16(manufacturing_groups);
    int32_t distinct_sectors = 0;
    int32_t dominant_sector = -1;
    int64_t dominant_owned = 0;
    for (int32_t sector = 0; sector < 5; ++sector) {
        if (sector_count[sector] <= 0) continue;
        ++distinct_sectors;
        if (sector_count[sector] > dominant_owned ||
            (sector_count[sector] == dominant_owned &&
             (dominant_sector < 0 || sector < dominant_sector))) {
            dominant_owned = sector_count[sector];
            dominant_sector = sector;
        }
    }
    if (metric_count > FAMILY_METRIC_CELL_DISTINCT_SECTOR_COUNT)
        metrics[FAMILY_METRIC_CELL_DISTINCT_SECTOR_COUNT] =
            count_q16(distinct_sectors);
    if (metric_count > FAMILY_METRIC_CELL_DOMINANT_SECTOR_ID)
        metrics[FAMILY_METRIC_CELL_DOMINANT_SECTOR_ID] =
            dominant_sector < 0 ? -Q16_ONE : count_q16(dominant_sector);
    if (metric_count > FAMILY_METRIC_CELL_DOMINANT_SECTOR_SHARE_Q16)
        metrics[FAMILY_METRIC_CELL_DOMINANT_SECTOR_SHARE_Q16] = total_buildings > 0
            ? static_cast<int32_t>(std::clamp<int64_t>(
                dominant_owned * Q16_ONE / total_buildings, 0, Q16_ONE)) : 0;
    if (metric_count > FAMILY_METRIC_CELL_COMPLETE_CHAIN_COUNT) {
        int32_t complete = 0;
        if (building_csr) {
            std::unordered_map<int32_t, uint32_t> owned_tiers;
            std::unordered_map<int32_t, int32_t> catalog_tier_count;
            std::unordered_map<int32_t, uint32_t> catalog_tiers;
            for (size_t type = 0; type < _building_upgrade_family_indices.size();
                 ++type) {
                ...
                if (family < 0 || tier < 0 || tier >= 32) continue;
                const uint32_t bit = 1u << static_cast<uint32_t>(tier);
                if ((catalog_tiers[family] & bit) == 0)
                    ++catalog_tier_count[family];
                catalog_tiers[family] |= bit;
            }
            for (int32_t group_index = _building_cell_offsets[cell];
                 group_index < _building_cell_offsets[cell + 1]; ++group_index) {
                ...
                const int32_t family = _building_upgrade_family_indices[
                    static_cast<size_t>(group.type_id)];
                const int32_t tier = group.type_id < static_cast<int32_t>(
                        _building_upgrade_tiers.size())
                    ? _building_upgrade_tiers[static_cast<size_t>(group.type_id)] : -1;
                if (family < 0 || tier < 0 || tier >= 32) continue;
                owned_tiers[family] |= 1u << static_cast<uint32_t>(tier);
            }
            for (const auto &owned : owned_tiers) {
                const uint32_t required = catalog_tiers[owned.first];
                const int32_t required_count = catalog_tier_count[owned.first];
                if (required_count >= 2 && required != 0 &&
                    (owned.second & required) == required)
                    ++complete;
            }
        }
        metrics[FAMILY_METRIC_CELL_COMPLETE_CHAIN_COUNT] = count_q16(complete);
    }
    if (metric_count > FAMILY_METRIC_CELL_HAS_EXTRACTIVE_RESOURCE) {
        bool extractive = extractive_groups > 0;
        if (!extractive && !_resource_snapshot.empty()) {
            const size_t resources = _resource_ids.size();
            for (size_t resource = 0; resource < resources && !extractive; ++resource) {
                const size_t idx = resource * static_cast<size_t>(_cell_count) +
                    static_cast<size_t>(cell);
                if (idx < _resource_snapshot.size() && _resource_snapshot[idx] > 0)
                    extractive = true;
            }
        }
        metrics[FAMILY_METRIC_CELL_HAS_EXTRACTIVE_RESOURCE] =
            extractive ? Q16_ONE : 0;
    }
    if (metric_count > FAMILY_METRIC_CELL_LEGAL_BUILDING_TYPE_COUNT) {
        int32_t legal = 0;
        const int32_t country = cell < static_cast<int32_t>(_epoch_cell_country.size())
            ? _epoch_cell_country[static_cast<size_t>(cell)] : -1;
        if (country >= 0 &&
            country + 1 < static_cast<int32_t>(_epoch_country_building_type_offsets.size()))
            legal = std::max(0, _epoch_country_building_type_offsets[country + 1] -
                _epoch_country_building_type_offsets[country]);
        metrics[FAMILY_METRIC_CELL_LEGAL_BUILDING_TYPE_COUNT] = count_q16(legal);
    }
    if (metric_count > FAMILY_METRIC_CELL_VACANT_PROFESSION_COUNT)
        metrics[FAMILY_METRIC_CELL_VACANT_PROFESSION_COUNT] =
            count_q16(vacant_professions);
    if (metric_count > FAMILY_METRIC_FAMILY_BRANCH_COUNT ||
        metric_count > FAMILY_METRIC_FAMILY_REMOTE_BRANCH_COUNT) {
        int32_t branches = 0;
        int32_t remote = 0;
        const int32_t home = family_index >= 0 && family_index < static_cast<int32_t>(
                _families.home_cell.size())
            ? _families.home_cell[family_index] : -1;
        for (int32_t i = 0; i < static_cast<int32_t>(_family_influences.active.size());
             ++i) {
            if (_family_influences.active[i] == 0 || family_handle == 0 ||
                _family_influences.family_handle[i] != family_handle)
                continue;
            ++branches;
            if (_family_influences.cell[i] != home) ++remote;
        }
        if (metric_count > FAMILY_METRIC_FAMILY_BRANCH_COUNT)
            metrics[FAMILY_METRIC_FAMILY_BRANCH_COUNT] = count_q16(branches);
        if (metric_count > FAMILY_METRIC_FAMILY_REMOTE_BRANCH_COUNT)
            metrics[FAMILY_METRIC_FAMILY_REMOTE_BRANCH_COUNT] = count_q16(remote);
    }
    if (metric_count > FAMILY_METRIC_FAMILY_CASH_PER_CAPITA_VS_CELL_Q16 &&
        family_handle != 0) {
        const int64_t family_people = std::max<int64_t>(1,
            family_population(family_handle));
        const int64_t family_cash = family_cash_claim(family_handle);
        const int64_t family_capita = family_cash / family_people;
        const int64_t cell_people = std::max<int64_t>(1, cell_population);
        const int64_t cell_capita = cell_funds / cell_people;
        metrics[FAMILY_METRIC_FAMILY_CASH_PER_CAPITA_VS_CELL_Q16] = cell_capita > 0
            ? std::clamp<int64_t>(family_capita * Q16_ONE / cell_capita, 0,
                4 * Q16_ONE)
            : (family_capita > 0 ? 4 * Q16_ONE : 0);
    }
    if (metric_count > FAMILY_METRIC_CELL_KNOWLEDGE_BUILDING_CLASS_COUNT)
        metrics[FAMILY_METRIC_CELL_KNOWLEDGE_BUILDING_CLASS_COUNT] =
            count_q16(knowledge_groups);
    if (metric_count > FAMILY_METRIC_CELL_CAN_PRODUCE_CORN)
        metrics[FAMILY_METRIC_CELL_CAN_PRODUCE_CORN] =
            can_produce_corn ? Q16_ONE : 0;
}

bool NativeEconomyRuntime::evaluate_family_behavior_conditions(
        int32_t edge, const int64_t *metrics, int32_t metric_count) const {
    if (edge < 0 || edge + 1 >= static_cast<int32_t>(
            _family_trait_behavior_condition_offsets.size()) ||
        metrics == nullptr)
        return false;
    const int32_t begin = _family_trait_behavior_condition_offsets[edge];
    const int32_t end = _family_trait_behavior_condition_offsets[
        static_cast<size_t>(edge) + 1];
    if (begin == end) return true;
    constexpr int32_t MAX_STACK = 8;
    std::array<uint8_t, MAX_STACK> stack{};
    int32_t sp = 0;
    auto metric = [&](int32_t id) -> int64_t {
        if (id < 0 || id >= metric_count) return 0;
        return metrics[id];
    };
    for (int32_t i = begin; i < end; ++i) {
        const int32_t op = _family_trait_behavior_condition_ops[i];
        const int32_t arg0 = _family_trait_behavior_condition_arg0[i];
        const int64_t value = _family_trait_behavior_condition_values[i];
        bool result = false;
        switch (op) {
            case 1: result = true; break;
            case 2: result = metric(arg0) >= value; break;
            case 3: result = metric(arg0) <= value; break;
            case 4: result = metric(arg0) == value; break;
            case 5: return false;
            case 6:
                if (sp < 2) return false;
                result = stack[sp - 2] != 0 && stack[sp - 1] != 0;
                --sp;
                stack[sp - 1] = result ? 1 : 0;
                continue;
            case 7:
                if (sp < 2) return false;
                result = stack[sp - 2] != 0 || stack[sp - 1] != 0;
...
int32_t NativeEconomyRuntime::family_trait_behavior_factor_q16(
        ...
        int32_t selector_id, int32_t cell) const {
    const uint32_t family_index = static_cast<uint32_t>(family_handle & 0xffffffffULL);
    const uint32_t generation = static_cast<uint32_t>(family_handle >> 32U);
    if (_family_behavior_factor_offsets.size() < 2 ||
        family_index >= _family_behavior_factor_offsets.size() - 1 ||
        family_index >= _families.generation.size() ||
        _families.active[family_index] == 0 ||
        _families.generation[family_index] != generation)
        return Q16_ONE;
    const int32_t begin = _family_behavior_factor_offsets[family_index];
    const int32_t end = _family_behavior_factor_offsets[family_index + 1];
    ...
    return found ? static_cast<int32_t>(std::clamp<int64_t>(factor, 0, 4 * Q16_ONE))
        : Q16_ONE;
}

int32_t NativeEconomyRuntime::family_behavior_score_term_q16(
        uint64_t family_handle, int32_t cell, int32_t score_term) const {
    if (score_term <= FAMILY_SCORE_CANDIDATE_WEIGHT) return Q16_ONE;
    const uint32_t family_index = static_cast<uint32_t>(family_handle & 0xffffffffULL);
    const uint32_t generation = static_cast<uint32_t>(family_handle >> 32U);
    if (_family_behavior_factor_offsets.size() < 2 ||
        family_index >= _family_behavior_factor_offsets.size() - 1 ||
    ...
    for (int32_t row = begin; row < end; ++row) {
        const FamilyBehaviorFactorRow &entry = _family_behavior_factor_rows[
            static_cast<size_t>(row)];
        if (entry.score_term != score_term) continue;
        if (entry.cell >= 0 && entry.cell != cell) continue;
        factor = factor * entry.factor_q16 / Q16_ONE;
        found = true;
    }
...
int32_t NativeEconomyRuntime::family_purchase_pay_factor_q16(
        int32_t cohort_slot) const {
    if (cohort_slot < 0 || cohort_slot >= static_cast<int32_t>(
            _population.active.size()) || _population.active[cohort_slot] == 0 ||
    ...
    for (int32_t p = _family_cohort_offsets[cohort_slot];
        ...
        int32_t family = -1;
        if (!_families.valid_handle(edge.family_handle, family)) continue;
        int32_t factor = Q16_ONE;
        if (family >= 0 && family < static_cast<int32_t>(
                _family_purchase_factor_q16.size()))
            factor = _family_purchase_factor_q16[family];
        if (factor >= Q16_ONE) continue;
        const int64_t share = mul_div_sat(edge.people, Q16_ONE, population, sat);
        const int64_t discount = mul_div_sat(share, Q16_ONE - factor, Q16_ONE, sat);
        pay = saturating_sub(pay, discount, sat);
    }
    return static_cast<int32_t>(std::clamp<int64_t>(pay, 0, Q16_ONE));
}

int32_t NativeEconomyRuntime::family_free_building_type_id(
        const Command &cmd) const {
    const int32_t authored = cmd.i32_1;
    if (authored >= 0 && authored < static_cast<int32_t>(_building_types.size()))
        return authored;
    const int32_t from_payload = static_cast<int32_t>(cmd.i64_1);
    if (from_payload >= 0 &&
        from_payload < static_cast<int32_t>(_building_types.size()))
        return from_payload;
    return -1;
}
...
int32_t NativeEconomyRuntime::building_local_resource_abundance_q16(
    ...
    if (cell < 0 || cell >= _cell_count || type_id < 0 ||
        ...
        return Q16_ONE;
    const BuildingType &type = _building_types[type_id];
    if (type.resource_count <= 0) return Q16_ONE;
    int64_t total = 0;
    int32_t counted = 0;
    int64_t sat = 0;
    ...
    if (counted <= 0) return Q16_ONE;
    return static_cast<int32_t>(std::clamp<int64_t>(
        total / counted, 0, Q16_ONE));
}
...
int32_t NativeEconomyRuntime::cell_profession_share_q16(
    ...
    int64_t employed = 0;
    _population.for_each_in_cell(cell, [&](int32_t slot) {
        population += std::max<int64_t>(0, _population.population[slot]);
        const int32_t signature = static_cast<int32_t>(_population.signature_id[slot]);
        if (signature < 0 || signature >= static_cast<int32_t>(_signatures.size()))
            return;
        if (_signatures[signature].profession_id != profession_id) return;
        employed += std::max<int64_t>(0, _population.owner_employed[slot]);
        employed += std::max<int64_t>(0, _population.employee_employed[slot]);
    });
    if (population <= 0) return 0;
    int64_t sat = 0;
    return static_cast<int32_t>(std::clamp<int64_t>(
        mul_div_sat(employed, Q16_ONE, population, sat), 0, Q16_ONE));
}
...
int32_t NativeEconomyRuntime::family_consumption_factor_q16(
    ...
    for (int32_t p = _family_cohort_offsets[cohort_slot];
        ...
        const int32_t factor = family_trait_behavior_factor_q16(
            edge.family_handle, 2, 0, need_id, cell);
        weighted = saturating_add(weighted,
...
int32_t NativeEconomyRuntime::family_good_consumption_factor_q16(
    ...
    if (!_family_traits.empty() && _family_cohort_offsets.size() ==
        ...
        for (int32_t p = _family_cohort_offsets[cohort_slot];
            ...
            if (edge.people <= 0 || edge.family_handle == 0) continue;
            const int32_t factor = family_trait_behavior_factor_q16(
                edge.family_handle, 3, 0, good_id, cell);
            weighted = saturating_add(weighted,
...
uint64_t NativeEconomyRuntime::preferred_family_for_cohort(
    ...
    const int32_t cell = _population.page_cell[cohort_slot / COHORT_PAGE_SIZE];
    const int32_t profession_share = axis == 1
        ? cell_profession_share_q16(cell, selector_id) : 0;
    uint64_t best = 0;
    ...
    for (int32_t p = _family_cohort_offsets[cohort_slot];
        ...
        int32_t factor = family_trait_behavior_factor_q16(
            edge.family_handle, axis, selector_kind, selector_id, cell);
        if (axis == 1) {
            const int32_t popularity = family_behavior_score_term_q16(
                edge.family_handle, cell, FAMILY_SCORE_LOCAL_POPULARITY);
            if (popularity != Q16_ONE)
                factor = static_cast<int32_t>(mul_div_sat(factor,
                    Q16_ONE + mul_div_sat(popularity - Q16_ONE,
                        profession_share, Q16_ONE, sat), Q16_ONE, sat));
            const int32_t mobility = family_behavior_score_term_q16(
                edge.family_handle, cell, FAMILY_SCORE_CAREER_MOBILITY);
            if (mobility != Q16_ONE)
                factor = static_cast<int32_t>(mul_div_sat(factor, mobility,
                    Q16_ONE, sat));
        }
        if (factor <= Q16_ONE) continue;
...
void NativeEconomyRuntime::rebuild_family_effect_binding_index() {
    _family_effect_binding_by_instance.clear();
    _family_effect_instances_by_branch.clear();
    _family_effect_instances_by_cell.clear();
    for (size_t index = 0; index < _family_effect_bindings.size(); ++index) {
        const FamilyEffectBinding &binding = _family_effect_bindings[index];
        if (binding.instance_id <= 0) continue;
        _family_effect_binding_by_instance[binding.instance_id] = index;
        _family_effect_instances_by_branch[binding.branch_handle].push_back(
            binding.instance_id);
        int32_t branch = -1;
        if (_family_influences.valid_handle(binding.branch_handle, branch))
...
void NativeEconomyRuntime::add_family_effect_binding(
        FamilyEffectBinding binding) {
    if (binding.instance_id <= 0 ||
        _family_effect_binding_by_instance.find(binding.instance_id) !=
            _family_effect_binding_by_instance.end())
        return;
    const size_t index = _family_effect_bindings.size();
    _family_effect_bindings.push_back(std::move(binding));
    const FamilyEffectBinding &stored = _family_effect_bindings.back();
    _family_effect_binding_by_instance[stored.instance_id] = index;
    _family_effect_instances_by_branch[stored.branch_handle].push_back(
        stored.instance_id);
...
void NativeEconomyRuntime::remove_family_effect_binding(size_t index) {
    if (index >= _family_effect_bindings.size()) return;
    const FamilyEffectBinding removed = _family_effect_bindings[index];
    _family_effect_binding_by_instance.erase(removed.instance_id);
    auto branch = _family_effect_instances_by_branch.find(removed.branch_handle);
    if (branch != _family_effect_instances_by_branch.end()) {
        auto &ids = branch->second;
        ids.erase(std::remove(ids.begin(), ids.end(), removed.instance_id),
                  ids.end());
        if (ids.empty()) _family_effect_instances_by_branch.erase(branch);
    }
    int32_t branch_index = -1;
    if (_family_influences.valid_handle(removed.branch_handle, branch_index)) {
        auto cell = _family_effect_instances_by_cell.find(
            _family_influences.cell[branch_index]);
        if (cell != _family_effect_instances_by_cell.end()) {
            auto &ids = cell->second;
            ids.erase(std::remove(ids.begin(), ids.end(), removed.instance_id),
                      ids.end());
            if (ids.empty()) _family_effect_instances_by_cell.erase(cell);
        }
    }
...
int64_t NativeEconomyRuntime::family_effect_metric_revision(int32_t phase) const {
    ...
    if (day >= (std::numeric_limits<int64_t>::max() - PHASE_COUNT) /
                   PHASE_COUNT)
        return std::numeric_limits<int64_t>::max();
    return (day + 1) * PHASE_COUNT + std::clamp<int32_t>(phase, 0, 3);
}
...
bool NativeEconomyRuntime::publish_family_effect_metrics(
    ...
    if (!_family_influences.valid_handle(binding.branch_handle, branch) ||
        ...
        return false;
    const uint64_t mask = binding.metric_mask & requested_mask;
    if (mask == 0) return true;
    const int32_t cell = _family_influences.cell[branch];
    std::array<int64_t, FAMILY_METRIC_COUNT> metrics{};
    fill_family_behavior_metrics(family, branch, cell, metrics.data(),
        FAMILY_METRIC_COUNT);
    metrics[FAMILY_METRIC_MAGNITUDE_Q16] = binding.strength_q16;
    std::string error;
    for (int32_t metric_id = 0; metric_id < FAMILY_METRIC_COUNT; ++metric_id) {
        if ((mask & (1ULL << static_cast<uint32_t>(metric_id))) == 0) continue;
        if (!_effect_runtime->set_metric_pod(binding.instance_id, metric_id,
                revision, metrics[static_cast<size_t>(metric_id)], error))
            return false;
    }
    return true;
}

void NativeEconomyRuntime::refresh_family_effect_metrics_for_branch(
        uint64_t branch_handle, int64_t revision, uint64_t requested_mask) {
    const auto found = _family_effect_instances_by_branch.find(branch_handle);
    if (found == _family_effect_instances_by_branch.end()) return;
    const std::vector<int64_t> instance_ids = found->second;
    for (const int64_t instance_id : instance_ids) {
        ...
        publish_family_effect_metrics(_family_effect_bindings[indexed->second],
                                      revision, requested_mask);
    }
}

void NativeEconomyRuntime::refresh_family_effect_metrics_for_cell(
        int32_t cell, int64_t revision, uint64_t requested_mask) {
    const auto found = _family_effect_instances_by_cell.find(cell);
    if (found == _family_effect_instances_by_cell.end()) return;
    const std::vector<int64_t> instance_ids = found->second;
    for (const int64_t instance_id : instance_ids) {
        const auto indexed = _family_effect_binding_by_instance.find(instance_id);
        if (indexed == _family_effect_binding_by_instance.end()) continue;
...
void NativeEconomyRuntime::clear_family_branch_effects(uint64_t branch_handle) {
    ...
    if (_effect_runtime != nullptr) {
        ...
        const auto branch_bindings = _family_effect_instances_by_branch.find(
            branch_handle);
        const std::vector<int64_t> instance_ids = branch_bindings ==
                _family_effect_instances_by_branch.end()
            ? std::vector<int64_t>() : branch_bindings->second;
        for (const int64_t instance_id : instance_ids) {
            const auto indexed = _family_effect_binding_by_instance.find(instance_id);
            if (indexed == _family_effect_binding_by_instance.end()) continue;
...
void NativeEconomyRuntime::reconcile_family_branch_effects(
    ...
    const uint64_t branch_stable_id = static_cast<uint64_t>(
        _family_influences.stable_id[branch]);
    const uint32_t branch_generation = static_cast<uint32_t>(branch_handle >> 32U);
    auto stable_effect_instance_id = [&](const std::string &definition_key) {
        ...
        for (unsigned char ch : definition_key)
            definition_hash = trace_hash_mix(definition_hash, ch);
        identity = trace_hash_mix(identity, definition_hash);
        return static_cast<int64_t>(identity & 0x7fffffffffffffffULL);
    };
    auto resolve_effect_target = [&](const std::string &definition_key,
                                     uint64_t &target_handle,
                                     uint32_t &target_generation,
                                     EffectRuntime::FamilyEffectMetadataPod &metadata)
                                     -> bool {
        if (_effect_runtime == nullptr ||
            !_effect_runtime->family_effect_metadata_pod(
                definition_key, metadata))
            return false;
        const int32_t cell = _family_influences.cell[branch];
        auto source_country = [&]() -> bool {
        ...
        if (metadata.target_selector_kind == 1) { // SOURCE_BRANCH.
            if (metadata.target_domain != 1) return false;
            target_handle = branch_handle;
            target_generation = branch_generation;
            return true;
        }
        ...
        if (metadata.target_selector_kind == 3) { // SOURCE_COUNTRY.
            return metadata.target_domain == 3 && source_country();
        }
        if (metadata.target_selector_kind == 4) { // STATIC_HANDLE.
            const char *begin = metadata.target_selector_id.data();
            const char *end = begin + metadata.target_selector_id.size();
            uint64_t parsed = 0;
            const auto result = std::from_chars(begin, end, parsed);
            if (result.ec != std::errc{} || result.ptr != end || parsed == 0)
                return false;
            target_handle = parsed;
            target_generation = metadata.target_domain == 2 ||
                    metadata.target_domain == 4 || metadata.target_domain == 5
                ? 1 : static_cast<uint32_t>(parsed >> 32U);
            return target_generation != 0;
        }
        if (metadata.target_selector_kind == 5) { // SELECTOR_ID.
            ...
            const bool known = std::binary_search(_building_type_ids.begin(),
                    _building_type_ids.end(), selector) ||
                std::binary_search(_resource_ids.begin(), _resource_ids.end(),
                                   selector) ||
                std::binary_search(_good_ids.begin(), _good_ids.end(), selector);
            if (!known) return false;
            target_handle = static_cast<uint64_t>(cell);
            target_generation = 1;
            return true;
        }
        switch (metadata.target_domain) {
            case 0: // Family.
                target_handle = _family_influences.family_handle[branch];
                target_generation = static_cast<uint32_t>(target_handle >> 32U);
                break;
            case 1: // Branch.
                target_handle = branch_handle;
            ...
            case 5: // Building/resource selector is evaluated within the cell.
                ...
                target_generation = 1;
                break;
            case 3: // Country owning the source branch cell.
    ...
    std::unordered_map<std::string, int32_t> desired_effects;
    std::unordered_map<std::string, int32_t> desired_triggers;
    const uint64_t family_handle = _family_influences.family_handle[branch];
    const auto trait_begin = std::lower_bound(_family_traits.begin(),
        _family_traits.end(), family_handle,
        [](const FamilyTraitRoll &roll, uint64_t handle) {
            return roll.family_handle < handle;
        });
    const auto trait_end = std::upper_bound(trait_begin, _family_traits.end(),
        family_handle, [](uint64_t handle, const FamilyTraitRoll &roll) {
            return handle < roll.family_handle;
        });
    for (auto trait_it = trait_begin; trait_it != trait_end; ++trait_it) {
        const FamilyTraitRoll &roll = *trait_it;
        if (roll.trait_id < 0 || roll.trait_id >= static_cast<int32_t>(
        ...
        if (roll.trait_id < static_cast<int32_t>(
                _family_trait_effect_offsets.size()) - 1) {
            for (int32_t edge = _family_trait_effect_offsets[roll.trait_id];
                 edge < _family_trait_effect_offsets[roll.trait_id + 1]; ++edge) {
                const std::string &key = _family_trait_effect_keys[edge];
                int32_t tier = Q16_ONE;
                const int32_t effect_id = family_effect_id_for_key(key);
                if (effect_id >= 0)
                    tier = family_effect_prestige_magnitude_q16(effect_id, level);
                else {
                    EffectRuntime::FamilyEffectMetadataPod metadata;
                    if (_effect_runtime != nullptr &&
                        _effect_runtime->family_effect_metadata_pod(key, metadata))
                        tier = metadata.magnitude_by_prestige_q16[
                            std::clamp(level, 0, 5)];
                }
                const int64_t scaled = static_cast<int64_t>(tier) *
                    roll.strength_q16 / Q16_ONE;
                int32_t &value = desired_effects[key];
                value = std::clamp<int64_t>(static_cast<int64_t>(value) + scaled,
                    0, 4 * Q16_ONE);
            }
        }
    ...
    auto add_effect_triggers = [&](const std::string &program_key) {
        const int32_t effect_id = family_effect_id_for_key(program_key);
        if (effect_id < 0 ||
            _family_effect_trigger_definition_keys_by_tier.size() <
                static_cast<size_t>(effect_id + 1) * 6U)
            return;
        const std::string &key = _family_effect_trigger_definition_keys_by_tier[
            static_cast<size_t>(effect_id) * 6U + static_cast<size_t>(
                std::clamp(level, 0, 5))];
        if (key.empty()) return;
        const int32_t reward = effect_id < static_cast<int32_t>(
                _family_effect_trigger_reward_targets.size())
            ? _family_effect_trigger_reward_targets[static_cast<size_t>(effect_id)] : 0;
        desired_triggers[key] = reward;
    };
    if (_effect_runtime != nullptr) {
        const auto branch_bindings = _family_effect_instances_by_branch.find(
            branch_handle);
        const std::vector<int64_t> existing_instance_ids = branch_bindings ==
                _family_effect_instances_by_branch.end()
            ? std::vector<int64_t>() : branch_bindings->second;
        for (const int64_t existing_instance_id : existing_instance_ids) {
            const auto indexed = _family_effect_binding_by_instance.find(
            ...
            const bool trait_source = _effect_runtime->family_effect_metadata_pod(
                    definition_key, metadata) && metadata.source_kind == 0;
            if (!trait_source) {
                int32_t magnitude = metadata.magnitude_by_prestige_q16[
                    std::clamp(level, 0, 5)];
                const int32_t effect_id = family_effect_id_for_key(definition_key);
                if (effect_id >= 0)
                    magnitude = family_effect_prestige_magnitude_q16(effect_id, level);
                magnitude = std::clamp(magnitude, 0,
                    static_cast<int32_t>(4 * Q16_ONE));
                if (submit_changes && magnitude != binding.strength_q16) {
                    std::string error;
            ...
            bool consumed = strength == binding.strength_q16;
            if (submit_changes && strength != binding.strength_q16) {
                std::string error;
                if (strength == 0) {
                    if (_effect_runtime->retire_instance_pod(binding.instance_id,
                            binding.generation, _current_day, error)) {
            ...
            if (consumed && (found == desired_effects.end() ||
                    binding.strength_q16 == 0))
                desired_effects.erase(definition_key);
            else if (consumed)
        ...
        for (const auto &item : desired_effects) {
            ...
            uint64_t base_target_handle = 0;
            uint32_t base_target_generation = 0;
            EffectRuntime::FamilyEffectMetadataPod metadata;
            if (!resolve_effect_target(item.first, base_target_handle,
                    base_target_generation, metadata))
                continue;
            add_effect_triggers(item.first);
            const bool event_once = metadata.lifecycle == 2;
            std::vector<int32_t> target_cells;
            collect_family_effect_target_cells(_family_influences.cell[branch],
                metadata.target_selector_kind, target_cells);
            if (target_cells.empty())
                target_cells.push_back(_family_influences.cell[branch]);
            for (int32_t target_cell : target_cells) {
                uint64_t identity = trace_hash_mix(1469598103934665603ULL,
                    branch_stable_id);
                identity = trace_hash_mix(identity, static_cast<uint32_t>(
                    _family_influences.cell[branch]));
                ...
                uint64_t definition_hash = 1469598103934665603ULL;
                for (unsigned char ch : item.first)
                    definition_hash = trace_hash_mix(definition_hash, ch);
                identity = trace_hash_mix(identity, definition_hash);
                const int64_t instance_id = static_cast<int64_t>(identity &
                    0x7fffffffffffffffULL);
                uint64_t target_handle = base_target_handle;
                ...
                if (!submit_changes) {
                    if (event_once || _effect_runtime->has_instance_pod(
                            instance_id, branch_generation)) {
                        add_family_effect_binding({branch_handle,
                            item.first, item.second, instance_id, branch_generation,
                            metadata.target_domain, target_handle,
                            target_generation, metadata.metric_mask});
                    }
                    continue;
                }
                if (event_once) {
                    add_family_effect_binding({branch_handle, item.first,
                        item.second, instance_id, branch_generation,
        ...
        std::vector<std::string> sibling_independent;
        for (const FamilyEffectBinding &binding : _family_effect_bindings) {
            if (binding.branch_handle == branch_handle) continue;
            int32_t other_branch = -1;
            int32_t other_family = -1;
            if (!_family_influences.valid_handle(binding.branch_handle,
                    other_branch) ||
                !_families.valid_handle(
                    _family_influences.family_handle[other_branch],
                    other_family) ||
                other_family != family)
                continue;
            EffectRuntime::FamilyEffectMetadataPod metadata;
            if (_effect_runtime == nullptr ||
                ...
                continue;
            bool already = false;
            for (const FamilyEffectBinding &mine : _family_effect_bindings) {
                if (mine.branch_handle == branch_handle &&
                    mine.definition_key == binding.definition_key) {
                    already = true;
                    break;
                }
            }
            if (!already)
                sibling_independent.push_back(binding.definition_key);
        }
        std::sort(sibling_independent.begin(), sibling_independent.end());
        sibling_independent.erase(std::unique(sibling_independent.begin(),
            sibling_independent.end()), sibling_independent.end());
        for (const std::string &key : sibling_independent) {
            const int64_t instance_id = stable_effect_instance_id(key);
            uint64_t target_handle = 0;
            uint32_t target_generation = 0;
            EffectRuntime::FamilyEffectMetadataPod metadata;
            if (!resolve_effect_target(key, target_handle, target_generation,
                    metadata))
                continue;
            const int32_t magnitude = family_effect_prestige_magnitude_q16(
                family_effect_id_for_key(key), level);
            if (!submit_changes) {
                if (_effect_runtime->has_instance_pod(instance_id,
                        branch_generation)) {
                    add_family_effect_binding({branch_handle, key, magnitude,
                        instance_id, branch_generation, metadata.target_domain,
            ...
            add_family_effect_binding({branch_handle, key, magnitude, instance_id,
                branch_generation, metadata.target_domain, target_handle,
                target_generation, metadata.metric_mask});
            const auto indexed = _family_effect_binding_by_instance.find(instance_id);
            if (indexed != _family_effect_binding_by_instance.end())
                publish_family_effect_metrics(
        ...
        grant_random_pool_family_effect(family, submit_changes);
        if (!submit_changes) {
            for (const std::string &key : _family_effect_keys) {
                EffectRuntime::FamilyEffectMetadataPod metadata;
                if (!_effect_runtime->family_effect_metadata_pod(key, metadata) ||
                    metadata.source_kind == 0)
                    continue;
                bool already = false;
                for (const FamilyEffectBinding &mine : _family_effect_bindings) {
                    if (mine.branch_handle == branch_handle &&
                        mine.definition_key == key) {
                        already = true;
                        break;
                    }
                }
...
void NativeEconomyRuntime::split_family_branches() {
    ...
    for (const Candidate &candidate : candidates) {
        ...
        const uint16_t split_flags = _families.flags[parent];
        const bool retain_only =
            (split_flags & FAMILY_FLAG_SPLIT_RETAIN_ONLY) != 0;
        const bool bonus_weight =
            !retain_only && (split_flags & FAMILY_FLAG_SPLIT_BONUS_WEIGHT) != 0;
        const bool replace_all =
            !retain_only && !bonus_weight &&
            (split_flags & FAMILY_FLAG_SPLIT_REPLACE) != 0;
        const int32_t weight_num = bonus_weight
            ? std::max<int32_t>(128, static_cast<int32_t>(
                split_flags >> FAMILY_FLAG_SPLIT_WEIGHT_SHIFT))
            : 128;
        ...
        const int32_t removed_index = retain_only || replace_all ||
            removable_indices.empty() ? -1 :
        ...
        auto legal_trait = [&](int32_t trait_id) {
            ...
            return family_trait_context_allows(trait_id, child);
        };
        const int32_t additions = retain_only ? 0 : 1 + static_cast<int32_t>(
        ...
        for (int32_t add = 0; add < additions; ++add) {
            ...
            for (int32_t trait_id = 0; trait_id < static_cast<int32_t>(
                ...
                candidates.push_back(trait_id);
                int64_t weight = std::max<int32_t>(1, _family_trait_weights[trait_id]);
                if (bonus_weight)
                    weight = std::max<int64_t>(1, weight * weight_num / 128);
                total_weight += weight;
            }
            if (candidates.empty() || total_weight <= 0) break;
            ...
            for (int32_t trait_id : candidates) {
                int64_t weight = std::max<int32_t>(1,
        ...
        if ((split_flags & (FAMILY_FLAG_SPLIT_GIFT_BUILDING |
            ...
            int32_t gift_type = -1;
            for (const FamilyBuildingOwnership &ownership : _family_ownerships) {
                if (ownership.family_handle != child_handle ||
                    ownership.owned_count <= 0) continue;
                const int32_t group = building_index_for_handle(
                    ownership.building_handle);
                if (group >= 0 && group < static_cast<int32_t>(_buildings.size()) &&
                    _buildings[static_cast<size_t>(group)].cell == candidate.cell) {
                    gift_type = _buildings[static_cast<size_t>(group)].type_id;
                    break;
                }
            }
            PendingFamilySplitGift gift;
            gift.family_handle = child_handle;
            gift.cell = candidate.cell;
            gift.flags = static_cast<uint16_t>(split_flags &
                (FAMILY_FLAG_SPLIT_GIFT_BUILDING | FAMILY_FLAG_SPLIT_GIFT_POPULATION));
            gift.building_type_id = gift_type;
            gift.population = 8;
            _pending_family_split_gifts.push_back(gift);
        }
        _family_indices_dirty = true;
        ++_families_formed;
    }
...
void NativeEconomyRuntime::dissolve_family(uint64_t family_handle) {
    ...
    reset_family_policy_factors(index);
...
void NativeEconomyRuntime::rebuild_family_influences(bool rebuild_derived) {
    ...
    for (const auto &item : aggregates) {
        ...
        if (review_due && _family_influences.last_review_day[branch] != _current_day) {
            ...
            if (target == current) {
                ...
                if (_family_influences.review_streak[branch] >= 2) {
                    ...
                    if (target > current) {
                        int32_t family_index = -1;
                        if (_families.valid_handle(key.family, family_index))
                            fire_family_event_once_effect(family_index,
                                "family.effect.new_nobility");
                    }
                }
            }
        }
        const uint64_t branch_handle =
            _family_influences.handle_for_index(branch);
        constexpr uint64_t FAMILY_COMMIT_METRICS =
            ((1ULL << FAMILY_METRIC_COUNT) - 1ULL) ^ 1ULL;
        refresh_family_effect_metrics_for_branch(branch_handle,
            family_effect_metric_revision(1), FAMILY_COMMIT_METRICS);
        if (changed) reconcile_family_branch_effects(branch_handle, true);
    }
...
void NativeEconomyRuntime::move_family_membership(
    ...
    const int32_t dest_cell = _population.valid_handle(destination_handle,
            destination_slot)
        ? _population.page_cell[destination_slot / COHORT_PAGE_SIZE] : -1;
...
int64_t NativeEconomyRuntime::state_hash() const {
    ...
    for (const BuildingGroup &group : _buildings) {
        ...
        mix_u64(static_cast<uint64_t>(group.last_market_receipt));
        mix_u64(static_cast<uint64_t>(group.last_bullion_mint_receipt));
        mix_u64(static_cast<uint64_t>(group.last_producer_support_receipt));
        mix_u64(static_cast<uint64_t>(group.last_business_tax_paid));
        mix_u64(static_cast<uint64_t>(group.last_business_subsidy_received));
        mix_u64(static_cast<uint64_t>(group.last_maintenance_due));
        mix_u64(static_cast<uint64_t>(group.last_observed_capacity_days_q16));
        mix_u64(static_cast<uint64_t>(group.last_quoted_market_receipt));
        mix_u64(static_cast<uint64_t>(group.last_quoted_operating_cost));
    }
    ...
    for (const uint64_t request_id : fiscal_peer_ids) {
        const FiscalPeerJournalRecord &record =
            _asset_peer_journal.at(request_id);
...
Dictionary NativeEconomyRuntime::reset(const String &reason) {
    ...
    _environment_precipitation_q16.clear();
    ...
    _city_factor_dirty_cells.clear();
    _city_output_shared_goods_q16.clear();
    _city_output_cell_offsets.clear();
    _city_output_good_indices.clear();
    _city_output_factors_q16.clear();
    _city_output_scope_cells_scratch.clear();
    _city_output_scope_stat_ids_scratch.clear();
