...
RUNS={
 'v5_cell1031':"D:/Godot/ProjectKeynes/Project.Keynes/tmp/economy_record_20260717_114418_v5_cell1031_q3_r17_",
 'v7_cell1060':"D:/Godot/ProjectKeynes/Project.Keynes/tmp/economy_record_20260717_140702_v7_cell1060_q32_r17_",
 'v7_cell1166':"D:/Godot/ProjectKeynes/Project.Keynes/tmp/economy_record_20260717_152531_v7_cell1166_q17_r19_",
}
...
BTYPE={30:'communal_hearth',65:'flint_quarry',69:'gathering_ground',79:'weaving_shelter',90:'knapping_workshop',
       102:'fish_collector',106:'merchant_post',203:'placer_gold',238:'hunting_camp',239:'stone_collector',242:'subsist_farm'}
def col(df,name): return df[name] if name in df.columns else pd.Series([0]*len(df))
def load(b):
...
def compute(tag,D):
    ...
    out['global']={'day':days,'labor_owner':s.filled_owner_jobs.tolist(),
        ...
        'res_gen':s.building_resource_generated.tolist(),'res_con':s.building_resource_consumed.tolist()}
    ...
    wg=rs('wild_game')
    out['kpi']={
        'day_first':int(cdays[0]),'day_last':int(cdays[-1]),
        ...
        'hunter_last':int(fL[fL.profession_id==12].population.sum()),
        'artisan_last':int(fL[fL.profession_id==2].population.sum()),
        ...
        'suspended_last':int(s.loss_suspended_building_groups.iloc[-1]),'loss_suspended_peak':int(s.loss_suspended_building_groups.max()),
        ...
        'pending_construction_last':int(col(s,'pending_construction_count').iloc[-1]),
        'births_total':int(col(s,'births').sum()),
        'deaths_total':int(col(s,'deaths').sum()),
        'wild_game_depleted_pct':round(100*(1-wg[-1]/max(1,wg[0])),1),
        'audit_ok':bool((s.population_error==0).all() and (s.money_error==0).all() and (s.goods_error==0).all()),
        'professions':[PROF.get(p,str(p)) for p in sorted(c.profession_id.unique())],
    }
    return out
runs={t:compute(t,load(p)) for t,p in RUNS.items()}
out={'runs':runs}
def _conv(o):
...
json.dump(out,open("D:/Godot/ProjectKeynes/Project.Keynes/tmp/econ_compare_three.json","w"),ensure_ascii=False,default=_conv)
print("=== KPI three-way ===")
order=['day_last','cell_pop_first','cell_pop_last','cell_pop_delta_pct','hunter_last','artisan_last',
       'elite_share_last','sat_last','suspended_last','loss_suspended_peak','merch_spend_ratio_last',
       'pending_construction_last','births_total','deaths_total','wild_game_depleted_pct','audit_ok']
for k in order:
    print("  %-22s"%k, "  ".join("%s=%s"%(t,runs[t]['kpi'][k]) for t in runs))
print("saved econ_compare_three.json")
