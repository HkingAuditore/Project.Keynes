...
- 家庭购买直接使用交易税后的预算报价。正税压低同一预算可购买数量；负税只有在本 cell
  已取得财政预留后才降低报价。
- 建筑招聘和职业转换统一比较税后、扣生活成本后的可支配收入；雇员为税后工资减目标签名
  生活成本，业主沿用已扣经营投入、工资、维护、营业税、所得税和业主生活成本的预测收入。
  失业来源的劳动/经营收益按零处理，但其失业职业对应的负所得税补贴按冻结
  `survival_household` 日生活成本和当前财政履约率通过 `expected_fiscal_transfer` 计入保留收益；
  迁移继承的存款、`epoch_income/expense` 和 `income_ema` 不充当当前失业收入。消费补贴只进入
  税后生活成本报价，不能再作为现金收入重复相加。收入改善率只相对来源保留收益计算；目标值
  已经扣过生活成本，不得再以来源或目标生活成本作分母重复施加生存门槛。无补贴失业者因此接受
  任何严格为正的税后净岗位收益，有补贴时仍须达到既有相对改善阈值。
  岗位吸引力还乘既有 labor pay ratio 与 planned utilization，有限 mobility budget 按
  ethnicity 比例分配，避免税后名义高薪岗位在一个周期集中吸收全部人口。
  生存、短缺、满意度和卖出率不再作为经营偏好键，物理执行约束仍然有效。
...
- 建筑预测先读取同一冻结 epoch 的事实锚：安装组按实际 owner/role fill 的最小容量缩放，
  ...
  `min(workforce, planned, climate)` 执行比例，避免把周期总额或到岗能力重复放大；实际零回款
  不回退到名义报价，生存品仅可在冻结生存下限内获得有界现金独立执行探针。
- 负税率不按名义全额进入预期。预期补贴按“本批财政预算 / 上批申请”确定性折算；首次启用
  交易税或营业税且没有历史申请时预期兑现为零。负所得税例外：已冻结的人口最低生活申请
...
- 负所得税改在全部应税来源完成后按 cohort 汇总结算。每个 cohort 的最低生活税基为
  ...
  负税，正税绝不对推定收入征收。
- 交易税作用于最终吸收点的家庭订单（以及生产投入的最终购买报价）。正税进入买方报价，
  商人仍只取得商品基准成交价；
  负税按本 cell 已预留额度和稳定订单顺序降低预算价，实际成交才扣用托管额度。
- 营业税正税按建筑本批实际取得的生产者回款计税，多输出建筑汇总实际回款；金银按
  `monetary_issue_value` 取得的铸币发行回款同样属于营业税税基，但仍不属于所得税税基。负税只按
  未税投入、实际支付工资和维护费补贴，不含业主生活成本、奖金、债务和税费。正税源头扣除并可从业主
  经营所得税基扣除；负税付给业主且不形成新的应税所得。
- 跨格贸易中的交易税正税计入买方最终到价；负交易税不在库存循环中兑现，只在家庭或企业最终
  吸收时从相应财政 lane 支付。
...
所得税在预算不足时按申请额同比例分配，稳定前缀处理整数余数。交易税和营业税仍由 worker
...
- PKCN v12 保存五类默认率、稀疏覆盖、政策版本和 Country Modifier domain。
- PKEC v47 保存逐 cell 上批补贴权重、generation-safe 国家 handle、财政累计值、确定性 hash、
  交易订单的交易税转移列，以及建筑市场/铸币/托底回执、维护应付、实际容量和冻结报价字段。
...
税务页位于国家“经济”工作区内部，包含国库、所得税、交易税、营业税和关税分页；关税
