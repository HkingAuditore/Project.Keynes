...
func _run() -> void:
	...
	_expect("PKEC v27 save begins", bool(save_begin.get("ok", false))
		and int(save_begin.get("schema_version", 0)) == 27)
	...
	for _i in 512:
		...
		if chunk.is_empty():
			break
		chunks.append(chunk)
	_expect("PKEC v27 save completes", not chunks.is_empty()
	...
	_expect("PKEC v27 restores family and important-person authority",
