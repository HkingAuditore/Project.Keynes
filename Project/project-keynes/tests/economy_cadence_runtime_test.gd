...
func _test_save_restore_keeps_buckets(compiled: Dictionary) -> void:
	...
	_expect("current economy save begins", bool(begin.get("ok", false)) and
		int(begin.get("schema_version", 0)) >= 51)
	...
	while true:
		...
		if chunk.is_empty():
			break
		chunks.append(chunk)
	_expect("current economy save completes", bool(ext.end_economy_save().get("ok", false)))
	...
	_expect("restore keeps locked cadence",
