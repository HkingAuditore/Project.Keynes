...
func _run() -> int:
	...
	var economic_recorder = load("res://../../tmp/merchant_probe_recorder.gd").new()
	economic_recorder.probe_cells = country_cells
	economic_recorder.bind_main(host)
	economic_recorder.start()
	print("[merchant-probe/start] ", runtime_ext.get_economy_csv_recording_status())
	...
	for day in range(1, total_driver_days + 1):
		economic_recorder.on_fast_tick({})
		if day % 100 == 0:
			print("[merchant-probe/progress] day=", day, " status=", runtime_ext.get_economy_csv_recording_status())
	...
	economic_recorder.stop_and_export()
	for drain in range(10000):
		var recording_status: Dictionary = runtime_ext.get_economy_csv_recording_status()
		if str(recording_status.get("state", "")) in ["completed", "error"]:
			print("[merchant-probe/final] ", recording_status)
			break
		OS.delay_msec(1)
