...
- PKCN v1 保存国家科技；PKEC v13 保存匹配的 PKCN schema/generation/hash、企业停产/实际出库状态以及国内在途贸易。只有策略参数一致的旧 ACTIVE 档可迁移；旧默认 v11 的 25%/1 日商人策略与当前 12.5%/30 日基线策略不一致并明确拒绝，ACTIVE 也拒绝 v11 PROBE/v10；PKEC v2-v9 返回 `legacy_countryless_economy_save_unsupported`。
