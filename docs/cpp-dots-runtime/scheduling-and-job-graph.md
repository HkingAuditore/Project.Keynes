...
- `season_refresh` 现在从 `season_refresh_state_snapshot()` 与 `season_cadence_policy` 进入 native daily bundle。`cell_vegetation` 的权威 writer 是 `vegetation_dynamics`；B+ `sync_current_state` 用 `skip_vegetation_rewrite` 跳过全图植被重判。atlas queue 与 detail scatter 仍是 `retained_boundaries`。ACTIVE native daily round 用 `refresh_slot_keys` 做有界 slot 回灌，不再每轮全 BIND_TABLE `refresh_slots_from_map`。
