...
结论写在这里免得再走一遍：**全屏俯视云层用高度场 + 探针，不要用体积积分。**
散射模型该给的东西，不积分也拿得到。

### 高度场 + 探针的散射模型

一次 `cloud_cumulus_fbm_d()` 拿到云顶高度与解析导数（法线），然后：

| 环节 | 作用 | 实现 |
| --- | --- | --- |
| 自阴影 | 沿太阳水平方向 2~3 个高度探针，量「前方比我高多少」= 光要穿过的云量 | 主循环 |
| 多重散射消光 | 双指数 Beer。单指数把阴影压成死黑，那是烟不是云 | `cloud_beer_ms()` |
| 多重散射回填 | 暗部的主要亮度来源，**有理式**衰减而非指数（见坑三） | 主循环 |
...
| 天空遮蔽 | 塔顶看得见整片天空、谷底看不见 | `sky = mix(valley_sky, 1, top)` |

原语在 `shaders/include/cloud_volume.gdshaderinc`（保留这个文件名：它装的是体积
散射模型，只是不含 march）。

**云海的层次主要来自天空遮蔽与多重散射回填，不是来自法线。** 法线只负责直射的明暗；
把层次押在法线上，结果就是下面这一串坑。

### 形状：billow FBM 高度场

域扭曲（`warp_amount`）+ `cloud_cumulus_fbm_d()`。低频 octave 保持普通梯度噪声，
负责大尺度覆盖起伏；高频转 billow，负责花椰菜团。

`cloud_billow()` 取 `|2n-1|` 的平滑版：在噪声极值处是圆润的最大值、在 `n=0.5` 处
形成折谷，于是高处一团团圆头、低处是尖锐的缝。用 `sqrt(s²+ε)` 而不是 `abs()` 是为
了保住导数连续，否则谷底会出现一条条黑线。全部 octave 都用 billow 会得到一层过于
均匀的泡沫，所以只有高频转。

不扭曲的 FBM 等值线是各向同性的糊团，打多少光都像烟；扭曲之后才有卷曲、走向和撕扯
的云缘。q2 一层扭曲，q3 叠第二层（扭曲的扭曲）。

### 走过的弯路：胞状噪声（已回退）

曾经判断「连续高度场从上方看永远是大理石纹，做不出云海那种一朵一朵的离散感」，于是
把形状换成胞状噪声试了两轮，最后按实际观感回退到 billow FBM。**当前形状就是 FBM，
下面只是留个记录，免得再走一遍。**
...
- **`top_lo`/`top_hi` 与 `shadow_absorb` 必须随高度场分布重调。** 胞场的高度差比 FBM
  大得多，沿用 FBM 的窄窗口会让包心饱和成发光白核、把缝压成死黑。
...
`cloud_cumulus_fbm_d()` 的 `grad_falloff` 给高频再乘一个几何衰减（0.5 = 每高一个
octave 斜率贡献减半），法线因此取自「平滑后」的曲面：大形状决定明暗，细节只做轻微
扰动。返回的导数**不是** value 的真实梯度，这是刻意的。

顺带一提：`N = normalize(vec3(-grad × cloud_relief × hex_world_diameter × cloud_scale, 1))`
里 `hex_world_diameter × cloud_scale` 正好被 `grad` 里的域缩放约掉，所以法线倾斜
只由 `cloud_relief` 和噪声域导数决定，与 hex 尺寸和云团大小无关。

### 坑二：把云当不透明漫反射面 = 没有 GI

云的单次散射反照率≈1，光子在云里弹几十次才出来。所以：

- **背光面不会掉黑。** `light_wrap` 要给足（默认 0.75），硬 `N·L` 是固体的行为。
- **暗部亮度主要来自多重散射回填**（`bounce`），不是环境光。这一项刻意**不吃
  `N·L`** —— 邻近云体的二次照明与局部法线基本无关。少了它就是「仿佛没有 GI」的
  石头感。
- **谷底不是黑的。** `valley_sky` 是天空遮蔽的下限；压太低缝隙就全黑，而真实云谷
  还被四周的云壁照着。
...
- 日月方向近乎相反，用 `step(moon_strength, local_day)` 硬切，**不做向量线性
  混合** —— 晨昏中点会归一化出零向量。
- `key_col = sun_col·local_day + moon_col·moon_strength`（已含强度，下游直接乘）。
- 天空 SH 的方位补光色板走 `mix(moon_col, sun_col, local_day)`，否则午夜会拿白昼
  太阳色去染云。

#### 晨昏线还要额外一道天光地板

切了月光也不够。晨昏线附近 `local_day` 与 `moon_strength` **同时**逼近 0
（前者 `smoothstep(-0.14, 0.14, sinh)`、后者 `smoothstep(0.02, 0.28, -sinh)`，
中间有一段两头都不管的缝），`key_col` 在这里整个塌掉。

地形能扛过去，是因为它的环境光用的是**不打折的** `amb_col`；而雾层的环境光还要再
乘 `sky × ambient_gain ≈ 0.14`，于是晨昏线上云比地形暗一大截。

补一道地板，且**走 `sky_col` 而不是 `amb_col`** —— 补上来的必须是冷蓝，不是灰：

```
floor_w = mix(night_floor, night_floor * 0.35, local_day)
col = max(col, cloud_albedo * sky_col * floor_w)
```

白昼收窄到 1/3：那时算出来的颜色远高于地板，不会把云缝的层次抹平。

### 探针必须和主高度场同频

自阴影探针只取 3 octave（只需要知道「前方那团比我高多少」，省掉高频的开销），而主
高度场取 4~5 octave。**拿低频探针去减全频的主场，差值会被主场独有的高频主导，出来
是斑点噪声而不是方向性投影。** 所以要用与探针完全相同的函数和 octave 数另算一个
`h_ref` 作比较基准。这个坑踩过两次，写在这里。
...
走 `earth_daylight.gdshaderinc` 的逐像素晨昏线，与地形 / 水面 / 植被 / 天气云同一
真源，所以迷雾会随昼夜被晨昏线扫过：正午冷白顶配蓝灰缝隙，黄昏暖橙侧光配拉长的
云间投影（探针距离 ∝ `sun_horiz / sun_up`），夜里只剩冷蓝环境光。
...
夜侧必须留地板（`col = max(col, albedo × amb_col × …)`），否则未探索区会变成一块
死黑。
...
| q1 | 3 octaves + 法线漫反射 + 天空遮蔽 + TOD。无域扭曲、无自阴影探针 | 3 |
| q2 | 4 octaves + 1 层域扭曲 + 2 探针自阴影 + Powder + 银边 + 3S | 4 + 2 + 2×3 = 12 |
| q3 | 5 octaves + 2 层域扭曲 + 3 探针自阴影 + 天空 SH 环境光 | 5 + 4 + 3×3 = 18 |
