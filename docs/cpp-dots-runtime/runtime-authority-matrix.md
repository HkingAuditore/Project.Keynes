...
| Population, market, buildings, notable families and important people, prosperity/settlement identity (including bootstrap capital forced-name bit), taxable events and fiscal escrow | PKEC / `NativeEconomyRuntime` | PKSV `pkec` / PKEC v27 | Restore only after PKCN and trade topology |
...
| Economy/building modifiers | Economy `ModifierStore` + `BuildingIdentityStore` | embedded in PKEC v27 | Factors feed deterministic output/construction/trade helpers, never ledgers directly |
...
| `economy_daily` | `NativeEconomyRuntime` 持有滚动五相结算、stage/cursor、PopulationStore、SettlementStore、MarketStore、FamilyStore、NotablePersonStore、家族成员/建筑所有权及人物需求稀疏边、BUILDING_GRAPH、国内 Trade stores、税率冻结、应税事件、负所得税 cohort 汇总与最低生活税基、补贴权重及 fiscal escrow；国家税表与国库由 `NativeCountryRuntime` 提供。 | 独立 native vectors；家族/人物/繁荣/地名不写 DataCore 或 MapData；due-cell sample 冻结环境、`cell → country`、科技、税率、Modifier factor 和负所得税最低生活申请。 | 有界 continuation 隔离半成品；COMMIT 消费去重人口变化集更新繁荣/名称；`FAMILY_COMMIT` 归一化家族 claim 并约束业主岗位，`PERSON_COMMIT` 绑定人物岗位、归因已实现经济结果并重建 CSR；财政提交要求 escrow 清零；发布 summaries、聚居地 delta、审计与 PKEC v27 stream。 | 无大规模 GDScript fallback；国家桥无效时 disabled。 | 本地市场、家族/人物、繁荣度、税收财政、商人信用和国内贸易默认 **ACTIVE / 每 cell 固定 5 日**。 | 进口/出口关税仅有政策、Modifier、存档和 UI 占位；跨国贸易/外交仍未接入。 |
