...
3. UI-consumed events must not reach world interaction. A focused `LineEdit` or
   ...
   controls also block `handle_input()` direct dispatch, but must not lock
   `_unhandled_input` map gestures. HUD chrome (map overlay, top bar, country
   action bar) uses `FOCUS_NONE`. Empty-map pointer events release leftover
   ordinary UI focus.
