...
func _test_native_publish_event_adapter(country_ir: Dictionary) -> void:
	...
	var orphaned: Dictionary = journal.duplicate(true)
	orphaned.erase("event_id")
	orphaned.erase("tick")
	orphaned.erase("phase")
	orphaned.erase("type")
	orphaned.erase("source")
	orphaned.erase("flags")
	orphaned.erase("entity_handle")
	orphaned.erase("entity_id")
	orphaned.erase("cell_idx")
	orphaned.erase("payload_schema")
	orphaned.erase("value_i64")
	orphaned.erase("payload_i0")
	orphaned.erase("payload_i1")
	orphaned.erase("payload_i2")
	orphaned.erase("payload_i3")
	orphaned["event_id"] = PackedInt64Array()
	orphaned["tick"] = PackedInt64Array()
	orphaned["phase"] = PackedInt32Array()
	orphaned["type"] = PackedInt32Array()
	orphaned["source"] = PackedInt32Array()
	orphaned["flags"] = PackedInt32Array()
	orphaned["entity_handle"] = PackedInt64Array()
	orphaned["entity_id"] = PackedInt32Array()
	orphaned["cell_idx"] = PackedInt32Array()
	orphaned["payload_schema"] = PackedInt32Array()
	orphaned["value_i64"] = PackedInt64Array()
	orphaned["payload_i0"] = PackedInt32Array()
	orphaned["payload_i1"] = PackedInt32Array()
	orphaned["payload_i2"] = PackedInt32Array()
	orphaned["payload_i3"] = PackedInt32Array()
	var orphaned_restored: Object = ClassDB.instantiate("DCWorldExt")
	_expect("journal restores idempotency even when referenced events scrolled out",
		bool(orphaned_restored.restore_gameplay_event_journal(orphaned).get("ok", false))
		and int(orphaned_restored.get_gameplay_event_bus_report().get(
			"effect_idempotency_count", 0)) == 3)
