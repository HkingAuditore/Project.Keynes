...
func _init() -> void:
	...
	_expect("country bootstraps", bool(country.bootstrap(PackedByteArray([0]), {
		...
		"treasury_quantities": PackedInt64Array([2000000]),
	}).get("ok", false)))
	...
	_expect("single-domain completion queues", bool(country.set_research_weights(
		handle, PackedInt32Array([10000, 0, 0, 0]), 0, 1).get("ok", false))
		and bool(country.enqueue_research(handle, &"tech.seasonal_foraging",
	...
	var tech := ids.find("tech.seasonal_foraging")
	...
	_expect("pending state does not expose completion tag",
		...
		and not (country.snapshot(handle).technology_ids as PackedStringArray).has(
			"tech.seasonal_foraging"))
	var stat := "country.research.agriculture_efficiency"
