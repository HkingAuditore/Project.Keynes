...
- `_field_precip_inertia = 0.40`（2026-06-22 生命周期修复：precip 改为 EMA 状态量 `precip = lerp(prev_precip, target, α)`，统一替代旧 carryover/拖尾/滞回三件套。`_field_precip_decay` / `_field_precip_carryover_max` 已退出 C++ 主路径，仅 ClimateProfile/verify 残留）
