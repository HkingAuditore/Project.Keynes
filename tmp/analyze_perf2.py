...
print("\n=== largest_slice attribution (which job dominates each tick) ===")
...
out = dict(nrows=nrows, frame_budget=bud, t_sus=stats(frame), jobs=job_summary,
           largest=dict(counter=dict(lc), worst_ms=float(max(ls_ms))))

# histogram of t_sus (0.5ms buckets, 0..16)
hist = []
...
with open("D:/Godot/ProjectKeynes/Project.Keynes/tmp/perf_analysis2.json", "w") as f:
    json.dump(out, f, indent=2)
print("\nwrote perf_analysis2.json")
