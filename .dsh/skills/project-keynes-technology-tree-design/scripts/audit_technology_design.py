...
def audit_authoring_network(data: dict[str, Any]) -> tuple[list[str], list[str], dict[str, Any]]:
    ...
    if not 360 <= modifier_terms <= 750:
        errors.append(f"modifier term count outside 360-750: {modifier_terms}")
    if nonstarting_nodes and broad_nodes > int(nonstarting_nodes * 0.40):
        errors.append(f"broad-effect nodes exceed 40%: {broad_nodes}/{nonstarting_nodes}")
    for stat, value in family_totals.items():
        if value > 4.000001:
            errors.append(f"{stat}: family modifier stack exceeds +400%")
    for stat, value in broad_totals.items():
        if value > 4.000001:
            errors.append(f"{stat}: broad modifier stack exceeds +400%")
