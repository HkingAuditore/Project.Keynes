...
# 读取采样数据（50000行，覆盖多个tick）
print("[1] 读取数据基本信息...")
df_sample = pd.read_csv('d:/Godot/ProjectKeynes/Project.Keynes/tmp/tile_data_record_20260527_142740.csv', nrows=100000)
...
sample_cells = df_sample['cell_index'].unique()[:5]
for cell_id in sample_cells:
    cell_data = df_sample[df_sample['cell_index'] == cell_id].sort_values('tick_idx')
    if len(cell_data) > 100:  # 至少有100个tick的数据
        ...
        if len(temps) > 365:
            # 检查温度是否每年重复
            temp_diff = np.diff(temps[:730])  # 前2年
            zeros_crossings = np.sum((temp_diff[:-1] < 0) & (temp_diff[1:] > 0))  # 从下降到上升
            print(f"    温度过零点次数(前2年): {zeros_crossings} (期望~2个周期)")
...
# 检查 temp_season_offset 是否变化
print("[6] 检查 temp_season_offset 是否随季节变化")
if 'temp_season_offset_arr' in df_sample.columns:
    for cell_id in sample_cells[:3]:
        cell_data = df_sample[df_sample['cell_index'] == cell_id].sort_values('tick_idx')
        if len(cell_data) > 100:
            ...
            if offsets.std() < 0.001:
                print(f"    [ERROR] season_offset几乎无变化!")
            else:
                print(f"    [OK] season_offset有变化")
            print()

# 检查纬度-温度相关性（跨所有tick）
print("[7] 纬度-温度相关性（最新tick快照）")
...
if len(df_latest) > 100:
    ...
    if corr > -0.3:
        ...
        print(f"  建议: 检查 temp_baseline_year_arr 计算公式")
    else:
        print(f"  [OK] 相关性合理")
