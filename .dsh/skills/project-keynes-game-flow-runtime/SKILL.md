...
per opening country. Allow employee roles only on `placer_gold_working` (1 miner)
and `surface_silver_working` (2 miners); do not prefill those slots. Other
Stone-Age starter buildings must remain owner-only. Require the planner's closed
regional bundle plus the matching precious-metal work site and
`early_merchant_post` per country. Aggregate all starts through EconomyFacade
