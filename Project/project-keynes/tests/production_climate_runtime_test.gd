...
func _test_worker_scalar_and_save(catalog: Dictionary) -> void:
	...
	_expect("PKEC v42 climate save streams at a committed boundary",
		bool(saved.get("ok", false)) and int(saved.get("schema", 0)) == 42)
	if not bool(saved.get("ok", false)):
		return
	var restored := _configured_runtime(catalog, CELL_COUNT, 4403, false)
	var restored_profile := _profile(false)
	_expect("PKEC v42 restore target configures", bool(restored.get("ok", false)) and
	...
	_expect("PKEC v42 round-trip preserves climate state hash",
	...
	_expect("PKEC v42 rejects an illegal climate diagnostic without bootstrapping",
	...
	_expect("PKEC v42 rejects a truncated climate building record without bootstrapping",
...
func _restore_corrupted_building(ext: Object, chunks: Array) -> Dictionary:
	...
	for value in chunks:
		...
		if chunk.size() >= 60 and chunk.decode_u16(6) == 5:
			# Building payload offset 36 is last_temperature_fit_q16 in PKEC v42.
